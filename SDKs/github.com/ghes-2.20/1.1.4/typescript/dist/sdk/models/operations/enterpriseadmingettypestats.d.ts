import { SpeakeasyBase } from "../../../internal/utils";
export declare class EnterpriseAdminGetTypeStatsPathParams extends SpeakeasyBase {
    type: string;
}
export declare class EnterpriseAdminGetTypeStatsRequest extends SpeakeasyBase {
    pathParams: EnterpriseAdminGetTypeStatsPathParams;
}
export declare class EnterpriseAdminGetTypeStatsResponse extends SpeakeasyBase {
    contentType: string;
    statusCode: number;
    enterpriseAdminGetTypeStats200ApplicationJSONAnyOf?: any;
}
