...
use \OpenAPI\OpenAPI\Models\Operations\GetItemSnapshotFeedRequest;
use \OpenAPI\OpenAPI\Models\Operations\GetItemSnapshotFeedSecurity;

$sdk = SDK::builder()
    ->build();

try {
    $request = new GetItemSnapshotFeedRequest();
    $request->accept = 'iusto';
    $request->range = 'excepturi';
    $request->xEbayCMarketplaceId = 'nisi';
    $request->categoryId = 'recusandae';
    $request->snapshotDate = 'temporibus';

    $requestSecurity = new GetItemSnapshotFeedSecurity();
    $requestSecurity->clientCredentials = 'Bearer YOUR_ACCESS_TOKEN_HERE';

    $response = $sdk->itemSnapshot->getItemSnapshotFeed($request, $requestSecurity);

    if ($response->itemSnapshotResponse !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
