/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import { AxiosResponse } from "axios";
import { Expose, Type } from "class-transformer";

/**
 * Details for API-only IP restrictions.
 */
export class UpdateOrganizationLoginSecurityRequestBodyApiAuthenticationIpRestrictionsForKeys extends SpeakeasyBase {
  /**
   * Boolean indicating whether the organization will restrict API key (not Dashboard GUI) usage to a specific list of IP addresses or CIDR ranges.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enabled" })
  enabled?: boolean;

  /**
   * List of acceptable IP ranges. Entries can be single IP addresses, IP address ranges, and CIDR subnets.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "ranges" })
  ranges?: string[];
}

/**
 * Details for indicating whether organization will restrict access to API (but not Dashboard) to certain IP addresses.
 */
export class UpdateOrganizationLoginSecurityRequestBodyApiAuthentication extends SpeakeasyBase {
  /**
   * Details for API-only IP restrictions.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "ipRestrictionsForKeys" })
  @Type(
    () =>
      UpdateOrganizationLoginSecurityRequestBodyApiAuthenticationIpRestrictionsForKeys
  )
  ipRestrictionsForKeys?: UpdateOrganizationLoginSecurityRequestBodyApiAuthenticationIpRestrictionsForKeys;
}

export class UpdateOrganizationLoginSecurityRequestBody extends SpeakeasyBase {
  /**
   * Number of consecutive failed login attempts after which users' accounts will be locked.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "accountLockoutAttempts" })
  accountLockoutAttempts?: number;

  /**
   * Details for indicating whether organization will restrict access to API (but not Dashboard) to certain IP addresses.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "apiAuthentication" })
  @Type(() => UpdateOrganizationLoginSecurityRequestBodyApiAuthentication)
  apiAuthentication?: UpdateOrganizationLoginSecurityRequestBodyApiAuthentication;

  /**
   * Boolean indicating whether users' Dashboard accounts will be locked out after a specified number of consecutive failed login attempts.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceAccountLockout" })
  enforceAccountLockout?: boolean;

  /**
   * Boolean indicating whether users, when setting a new password, are forced to choose a new password that is different from any past passwords.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceDifferentPasswords" })
  enforceDifferentPasswords?: boolean;

  /**
   * Boolean indicating whether users will be logged out after being idle for the specified number of minutes.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceIdleTimeout" })
  enforceIdleTimeout?: boolean;

  /**
   * Boolean indicating whether organization will restrict access to Dashboard (including the API) from certain IP addresses.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceLoginIpRanges" })
  enforceLoginIpRanges?: boolean;

  /**
   * Boolean indicating whether users are forced to change their password every X number of days.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforcePasswordExpiration" })
  enforcePasswordExpiration?: boolean;

  /**
   * Boolean indicating whether users will be forced to choose strong passwords for their accounts. Strong passwords are at least 8 characters that contain 3 of the following: number, uppercase letter, lowercase letter, and symbol
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceStrongPasswords" })
  enforceStrongPasswords?: boolean;

  /**
   * Boolean indicating whether users in this organization will be required to use an extra verification code when logging in to Dashboard. This code will be sent to their mobile phone via SMS, or can be generated by the authenticator application.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceTwoFactorAuth" })
  enforceTwoFactorAuth?: boolean;

  /**
   * Number of minutes users can remain idle before being logged out of their accounts.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "idleTimeoutMinutes" })
  idleTimeoutMinutes?: number;

  /**
   * List of acceptable IP ranges. Entries can be single IP addresses, IP address ranges, and CIDR subnets.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "loginIpRanges" })
  loginIpRanges?: string[];

  /**
   * Number of recent passwords that new password must be distinct from.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "numDifferentPasswords" })
  numDifferentPasswords?: number;

  /**
   * Number of days after which users will be forced to change their password.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "passwordExpirationDays" })
  passwordExpirationDays?: number;
}

export class UpdateOrganizationLoginSecurityRequest extends SpeakeasyBase {
  @SpeakeasyMetadata({ data: "request, media_type=application/json" })
  requestBody?: UpdateOrganizationLoginSecurityRequestBody;

  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=organizationId",
  })
  organizationId: string;
}

/**
 * Details for API-only IP restrictions.
 */
export class UpdateOrganizationLoginSecurity200ApplicationJSONApiAuthenticationIpRestrictionsForKeys extends SpeakeasyBase {
  /**
   * Boolean indicating whether the organization will restrict API key (not Dashboard GUI) usage to a specific list of IP addresses or CIDR ranges.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enabled" })
  enabled?: boolean;

  /**
   * List of acceptable IP ranges. Entries can be single IP addresses, IP address ranges, and CIDR subnets.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "ranges" })
  ranges?: string[];
}

/**
 * Details for indicating whether organization will restrict access to API (but not Dashboard) to certain IP addresses.
 */
export class UpdateOrganizationLoginSecurity200ApplicationJSONApiAuthentication extends SpeakeasyBase {
  /**
   * Details for API-only IP restrictions.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "ipRestrictionsForKeys" })
  @Type(
    () =>
      UpdateOrganizationLoginSecurity200ApplicationJSONApiAuthenticationIpRestrictionsForKeys
  )
  ipRestrictionsForKeys?: UpdateOrganizationLoginSecurity200ApplicationJSONApiAuthenticationIpRestrictionsForKeys;
}

/**
 * Successful operation
 */
export class UpdateOrganizationLoginSecurity200ApplicationJSON extends SpeakeasyBase {
  /**
   * Number of consecutive failed login attempts after which users' accounts will be locked.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "accountLockoutAttempts" })
  accountLockoutAttempts?: number;

  /**
   * Details for indicating whether organization will restrict access to API (but not Dashboard) to certain IP addresses.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "apiAuthentication" })
  @Type(
    () => UpdateOrganizationLoginSecurity200ApplicationJSONApiAuthentication
  )
  apiAuthentication?: UpdateOrganizationLoginSecurity200ApplicationJSONApiAuthentication;

  /**
   * Boolean indicating whether users' Dashboard accounts will be locked out after a specified number of consecutive failed login attempts.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceAccountLockout" })
  enforceAccountLockout?: boolean;

  /**
   * Boolean indicating whether users, when setting a new password, are forced to choose a new password that is different from any past passwords.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceDifferentPasswords" })
  enforceDifferentPasswords?: boolean;

  /**
   * Boolean indicating whether users will be logged out after being idle for the specified number of minutes.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceIdleTimeout" })
  enforceIdleTimeout?: boolean;

  /**
   * Boolean indicating whether organization will restrict access to Dashboard (including the API) from certain IP addresses.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceLoginIpRanges" })
  enforceLoginIpRanges?: boolean;

  /**
   * Boolean indicating whether users are forced to change their password every X number of days.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforcePasswordExpiration" })
  enforcePasswordExpiration?: boolean;

  /**
   * Boolean indicating whether users will be forced to choose strong passwords for their accounts. Strong passwords are at least 8 characters that contain 3 of the following: number, uppercase letter, lowercase letter, and symbol
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceStrongPasswords" })
  enforceStrongPasswords?: boolean;

  /**
   * Boolean indicating whether users in this organization will be required to use an extra verification code when logging in to Dashboard. This code will be sent to their mobile phone via SMS, or can be generated by the authenticator application.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enforceTwoFactorAuth" })
  enforceTwoFactorAuth?: boolean;

  /**
   * Number of minutes users can remain idle before being logged out of their accounts.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "idleTimeoutMinutes" })
  idleTimeoutMinutes?: number;

  /**
   * List of acceptable IP ranges. Entries can be single IP addresses, IP address ranges, and CIDR subnets.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "loginIpRanges" })
  loginIpRanges?: string[];

  /**
   * Number of recent passwords that new password must be distinct from.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "numDifferentPasswords" })
  numDifferentPasswords?: number;

  /**
   * Number of days after which users will be forced to change their password.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "passwordExpirationDays" })
  passwordExpirationDays?: number;
}

export class UpdateOrganizationLoginSecurityResponse extends SpeakeasyBase {
  @SpeakeasyMetadata()
  contentType: string;

  @SpeakeasyMetadata()
  statusCode: number;

  @SpeakeasyMetadata()
  rawResponse?: AxiosResponse;

  /**
   * Successful operation
   */
  @SpeakeasyMetadata()
  updateOrganizationLoginSecurity200ApplicationJSONObject?: UpdateOrganizationLoginSecurity200ApplicationJSON;
}
