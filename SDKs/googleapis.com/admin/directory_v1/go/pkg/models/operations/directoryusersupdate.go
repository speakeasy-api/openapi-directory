...
type DirectoryUsersUpdateSecurity struct {
	Oauth2  shared.SchemeOauth2  `security:"scheme,type=oauth2"`
	Oauth2c shared.SchemeOauth2c `security:"scheme,type=oauth2"`
}

type DirectoryUsersUpdatePathParams struct {
	// Identifies the user in the API request. The value can be the user's primary email address, alias email address, or unique user ID.
	UserKey string `pathParam:"style=simple,explode=false,name=userKey"`
}

type DirectoryUsersUpdateQueryParams struct {
	// V1 error format.
...
type DirectoryUsersUpdateResponse struct {
	ContentType string
	StatusCode  int
