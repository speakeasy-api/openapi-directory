...
export class Membership {
  ...
  membershipGet(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.MembershipGetResponse =
        new operations.MembershipGetResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            ...
            res.memberDTOS = utils.objectToClass(
              httpRes?.data,
              shared.MemberDTO,
              resFieldDepth
            );
  ...
  membershipPost(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.MembershipPostResponse =
        new operations.MembershipPostResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
