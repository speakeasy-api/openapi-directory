...
export class Organization {
   ...
   */
  fetchOrganization(
    req: operations.FetchOrganizationRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.FetchOrganizationResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.FetchOrganizationRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/organization/{id}", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.FetchOrganizationResponse =
        new operations.FetchOrganizationResponse({
