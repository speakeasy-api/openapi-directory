...
export class Metadata {
  ...
  **/
  deleteArtifactVersionMetaData(
    ...
    const url: string = utils.generateURL(baseURL, "/groups/{groupId}/artifacts/{artifactId}/versions/{version}/meta", req.pathParams);
  ...
  **/
  getArtifactMetaData(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/groups/{groupId}/artifacts/{artifactId}/meta", req.pathParams);
  ...
  **/
  getArtifactVersionMetaDataByContent(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetArtifactVersionMetaDataByContentResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.versionMetaData = httpRes?.data;
