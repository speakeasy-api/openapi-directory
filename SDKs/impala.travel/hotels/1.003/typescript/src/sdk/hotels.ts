...
export class Hotels {
   ...
   */
  listHotels(
    req: operations.ListHotelsRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.ListHotelsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.ListHotelsRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/hotels";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.ListHotelsResponse =
        new operations.ListHotelsResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.listHotels200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.ListHotels200ApplicationJSON
            );
          }
          break;
   ...
   */
  retrieveHotel(
    req: operations.RetrieveHotelRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.RetrieveHotelResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.RetrieveHotelRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/hotels/{hotelId}", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.RetrieveHotelResponse =
        new operations.RetrieveHotelResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.hotelFullDetail = utils.objectToClass(
              httpRes?.data,
              shared.HotelFullDetail
            );
          }
          break;
        case httpRes?.status == 400:
