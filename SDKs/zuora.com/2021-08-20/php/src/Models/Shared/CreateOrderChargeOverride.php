<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * CreateOrderChargeOverride - Charge associated with a rate plan.
 * 
 * 
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class CreateOrderChargeOverride
{
    /**
     * Billing information about the charge.
     * 
     * 
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverrideBilling $billing
     */
	#[\JMS\Serializer\Annotation\SerializedName('billing')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverrideBilling')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?CreateOrderChargeOverrideBilling $billing = null;
    
    /**
     * Charge number of the charge. For example, C-00000307.
     * 
     * 
     * If you do not set this field, Zuora will generate the charge number.
     * 
     * 
     * @var ?string $chargeNumber
     */
	#[\JMS\Serializer\Annotation\SerializedName('chargeNumber')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $chargeNumber = null;
    
    /**
     * Container for custom fields of a Rate Plan Charge object.
     * 
     * 
     * 
     * @var ?array<string, mixed> $customFields
     */
	#[\JMS\Serializer\Annotation\SerializedName('customFields')]
    #[\JMS\Serializer\Annotation\Type('array<string, mixed>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?array $customFields = null;
    
    /**
     * Description of the charge.
     * 
     * 
     * 
     * @var ?string $description
     */
	#[\JMS\Serializer\Annotation\SerializedName('description')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $description = null;
    
    /**
     * Specifies when a charge becomes inactive.
     * 
     * 
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\EndConditions $endDate
     */
	#[\JMS\Serializer\Annotation\SerializedName('endDate')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\EndConditions')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?EndConditions $endDate = null;
    
    /**
     * Pricing information about the charge.
     * 
     * 
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverridePricing $pricing
     */
	#[\JMS\Serializer\Annotation\SerializedName('pricing')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverridePricing')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?CreateOrderChargeOverridePricing $pricing = null;
    
    /**
     * Internal identifier of the product rate plan charge that the charge is based on.
     * 
     * 
     * 
     * @var string $productRatePlanChargeId
     */
	#[\JMS\Serializer\Annotation\SerializedName('productRatePlanChargeId')]
    #[\JMS\Serializer\Annotation\Type('string')]
    public string $productRatePlanChargeId;
    
    /**
     * Revenue Recognition Code
     * 
     * 
     * 
     * @var ?string $revRecCode
     */
	#[\JMS\Serializer\Annotation\SerializedName('revRecCode')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $revRecCode = null;
    
    /**
     * Specifies the revenue recognition trigger condition.
     * 
     * 
     *   * `Contract Effective Date` 
     *   * `Service Activation Date`
     *   * `Customer Acceptance Date`
     * 
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverrideRevRecTriggerConditionEnum $revRecTriggerCondition
     */
	#[\JMS\Serializer\Annotation\SerializedName('revRecTriggerCondition')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverrideRevRecTriggerConditionEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?CreateOrderChargeOverrideRevRecTriggerConditionEnum $revRecTriggerCondition = null;
    
    /**
     * Specifies the revenue recognition rule.
     * 
     * 
     *   * `Recognize upon invoicing` 
     *   * `Recognize daily over time`
     * 
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverrideRevenueRecognitionRuleNameEnum $revenueRecognitionRuleName
     */
	#[\JMS\Serializer\Annotation\SerializedName('revenueRecognitionRuleName')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Shared\CreateOrderChargeOverrideRevenueRecognitionRuleNameEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?CreateOrderChargeOverrideRevenueRecognitionRuleNameEnum $revenueRecognitionRuleName = null;
    
    /**
     * Specifies when a charge becomes active.
     * 
     * 
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\CreateOrderTriggerParams $startDate
     */
	#[\JMS\Serializer\Annotation\SerializedName('startDate')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\CreateOrderTriggerParams')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?CreateOrderTriggerParams $startDate = null;
    
    /**
     * Unique identifier for the charge. This identifier enables you to refer to the charge before the charge has an internal identifier in Zuora.
     * 
     * 
     * For instance, suppose that you want to use a single order to add a product to a subscription and later update the same product. When you add the product, you can set a unique identifier for the charge. Then when you update the product, you can use the same unique identifier to specify which charge to modify.
     * 
     * 
     * @var ?string $uniqueToken
     */
	#[\JMS\Serializer\Annotation\SerializedName('uniqueToken')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $uniqueToken = null;
    
	public function __construct()
	{
		$this->billing = null;
		$this->chargeNumber = null;
		$this->customFields = null;
		$this->description = null;
		$this->endDate = null;
		$this->pricing = null;
		$this->productRatePlanChargeId = "";
		$this->revRecCode = null;
		$this->revRecTriggerCondition = null;
		$this->revenueRecognitionRuleName = null;
		$this->startDate = null;
		$this->uniqueToken = null;
	}
}
