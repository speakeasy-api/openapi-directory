...
export class Callback {
  ...
  integrationsCallbackGenerateToken(
    ...
    const url: string =
      baseURL.replace(/\/$/, "") + "/v1alpha/callback:generateToken";
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.IntegrationsCallbackGenerateTokenSecurity(
        security
      );
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.IntegrationsCallbackGenerateTokenResponse =
        new operations.IntegrationsCallbackGenerateTokenResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.googleCloudIntegrationsV1alphaGenerateTokenResponse =
              utils.objectToClass(
                httpRes?.data,
                shared.GoogleCloudIntegrationsV1alphaGenerateTokenResponse
              );
          }
          break;
      }
