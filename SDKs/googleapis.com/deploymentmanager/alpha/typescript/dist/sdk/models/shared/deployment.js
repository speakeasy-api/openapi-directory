"use strict";
/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Deployment = void 0;
var utils_1 = require("../../../internal/utils");
var credential_1 = require("./credential");
var deploymentlabelentry_1 = require("./deploymentlabelentry");
var deploymentoutputentry_1 = require("./deploymentoutputentry");
var deploymentupdate_1 = require("./deploymentupdate");
var operation_1 = require("./operation");
var targetconfiguration_1 = require("./targetconfiguration");
var class_transformer_1 = require("class-transformer");
/**
 * Successful response
 */
var Deployment = /** @class */ (function (_super) {
    __extends(Deployment, _super);
    function Deployment() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "credential" }),
        (0, class_transformer_1.Type)(function () { return credential_1.Credential; }),
        __metadata("design:type", credential_1.Credential)
    ], Deployment.prototype, "credential", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "description" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "description", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "fingerprint" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "fingerprint", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "id" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "id", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "insertTime" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "insertTime", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)({ elemType: deploymentlabelentry_1.DeploymentLabelEntry }),
        (0, class_transformer_1.Expose)({ name: "labels" }),
        (0, class_transformer_1.Type)(function () { return deploymentlabelentry_1.DeploymentLabelEntry; }),
        __metadata("design:type", Array)
    ], Deployment.prototype, "labels", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "manifest" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "manifest", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "name" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "name", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "operation" }),
        (0, class_transformer_1.Type)(function () { return operation_1.Operation; }),
        __metadata("design:type", operation_1.Operation)
    ], Deployment.prototype, "operation", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)({ elemType: deploymentoutputentry_1.DeploymentOutputEntry }),
        (0, class_transformer_1.Expose)({ name: "outputs" }),
        (0, class_transformer_1.Type)(function () { return deploymentoutputentry_1.DeploymentOutputEntry; }),
        __metadata("design:type", Array)
    ], Deployment.prototype, "outputs", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "selfLink" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "selfLink", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "target" }),
        (0, class_transformer_1.Type)(function () { return targetconfiguration_1.TargetConfiguration; }),
        __metadata("design:type", targetconfiguration_1.TargetConfiguration)
    ], Deployment.prototype, "target", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "update" }),
        (0, class_transformer_1.Type)(function () { return deploymentupdate_1.DeploymentUpdate; }),
        __metadata("design:type", deploymentupdate_1.DeploymentUpdate)
    ], Deployment.prototype, "update", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "updateTime" }),
        __metadata("design:type", String)
    ], Deployment.prototype, "updateTime", void 0);
    return Deployment;
}(utils_1.SpeakeasyBase));
exports.Deployment = Deployment;
