...
export class Item {
  ...
  getItemFeed(
  ...
  ): Promise<operations.GetItemFeedResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetItemFeedRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/item";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetItemFeedResponse =
        new operations.GetItemFeedResponse({
      ...
      switch (true) {
        case [200, 206].includes(httpRes?.status):
          if (utils.matchContentType(contentType, `application/json`)) {
            res.itemResponse = utils.objectToClass(
              httpRes?.data,
              shared.ItemResponse
            );
          }
