...
class Activity:
    ...
    def get_api_activity(self, request: operations.GetAPIActivityRequest) -> operations.GetAPIActivityResponse:
        ...
        res = operations.GetAPIActivityResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            res.headers = r.headers
            
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[list[shared.APIRequest]])
                res.api_requests = out
