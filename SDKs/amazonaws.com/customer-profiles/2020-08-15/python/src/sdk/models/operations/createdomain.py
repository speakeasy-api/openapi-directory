...
class CreateDomainRequestBody:
    ...
    default_expiration_days: int = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('DefaultExpirationDays') }})
    ...
    r"""The URL of the SQS dead letter queue, which is used for reporting errors associated with ingesting data from third party applications. You must set up a policy on the DeadLetterQueue for the SendMessage operation to enable Amazon Connect Customer Profiles to send messages to the DeadLetterQueue."""  
    ...
    r"""The default encryption key, which is an AWS managed key, is used when no specific type of encryption key is specified. It is used to encrypt all data before it is placed in permanent or semi-permanent storage."""  
    matching: Optional[CreateDomainRequestBodyMatching] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('Matching'), 'exclude': lambda f: f is None }})
    r"""The flag that enables the matching process of duplicate profiles."""  
