...
class Vaults:
    ...
    def get_vault_by_id(self, request: operations.GetVaultByIDRequest) -> operations.GetVaultByIDResponse:
        ...
        res = operations.GetVaultByIDResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[shared.Vault])
                res.vault = out
    ...
    def get_vaults(self, request: operations.GetVaultsRequest) -> operations.GetVaultsResponse:
        ...
        res = operations.GetVaultsResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[list[shared.Vault]])
                res.vaults = out
        elif r.status_code == 401:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[shared.ErrorResponse])
