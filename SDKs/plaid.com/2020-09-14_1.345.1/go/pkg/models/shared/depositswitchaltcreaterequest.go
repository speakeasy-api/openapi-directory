// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
)

// DepositSwitchAltCreateRequestCountryCodeEnum - ISO-3166-1 alpha-2 country code standard.
type DepositSwitchAltCreateRequestCountryCodeEnum string

const (
	DepositSwitchAltCreateRequestCountryCodeEnumUs DepositSwitchAltCreateRequestCountryCodeEnum = "US"
	DepositSwitchAltCreateRequestCountryCodeEnumCa DepositSwitchAltCreateRequestCountryCodeEnum = "CA"
)

func (e DepositSwitchAltCreateRequestCountryCodeEnum) ToPointer() *DepositSwitchAltCreateRequestCountryCodeEnum {
	return &e
}

func (e *DepositSwitchAltCreateRequestCountryCodeEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "US":
		fallthrough
	case "CA":
		*e = DepositSwitchAltCreateRequestCountryCodeEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for DepositSwitchAltCreateRequestCountryCodeEnum: %v", v)
	}
}

// DepositSwitchAltCreateRequest - DepositSwitchAltCreateRequest defines the request schema for `/deposit_switch/alt/create`
type DepositSwitchAltCreateRequest struct {
	// Your Plaid API `client_id`. The `client_id` is required and may be provided either in the `PLAID-CLIENT-ID` header or as part of a request body.
	ClientID *string `json:"client_id,omitempty"`
	// ISO-3166-1 alpha-2 country code standard.
	CountryCode *DepositSwitchAltCreateRequestCountryCodeEnum `json:"country_code,omitempty"`
	// Options to configure the `/deposit_switch/create` request. If provided, cannot be `null`.
	Options *DepositSwitchCreateRequestOptions `json:"options,omitempty"`
	// Your Plaid API `secret`. The `secret` is required and may be provided either in the `PLAID-SECRET` header or as part of a request body.
	Secret *string `json:"secret,omitempty"`
	// The deposit switch destination account
	TargetAccount map[string]interface{} `json:"target_account"`
	// The deposit switch target user
	TargetUser map[string]interface{} `json:"target_user"`
}
