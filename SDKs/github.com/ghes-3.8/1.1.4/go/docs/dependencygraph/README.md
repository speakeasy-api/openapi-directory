# DependencyGraph

## Overview

Endpoints to access Dependency Graph features.

### Available Operations

* [DependencyGraphCreateRepositorySnapshot](#dependencygraphcreaterepositorysnapshot) - Create a snapshot of dependencies for a repository
* [DependencyGraphDiffRange](#dependencygraphdiffrange) - Get a diff of the dependencies between commits

## DependencyGraphCreateRepositorySnapshot

Create a new snapshot of a repository's dependencies. You must authenticate using an access token with the `repo` scope to use this endpoint for a repository that the requesting user has access to.

API method documentation
<https://docs.github.com/enterprise-server@3.8/rest/reference/dependency-graph#create-a-snapshot-of-dependencies-for-a-repository>

### Example Usage

```go
package main

import(
	"context"
	"log"
	"openapi"
	"openapi/pkg/models/operations"
	"openapi/pkg/models/shared"
	"openapi/pkg/types"
)

func main() {
    s := sdk.New()

    ctx := context.Background()
    res, err := s.DependencyGraph.DependencyGraphCreateRepositorySnapshot(ctx, operations.DependencyGraphCreateRepositorySnapshotRequest{
        Owner: "assumenda",
        Repo: "ea",
        Snapshot: shared.Snapshot{
            Detector: shared.SnapshotDetector{
                Name: "docker buildtime detector",
                URL: "http://example.com/docker-buildtimer-detector",
                Version: "1.0.0",
            },
            Job: shared.SnapshotJob{
                Correlator: "yourworkflowname_yourjobname",
                HTMLURL: sdk.String("http://example.com/build"),
                ID: "5622a2b0-63f6-4732-8c34-a1ab27e102a11",
            },
            Manifests: map[string]shared.Manifest{
                "error": shared.Manifest{
                    File: &shared.ManifestFile{
                        SourceLocation: sdk.String("/src/build/package-lock.json"),
                    },
                    Metadata: map[string]interface{}{
                        "officiis": "accusamus",
                        "natus": "minima",
                        "aspernatur": "ex",
                        "maiores": "corrupti",
                    },
                    Name: "package-lock.json",
                    Resolved: map[string]shared.Dependency{
                        "error": shared.Dependency{
                            Dependencies: []string{
                                "suscipit",
                                "repudiandae",
                                "atque",
                            },
                            Metadata: map[string]interface{}{
                                "sunt": "recusandae",
                                "dolorum": "repellendus",
                                "labore": "reiciendis",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                        "doloremque": shared.Dependency{
                            Dependencies: []string{
                                "dicta",
                                "accusantium",
                                "beatae",
                                "dolores",
                            },
                            Metadata: map[string]interface{}{
                                "laboriosam": "velit",
                                "a": "molestias",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                        "magnam": shared.Dependency{
                            Dependencies: []string{
                                "consequuntur",
                                "occaecati",
                                "officiis",
                                "perspiciatis",
                            },
                            Metadata: map[string]interface{}{
                                "adipisci": "eveniet",
                                "occaecati": "consequuntur",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                        "fugit": shared.Dependency{
                            Dependencies: []string{
                                "quis",
                                "reprehenderit",
                                "error",
                            },
                            Metadata: map[string]interface{}{
                                "corporis": "quidem",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                    },
                },
                "eveniet": shared.Manifest{
                    File: &shared.ManifestFile{
                        SourceLocation: sdk.String("/src/build/package-lock.json"),
                    },
                    Metadata: map[string]interface{}{
                        "vero": "doloremque",
                    },
                    Name: "package-lock.json",
                    Resolved: map[string]shared.Dependency{
                        "ipsa": shared.Dependency{
                            Dependencies: []string{
                                "quae",
                                "molestiae",
                                "eveniet",
                            },
                            Metadata: map[string]interface{}{
                                "cum": "iure",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                        "necessitatibus": shared.Dependency{
                            Dependencies: []string{
                                "laborum",
                            },
                            Metadata: map[string]interface{}{
                                "voluptatum": "rem",
                                "aliquam": "ad",
                                "repellat": "alias",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                    },
                },
                "corporis": shared.Manifest{
                    File: &shared.ManifestFile{
                        SourceLocation: sdk.String("/src/build/package-lock.json"),
                    },
                    Metadata: map[string]interface{}{
                        "nihil": "mollitia",
                        "voluptas": "alias",
                        "maiores": "reiciendis",
                    },
                    Name: "package-lock.json",
                    Resolved: map[string]shared.Dependency{
                        "id": shared.Dependency{
                            Dependencies: []string{
                                "dolore",
                                "dolorum",
                            },
                            Metadata: map[string]interface{}{
                                "quae": "recusandae",
                            },
                            PackageURL: sdk.String("pkg:/npm/%40actions/http-client@1.0.11"),
                            Relationship: shared.DependencyRelationshipEnumDirect.ToPointer(),
                            Scope: shared.DependencyScopeEnumRuntime.ToPointer(),
                        },
                    },
                },
            },
            Metadata: map[string]interface{}{
                "quaerat": "molestiae",
                "ex": "ut",
                "culpa": "adipisci",
            },
            Ref: "refs/heads/main",
            Scanned: types.MustTimeFromString("2020-06-13T14:52:50-05:00"),
            Sha: "ddc951f4b1293222421f2c8df679786153acf689",
            Version: 890653,
        },
    })
    if err != nil {
        log.Fatal(err)
    }

    if res.DependencyGraphCreateRepositorySnapshot201ApplicationJSONObject != nil {
        // handle response
    }
}
```

## DependencyGraphDiffRange

Gets the diff of the dependency changes between two commits of a repository, based on the changes to the dependency manifests made in those commits.

API method documentation
<https://docs.github.com/enterprise-server@3.8/rest/reference/dependency-graph#get-a-diff-of-the-dependencies-between-commits>

### Example Usage

```go
package main

import(
	"context"
	"log"
	"openapi"
	"openapi/pkg/models/operations"
)

func main() {
    s := sdk.New()

    ctx := context.Background()
    res, err := s.DependencyGraph.DependencyGraphDiffRange(ctx, operations.DependencyGraphDiffRangeRequest{
        Basehead: "laudantium",
        Name: sdk.String("Vivian Ward"),
        Owner: "quis",
        Repo: "eum",
    })
    if err != nil {
        log.Fatal(err)
    }

    if res.DependencyGraphDiff != nil {
        // handle response
    }
}
```
