<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class DisruptionsGetDisruptionsByStopRequest
{
    /**
     * Your developer id
     * 
     * @var ?string $devid
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=devid')]
    public ?string $devid = null;
    
    /**
     * Filter by status of disruption
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\DisruptionsGetDisruptionsByStopDisruptionStatusEnum $disruptionStatus
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=disruption_status')]
    public ?DisruptionsGetDisruptionsByStopDisruptionStatusEnum $disruptionStatus = null;
    
    /**
     * Authentication signature for request
     * 
     * @var ?string $signature
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=signature')]
    public ?string $signature = null;
    
    /**
     * Identifier of stop; values returned by Stops API - v3/stops
     * 
     * @var int $stopId
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=stop_id')]
    public int $stopId;
    
    /**
     * Please ignore
     * 
     * @var ?string $token
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=token')]
    public ?string $token = null;
    
	public function __construct()
	{
		$this->devid = null;
		$this->disruptionStatus = null;
		$this->signature = null;
		$this->stopId = 0;
		$this->token = null;
	}
}
