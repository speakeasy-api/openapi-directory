...
export class ProductFeatures {
  ...
  objectDELETEProductFeature(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.ObjectDELETEProductFeatureResponse =
        new operations.ObjectDELETEProductFeatureResponse({
  ...
  objectGETProductFeature(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.ObjectGETProductFeatureResponse =
        new operations.ObjectGETProductFeatureResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
          headers: utils.getHeadersFromResponse(httpRes.headers),
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (
            ...
            res.proxyGetProductFeature = utils.objectToClass(
              httpRes?.data,
              shared.ProxyGetProductFeature
            );
          }
          break;
        case httpRes?.status == 401:
