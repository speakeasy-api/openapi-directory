...
func NewListingRecommendation(defaultClient, securityClient HTTPClient, serverURL, language, sdkVersion, genVersion string) *ListingRecommendation {
...
func (s *ListingRecommendation) FindListingRecommendations(ctx context.Context, request operations.FindListingRecommendationsRequest) (*operations.FindListingRecommendationsResponse, error) {
	baseURL := s._serverURL
	url := strings.TrimSuffix(baseURL, "/") + "/find"
	...
	res := &operations.FindListingRecommendationsResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json;charset=UTF-8`):
			var out *shared.PagedListingRecommendationCollection
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}

			res.PagedListingRecommendationCollection = out
		}
