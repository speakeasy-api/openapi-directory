...
class GETAllCustomObjectDefinitionsInNamespaceSelectEnum(str, Enum):
    r"""If you set `select` to `type`, the response will only contain the `type` of each custom object."""
    TYPE = 'type'
...
class GETAllCustomObjectDefinitionsInNamespaceRequest:
    ...
    select: Optional[GETAllCustomObjectDefinitionsInNamespaceSelectEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'select', 'style': 'form', 'explode': True }})
    r"""If you set `select` to `type`, the response will only contain the `type` of each custom object."""  
