"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import grapherror as shared_grapherror
from typing import Any, Optional


@dataclasses.dataclass
class GroupsAddMemberRequest:
    
    api_version: str = dataclasses.field(metadata={'query_param': { 'field_name': 'api-version', 'style': 'form', 'explode': True }})
    r"""Client API version."""  
    group_object_id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'groupObjectId', 'style': 'simple', 'explode': False }})
    r"""The object ID of the group to which to add the member."""  
    request_body: dict[str, dict[str, Any]] = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})
    r"""The URL of the member object, such as https://graph.windows.net/0b1f9851-1bf0-433f-aec3-cb9272f093dc/directoryObjects/f260bbc4-c254-447b-94cf-293b5ec434dd."""  
    tenant_id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'tenantID', 'style': 'simple', 'explode': False }})
    r"""The tenant ID."""  
    

@dataclasses.dataclass
class GroupsAddMemberResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    graph_error: Optional[shared_grapherror.GraphError] = dataclasses.field(default=None)
    r"""Error response describing why the operation failed."""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    