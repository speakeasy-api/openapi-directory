...
class RegisterDomainRequestInput:
    ...
    contact_notices: Optional[list[RegisterDomainRequestContactNoticesEnum]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('contactNotices'), 'exclude': lambda f: f is None }})
    r"""The list of contact notices that the caller acknowledges. The notices needed here depend on the values specified in `registration.contact_settings`."""  
    domain_notices: Optional[list[RegisterDomainRequestDomainNoticesEnum]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('domainNotices'), 'exclude': lambda f: f is None }})
    r"""The list of domain notices that you acknowledge. Call `RetrieveRegisterParameters` to see the notices that need acknowledgement."""  
    ...
    r"""When true, only validation is performed, without actually registering the domain. Follows: https://cloud.google.com/apis/design/design_patterns#request_validation"""  
