...
export class Users {
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.accessTokenDetails = utils.objectToClass(
              httpRes?.data,
              shared.AccessTokenDetails
            );
          }
          break;
  ...
  getUserSubscriptionList(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GetUserSubscriptionListResponse =
        new operations.GetUserSubscriptionListResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.subscriptionDataList = utils.objectToClass(
              httpRes?.data,
              shared.SubscriptionDataList
            );
          }
          break;
        case [400, 401, 403].includes(httpRes?.status):
