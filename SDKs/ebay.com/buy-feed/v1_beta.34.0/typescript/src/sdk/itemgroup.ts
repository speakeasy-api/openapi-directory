...
export class ItemGroup {
   ...
   */
  getItemGroupFeed(
    req: operations.GetItemGroupFeedRequest,
    security: operations.GetItemGroupFeedSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.GetItemGroupFeedResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetItemGroupFeedRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/item_group";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetItemGroupFeedResponse =
        new operations.GetItemGroupFeedResponse({
      ...
      switch (true) {
        case [200, 206].includes(httpRes?.status):
          if (utils.matchContentType(contentType, `application/json`)) {
            res.itemGroupResponse = utils.objectToClass(
              httpRes?.data,
              shared.ItemGroupResponse
            );
          }
          if (
