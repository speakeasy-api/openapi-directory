...
class POSTCreateOrderAsynchronouslyRequest:
    ...
    post_order_request_type: shared_postorderrequesttype.POSTOrderRequestType = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})  
    ...
    r"""Specify whether to return IDs for the [Get job status and response](https://www.zuora.com/developer/api-reference/#operation/GET_JobStatusAndResponse) operation. If you set this query parameter to `true`, the corresponding IDs, which are associated with the numbers returned in this operation, can be returned in the \\"Get job status and response\\" response body."""  
