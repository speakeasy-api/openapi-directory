/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import org.openapis.openapi.utils.SpeakeasyMetadata;

public class GroupV2AbdicateFoundershipPathParams {
    /**
     * The new founder for this group. Must already be a group admin.
     */
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=founderIdNew")
    public Long founderIdNew;
    public GroupV2AbdicateFoundershipPathParams withFounderIdNew(Long founderIdNew) {
        this.founderIdNew = founderIdNew;
        return this;
    }
    
    /**
     * The target group id.
     */
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=groupId")
    public Long groupId;
    public GroupV2AbdicateFoundershipPathParams withGroupId(Long groupId) {
        this.groupId = groupId;
        return this;
    }
    
    /**
     * Membership type of the provided founderIdNew.
     */
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=membershipType")
    public Integer membershipType;
    public GroupV2AbdicateFoundershipPathParams withMembershipType(Integer membershipType) {
        this.membershipType = membershipType;
        return this;
    }
    
}
