...
// FantasyDefenseGameStatsByTeamFormatEnum - Desired response format. Valid entries are <code>XML</code> or <code>JSON</code>.
...
func (e FantasyDefenseGameStatsByTeamFormatEnum) ToPointer() *FantasyDefenseGameStatsByTeamFormatEnum {
	return &e
}

func (e *FantasyDefenseGameStatsByTeamFormatEnum) UnmarshalJSON(data []byte) error {
	var v string
	...
	case "XML":
		fallthrough
	case "JSON":
		*e = FantasyDefenseGameStatsByTeamFormatEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for FantasyDefenseGameStatsByTeamFormatEnum: %v", v)
	}
}
...
type FantasyDefenseGameStatsByTeamResponse struct {
	ContentType        string
	FantasyDefenseGame *shared.FantasyDefenseGame
	StatusCode         int
