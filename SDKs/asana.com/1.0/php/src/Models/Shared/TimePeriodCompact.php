<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * TimePeriodCompact - A generic Asana Resource, containing a globally unique identifier.
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class TimePeriodCompact
{
    /**
     * A string representing the cadence code and the fiscal year.
     * 
     * @var ?string $displayName
     */
	#[\JMS\Serializer\Annotation\SerializedName('display_name')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $displayName = null;
    
    /**
     * The localized end date of the time period in `YYYY-MM-DD` format.
     * 
     * @var ?string $endOn
     */
	#[\JMS\Serializer\Annotation\SerializedName('end_on')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $endOn = null;
    
    /**
     * Globally unique identifier of the resource, as a string.
     * 
     * @var ?string $gid
     */
	#[\JMS\Serializer\Annotation\SerializedName('gid')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $gid = null;
    
    /**
     * The cadence and index of the time period. The value is one of: `FY`, `H1`, `H2`, `Q1`, `Q2`, `Q3`, or `Q4`.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\TimePeriodCompactPeriodEnum $period
     */
	#[\JMS\Serializer\Annotation\SerializedName('period')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Shared\TimePeriodCompactPeriodEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?TimePeriodCompactPeriodEnum $period = null;
    
    /**
     * The base type of this resource.
     * 
     * @var ?string $resourceType
     */
	#[\JMS\Serializer\Annotation\SerializedName('resource_type')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $resourceType = null;
    
    /**
     * The localized start date of the time period in `YYYY-MM-DD` format.
     * 
     * @var ?string $startOn
     */
	#[\JMS\Serializer\Annotation\SerializedName('start_on')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $startOn = null;
    
	public function __construct()
	{
		$this->displayName = null;
		$this->endOn = null;
		$this->gid = null;
		$this->period = null;
		$this->resourceType = null;
		$this->startOn = null;
	}
}
