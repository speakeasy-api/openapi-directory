...
export class SslPolicies {
  ...
  computeSslPoliciesTestIamPermissions(
    req: operations.ComputeSslPoliciesTestIamPermissionsRequest,
    security: operations.ComputeSslPoliciesTestIamPermissionsSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.ComputeSslPoliciesTestIamPermissionsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.ComputeSslPoliciesTestIamPermissionsRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(
      baseURL,
      "/projects/{project}/global/sslPolicies/{resource}/testIamPermissions",
      req
    );
    ...
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.ComputeSslPoliciesTestIamPermissionsSecurity(
        security
      );
    }
    const client: AxiosInstance = utils.createSecurityClient(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.ComputeSslPoliciesTestIamPermissionsResponse =
        new operations.ComputeSslPoliciesTestIamPermissionsResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
