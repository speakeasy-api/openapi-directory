...
export class Similar {
  ...
  getSimilar(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetSimilarResponse =
        new operations.GetSimilarResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getSimilar200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.GetSimilar200ApplicationJSON
            );
          }
          break;
        case [400, 403, 501].includes(httpRes?.status):
          break;
      }
