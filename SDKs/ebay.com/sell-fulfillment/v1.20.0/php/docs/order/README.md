...
try {
    ...
    $request->fieldGroups = 'distinctio';
    $request->orderId = 'quibusdam';
...
try {
    ...
    $request->fieldGroups = 'unde';
    $request->filter = 'nulla';
    $request->limit = 'corrupti';
    $request->offset = 'illum';
    $request->orderIds = 'vel';

    $requestSecurity = new GetOrdersSecurity();
    $requestSecurity->apiAuth = 'Bearer YOUR_ACCESS_TOKEN_HERE';

    $response = $sdk->order->getOrders($request, $requestSecurity);

    if ($response->orderSearchPagedCollection !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
...
use \OpenAPI\OpenAPI\SDK;
use \OpenAPI\OpenAPI\Models\Operations\IssueRefundRequest;
use \OpenAPI\OpenAPI\Models\Shared\IssueRefundRequest;
use \OpenAPI\OpenAPI\Models\Shared\SimpleAmount;
use \OpenAPI\OpenAPI\Models\Shared\RefundItem;
use \OpenAPI\OpenAPI\Models\Shared\LegacyReference;
use \OpenAPI\OpenAPI\Models\Operations\IssueRefundSecurity;

$sdk = SDK::builder()
    ->build();

try {
    $request = new IssueRefundRequest();
    $request->issueRefundRequest = new IssueRefundRequest();
    $request->issueRefundRequest->comment = 'error';
    $request->issueRefundRequest->orderLevelRefundAmount = new SimpleAmount();
    $request->issueRefundRequest->orderLevelRefundAmount->currency = 'deserunt';
    $request->issueRefundRequest->orderLevelRefundAmount->value = 'suscipit';
    $request->issueRefundRequest->reasonForRefund = 'iure';
    $request->issueRefundRequest->refundItems = [
        new RefundItem(),
        new RefundItem(),
    ];
    $request->orderId = 'debitis';

    $requestSecurity = new IssueRefundSecurity();
    $requestSecurity->apiAuth = 'Bearer YOUR_ACCESS_TOKEN_HERE';

    $response = $sdk->order->issueRefund($request, $requestSecurity);
