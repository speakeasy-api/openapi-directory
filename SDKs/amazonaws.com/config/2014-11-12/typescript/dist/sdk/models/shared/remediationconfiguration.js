"use strict";
/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RemediationConfiguration = void 0;
var utils_1 = require("../../../internal/utils");
var executioncontrols_1 = require("./executioncontrols");
var remediationparametervalue_1 = require("./remediationparametervalue");
var remediationtargettypeenum_1 = require("./remediationtargettypeenum");
var class_transformer_1 = require("class-transformer");
/**
 * An object that represents the details about the remediation configuration that includes the remediation action, parameters, and data to execute the action.
 */
var RemediationConfiguration = /** @class */ (function (_super) {
    __extends(RemediationConfiguration, _super);
    function RemediationConfiguration() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "Arn" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "arn", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "Automatic" }),
        __metadata("design:type", Boolean)
    ], RemediationConfiguration.prototype, "automatic", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "ConfigRuleName" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "configRuleName", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "CreatedByService" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "createdByService", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "ExecutionControls" }),
        (0, class_transformer_1.Type)(function () { return executioncontrols_1.ExecutionControls; }),
        __metadata("design:type", executioncontrols_1.ExecutionControls)
    ], RemediationConfiguration.prototype, "executionControls", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "MaximumAutomaticAttempts" }),
        __metadata("design:type", Number)
    ], RemediationConfiguration.prototype, "maximumAutomaticAttempts", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)({ elemType: remediationparametervalue_1.RemediationParameterValue }),
        (0, class_transformer_1.Expose)({ name: "Parameters" }),
        (0, class_transformer_1.Transform)(function (_a) {
            var value = _a.value;
            var obj = {};
            for (var key in value) {
                obj[key] = (0, utils_1.objectToClass)(value[key], remediationparametervalue_1.RemediationParameterValue);
            }
            return obj;
        }, { toClassOnly: true }),
        __metadata("design:type", Object)
    ], RemediationConfiguration.prototype, "parameters", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "ResourceType" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "resourceType", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "RetryAttemptSeconds" }),
        __metadata("design:type", Number)
    ], RemediationConfiguration.prototype, "retryAttemptSeconds", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "TargetId" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "targetId", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "TargetType" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "targetType", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "TargetVersion" }),
        __metadata("design:type", String)
    ], RemediationConfiguration.prototype, "targetVersion", void 0);
    return RemediationConfiguration;
}(utils_1.SpeakeasyBase));
exports.RemediationConfiguration = RemediationConfiguration;
