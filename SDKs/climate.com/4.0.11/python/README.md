...
```bash
pip install git+https://github.com/speakeasy-api/openapi-directory.git#subdirectory=SDKs/climate.com/4.0.11/python
```
...
* `fetch_export_contents_by_id` - Retrieve the binary contents of a processed export request.
* `fetch_export_status_by_id` - Retrieve the status of an Export.
* `post_export` - Initiate a new export request.
...
* `fetch_all_fields` - Retrieve list of all Fields the user has access to.
...
* `fetch_operations` - Retrieve the operations accessible to a a given user.
