...
class Tags:
    ...
    def collection_tags_all(self, request: operations.CollectionTagsAllRequest, security: operations.CollectionTagsAllSecurity) -> operations.CollectionTagsAllResponse:
        r"""List Tags
        List Tags
        """
        base_url = self._server_url
        
        url = utils.generate_url(operations.CollectionTagsAllRequest, base_url, '/issue-tracking/collections/{collection_id}/tags', request)
        
        headers = utils.get_headers(request)
        query_params = utils.get_query_params(operations.CollectionTagsAllRequest, request)
        ...
        res = operations.CollectionTagsAllResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.GetCollectionTagsResponse])
                res.get_collection_tags_response = out
