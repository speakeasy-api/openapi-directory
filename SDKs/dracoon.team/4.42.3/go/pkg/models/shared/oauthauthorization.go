// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
	"time"
)

// OAuthAuthorizationUserAgentCategoryEnum - &#128640; Since v4.12.0
//
// User agent category.
type OAuthAuthorizationUserAgentCategoryEnum string

const (
	OAuthAuthorizationUserAgentCategoryEnumBrowser OAuthAuthorizationUserAgentCategoryEnum = "browser"
	OAuthAuthorizationUserAgentCategoryEnumNative  OAuthAuthorizationUserAgentCategoryEnum = "native"
	OAuthAuthorizationUserAgentCategoryEnumUnknown OAuthAuthorizationUserAgentCategoryEnum = "unknown"
)

func (e OAuthAuthorizationUserAgentCategoryEnum) ToPointer() *OAuthAuthorizationUserAgentCategoryEnum {
	return &e
}

func (e *OAuthAuthorizationUserAgentCategoryEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "browser":
		fallthrough
	case "native":
		fallthrough
	case "unknown":
		*e = OAuthAuthorizationUserAgentCategoryEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for OAuthAuthorizationUserAgentCategoryEnum: %v", v)
	}
}

// OAuthAuthorization - OAuth authorization
type OAuthAuthorization struct {
	// ID of the OAuth client
	ClientID string `json:"clientId"`
	// Name, which is shown at the client configuration and authorization.
	ClientName string `json:"clientName"`
	// &#128640; Since v4.13.0
	//
	// Creation date of the authorization
	CreatedAt *time.Time `json:"createdAt,omitempty"`
	// Expiration date of the authorization
	ExpiresAt *time.Time `json:"expiresAt,omitempty"`
	// &#128640; Since v4.12.0
	//
	// ID of the OAuth authorization
	ID *int64 `json:"id,omitempty"`
	// &#128640; Since v4.25.0
	//
	// Determines whether authorization matches the one from Authorization Header
	IsCurrentAuthorization *bool `json:"isCurrentAuthorization,omitempty"`
	// &#128640; Since v4.12.0
	//
	// Determines whether client is a standard client.
	IsStandard *bool `json:"isStandard,omitempty"`
	// &#128640; Since v4.13.0
	//
	// Usage date of the authorization
	//
	// (Time of last usage.)
	UsedAt *time.Time `json:"usedAt,omitempty"`
	// &#128640; Since v4.12.0
	//
	// User agent category.
	UserAgentCategory OAuthAuthorizationUserAgentCategoryEnum `json:"userAgentCategory"`
	// &#128640; Since v4.12.0
	//
	// User agent info.
	UserAgentInfo *string `json:"userAgentInfo,omitempty"`
	// &#128640; Since v4.12.0
	//
	// User agent OS.
	UserAgentOs *string `json:"userAgentOs,omitempty"`
	// &#128640; Since v4.12.0
	//
	// User agent type.
	UserAgentType *string `json:"userAgentType,omitempty"`
}
