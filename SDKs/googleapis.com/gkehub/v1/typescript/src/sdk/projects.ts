...
export class Projects {
  ...
  gkehubProjectsLocationsMembershipsBindingsPatch(
    req: operations.GkehubProjectsLocationsMembershipsBindingsPatchRequest,
    security: operations.GkehubProjectsLocationsMembershipsBindingsPatchSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.GkehubProjectsLocationsMembershipsBindingsPatchResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req =
        new operations.GkehubProjectsLocationsMembershipsBindingsPatchRequest(
          req
        );
    }
    ...
    if (!(security instanceof utils.SpeakeasyBase)) {
      security =
        new operations.GkehubProjectsLocationsMembershipsBindingsPatchSecurity(
          security
        );
    }
    const client: AxiosInstance = utils.createSecurityClient(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GkehubProjectsLocationsMembershipsBindingsPatchResponse =
        new operations.GkehubProjectsLocationsMembershipsBindingsPatchResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
  ...
  gkehubProjectsLocationsScopesCreate(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/v1/{parent}/scopes", req);
