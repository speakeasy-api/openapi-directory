...
class ListingRecommendation:
    ...
    def find_listing_recommendations(self, request: operations.FindListingRecommendationsRequest, security: operations.FindListingRecommendationsSecurity) -> operations.FindListingRecommendationsResponse:
        ...
        req_content_type, data, form = utils.serialize_request_body(request, "find_listing_recommendation_request", 'json')
        if req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        query_params = utils.get_query_params(operations.FindListingRecommendationsRequest, request)
        ...
        res = operations.FindListingRecommendationsResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json;charset=UTF-8'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.PagedListingRecommendationCollection])
