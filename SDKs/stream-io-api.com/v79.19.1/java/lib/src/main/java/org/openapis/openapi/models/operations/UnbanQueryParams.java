/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import org.openapis.openapi.utils.SpeakeasyMetadata;

public class UnbanQueryParams {
    @SpeakeasyMetadata("queryParam:serialization=json,name=id")
    public String id;
    public UnbanQueryParams withId(String id) {
        this.id = id;
        return this;
    }
    
    @SpeakeasyMetadata("queryParam:serialization=json,name=target_user_id")
    public String targetUserId;
    public UnbanQueryParams withTargetUserId(String targetUserId) {
        this.targetUserId = targetUserId;
        return this;
    }
    
    @SpeakeasyMetadata("queryParam:serialization=json,name=type")
    public String type;
    public UnbanQueryParams withType(String type) {
        this.type = type;
        return this;
    }
    
}
