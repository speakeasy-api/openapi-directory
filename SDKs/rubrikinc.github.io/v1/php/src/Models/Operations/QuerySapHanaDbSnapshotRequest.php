<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class QuerySapHanaDbSnapshotRequest
{
    /**
     * Filter snapshots to those taken on or after this time. The date-time string should be in ISO8601 format, such as "2016-01-01T01:23:45.678".
     * 
     * @var ?\DateTime $afterTime
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=after_time,dateTimeFormat=Y-m-d\TH:i:s.up')]
    public ?\DateTime $afterTime = null;
    
    /**
     * Filter snapshots to those of a particular backup type.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\QuerySapHanaDbSnapshotBackupTypeEnum $backupType
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=backup_type')]
    public ?QuerySapHanaDbSnapshotBackupTypeEnum $backupType = null;
    
    /**
     * Filter snapshots to those taken before or on this time. The date-time string should be in ISO8601 format, such as "2016-01-01T01:23:45.678".
     * 
     * @var ?\DateTime $beforeTime
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=before_time,dateTimeFormat=Y-m-d\TH:i:s.up')]
    public ?\DateTime $beforeTime = null;
    
    /**
     * ID assigned to a SAP HANA database object.
     * 
     * @var string $id
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=id')]
    public string $id;
    
	public function __construct()
	{
		$this->afterTime = null;
		$this->backupType = null;
		$this->beforeTime = null;
		$this->id = "";
	}
}
