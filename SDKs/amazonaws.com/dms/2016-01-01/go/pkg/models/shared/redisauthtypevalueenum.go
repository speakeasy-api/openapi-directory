// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
)

type RedisAuthTypeValueEnum string

const (
	RedisAuthTypeValueEnumNone      RedisAuthTypeValueEnum = "none"
	RedisAuthTypeValueEnumAuthRole  RedisAuthTypeValueEnum = "auth-role"
	RedisAuthTypeValueEnumAuthToken RedisAuthTypeValueEnum = "auth-token"
)

func (e RedisAuthTypeValueEnum) ToPointer() *RedisAuthTypeValueEnum {
	return &e
}

func (e *RedisAuthTypeValueEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "none":
		fallthrough
	case "auth-role":
		fallthrough
	case "auth-token":
		*e = RedisAuthTypeValueEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for RedisAuthTypeValueEnum: %v", v)
	}
}
