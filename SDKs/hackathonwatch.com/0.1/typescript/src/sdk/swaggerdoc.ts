  ...
  **/
  getSwaggerDocFormat(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetSwaggerDocFormatResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  getSwaggerDocNameFormat(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetSwaggerDocNameFormatResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
