...
use \OpenAPI\OpenAPI\Models\Operations\CreateBatchRequestRequest;
use \OpenAPI\OpenAPI\Models\Operations\CreateBatchRequestRequestBody;
use \OpenAPI\OpenAPI\Models\Shared\BatchRequest;
use \OpenAPI\OpenAPI\Models\Shared\BatchRequestAction;
use \OpenAPI\OpenAPI\Models\Shared\BatchRequestActionMethodEnum;
use \OpenAPI\OpenAPI\Models\Shared\BatchRequestActionOptions;

$sdk = SDK::builder()
    ->build();

try {
    $request = new CreateBatchRequestRequest();
    $request->requestBody = new CreateBatchRequestRequestBody();
    $request->requestBody->data = new BatchRequest();
    $request->requestBody->data->actions = [
        new BatchRequestAction(),
        new BatchRequestAction(),
        new BatchRequestAction(),
        new BatchRequestAction(),
    ];
    ...
    $response = $sdk->batchAPI->createBatchRequest($request);

    if ($response->createBatchRequest200ApplicationJSONObject !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
