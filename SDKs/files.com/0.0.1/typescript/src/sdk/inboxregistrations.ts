...
export class InboxRegistrations {
  ...
  getInboxRegistrations(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetInboxRegistrationsResponse =
        new operations.GetInboxRegistrationsResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.inboxRegistrationEntities = [];
            const resFieldDepth: number = utils.getResFieldDepth(res);
            res.inboxRegistrationEntities = utils.objectToClass(
              httpRes?.data,
              shared.InboxRegistrationEntity,
              resFieldDepth
            );
          }
          break;
