  ...
  **/
  putOrdersIdShip(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.PutOrdersIdShipResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  putOrdersIdStatus(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PutOrdersIdStatusResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.oneordersPostResponses201ContentApplication1jsonSchema = httpRes?.data;
