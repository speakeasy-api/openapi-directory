"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import certificatereissue as shared_certificatereissue
from ..shared import error as shared_error
from typing import Optional


@dataclasses.dataclass
class CertificateReissueRequest:
    
    certificate_id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'certificateId', 'style': 'simple', 'explode': False }})
    r"""Certificate id to reissue"""  
    certificate_reissue: shared_certificatereissue.CertificateReissue = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})
    r"""The reissue request info"""  
    

@dataclasses.dataclass
class CertificateReissueResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    error: Optional[shared_error.Error] = dataclasses.field(default=None)
    r"""Request was malformed"""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    