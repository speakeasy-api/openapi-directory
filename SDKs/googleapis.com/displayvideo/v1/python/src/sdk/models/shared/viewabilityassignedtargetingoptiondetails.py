"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
from dataclasses_json import Undefined, dataclass_json
from enum import Enum
from sdk import utils
from typing import Optional

class ViewabilityAssignedTargetingOptionDetailsViewabilityEnum(str, Enum):
    r"""The predicted viewability percentage. Output only in v1. Required in v2."""
    VIEWABILITY_UNSPECIFIED = 'VIEWABILITY_UNSPECIFIED'
    VIEWABILITY_10_PERCENT_OR_MORE = 'VIEWABILITY_10_PERCENT_OR_MORE'
    VIEWABILITY_20_PERCENT_OR_MORE = 'VIEWABILITY_20_PERCENT_OR_MORE'
    VIEWABILITY_30_PERCENT_OR_MORE = 'VIEWABILITY_30_PERCENT_OR_MORE'
    VIEWABILITY_40_PERCENT_OR_MORE = 'VIEWABILITY_40_PERCENT_OR_MORE'
    VIEWABILITY_50_PERCENT_OR_MORE = 'VIEWABILITY_50_PERCENT_OR_MORE'
    VIEWABILITY_60_PERCENT_OR_MORE = 'VIEWABILITY_60_PERCENT_OR_MORE'
    VIEWABILITY_70_PERCENT_OR_MORE = 'VIEWABILITY_70_PERCENT_OR_MORE'
    VIEWABILITY_80_PERCENT_OR_MORE = 'VIEWABILITY_80_PERCENT_OR_MORE'
    VIEWABILITY_90_PERCENT_OR_MORE = 'VIEWABILITY_90_PERCENT_OR_MORE'


@dataclass_json(undefined=Undefined.EXCLUDE)
@dataclasses.dataclass
class ViewabilityAssignedTargetingOptionDetails:
    r"""Assigned viewability targeting option details. This will be populated in the viewability_details field of an AssignedTargetingOption when targeting_type is `TARGETING_TYPE_VIEWABILITY`."""
    
    targeting_option_id: Optional[str] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('targetingOptionId'), 'exclude': lambda f: f is None }})
    r"""Required. The targeting_option_id of a TargetingOption of type `TARGETING_TYPE_VIEWABILITY` (e.g., \\"509010\\" for targeting the `VIEWABILITY_10_PERCENT_OR_MORE` option)."""  
    viewability: Optional[ViewabilityAssignedTargetingOptionDetailsViewabilityEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('viewability'), 'exclude': lambda f: f is None }})
    r"""The predicted viewability percentage. Output only in v1. Required in v2."""  
    