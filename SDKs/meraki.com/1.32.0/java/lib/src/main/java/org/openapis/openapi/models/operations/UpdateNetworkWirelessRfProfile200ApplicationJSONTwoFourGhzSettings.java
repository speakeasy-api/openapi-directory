/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings - Settings related to 2.4Ghz band
 */
public class UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings {
    /**
     * Determines whether ax radio on 2.4Ghz band is on or off. Can be either true or false. If false, we highly recommend disabling band steering. Defaults to true.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("axEnabled")
    public Boolean axEnabled;

    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings withAxEnabled(Boolean axEnabled) {
        this.axEnabled = axEnabled;
        return this;
    }
    
    /**
     * Sets max power (dBm) of 2.4Ghz band. Can be integer between 2 and 30. Defaults to 30.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("maxPower")
    public Long maxPower;

    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings withMaxPower(Long maxPower) {
        this.maxPower = maxPower;
        return this;
    }
    
    /**
     * Sets min bitrate (Mbps) of 2.4Ghz band. Can be one of '1', '2', '5.5', '6', '9', '11', '12', '18', '24', '36', '48' or '54'. Defaults to 11.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("minBitrate")
    public Float minBitrate;

    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings withMinBitrate(Float minBitrate) {
        this.minBitrate = minBitrate;
        return this;
    }
    
    /**
     * Sets min power (dBm) of 2.4Ghz band. Can be integer between 2 and 30. Defaults to 5.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("minPower")
    public Long minPower;

    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings withMinPower(Long minPower) {
        this.minPower = minPower;
        return this;
    }
    
    /**
     * The RX-SOP level controls the sensitivity of the radio. It is strongly recommended to use RX-SOP only after consulting a wireless expert. RX-SOP can be configured in the range of -65 to -95 (dBm). A value of null will reset this to the default.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("rxsop")
    public Long rxsop;

    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings withRxsop(Long rxsop) {
        this.rxsop = rxsop;
        return this;
    }
    
    /**
     * Sets valid auto channels for 2.4Ghz band. Can be one of '1', '6' or '11'. Defaults to [1, 6, 11].
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("validAutoChannels")
    public Long[] validAutoChannels;

    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings withValidAutoChannels(Long[] validAutoChannels) {
        this.validAutoChannels = validAutoChannels;
        return this;
    }
    
    public UpdateNetworkWirelessRfProfile200ApplicationJSONTwoFourGhzSettings(){}
}
