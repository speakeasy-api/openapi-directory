import dataclasses
from typing import Optional
from dataclasses_json import dataclass_json
from sdk import utils
from ..shared import batchpermissionsrequestentry as shared_batchpermissionsrequestentry


@dataclass_json
@dataclasses.dataclass
class BatchRevokePermissionsRequest:
    entries: list[shared_batchpermissionsrequestentry.BatchPermissionsRequestEntry] = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.field_name('Entries') }})
    catalog_id: Optional[str] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.field_name('CatalogId') }})
    
