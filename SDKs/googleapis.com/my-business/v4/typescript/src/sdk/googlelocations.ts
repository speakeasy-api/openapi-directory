...
export class GoogleLocations {
  ...
  mybusinessGoogleLocationsReport(
    ...
    const url: string = utils.generateURL(baseURL, "/v4/{name}:report", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.MybusinessGoogleLocationsReportResponse =
        new operations.MybusinessGoogleLocationsReportResponse({
  ...
  mybusinessGoogleLocationsSearch(
    ...
    const baseURL: string = this._serverURL;
    const url: string =
      baseURL.replace(/\/$/, "") + "/v4/googleLocations:search";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.MybusinessGoogleLocationsSearchResponse =
        new operations.MybusinessGoogleLocationsSearchResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.searchGoogleLocationsResponse = utils.objectToClass(
              httpRes?.data,
              shared.SearchGoogleLocationsResponse
            );
          }
          break;
      }
