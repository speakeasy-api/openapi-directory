/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import org.openapis.openapi.utils.SpeakeasyMetadata;

public class ListEmployeeWagesQueryParams {
    /**
     * A pointer to the next page of `EmployeeWage` results to fetch.
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=cursor")
    public String cursor;
    public ListEmployeeWagesQueryParams withCursor(String cursor) {
        this.cursor = cursor;
        return this;
    }
    
    /**
     * Filter the returned wages to only those that are associated with the specified employee.
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=employee_id")
    public String employeeId;
    public ListEmployeeWagesQueryParams withEmployeeId(String employeeId) {
        this.employeeId = employeeId;
        return this;
    }
    
    /**
     * The maximum number of `EmployeeWage` results to return per page. The number can range between
     * 1 and 200. The default is 200.
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=limit")
    public Long limit;
    public ListEmployeeWagesQueryParams withLimit(Long limit) {
        this.limit = limit;
        return this;
    }
    
}
