import dataclasses
from typing import Optional
from ..shared import security as shared_security
from ..shared import autopilot_v1_assistant_task_sample as shared_autopilot_v1_assistant_task_sample


CREATE_SAMPLE_SERVERS = [
	"https://autopilot.twilio.com",
]


@dataclasses.dataclass
class CreateSamplePathParams:
    assistant_sid: str = dataclasses.field(metadata={'path_param': { 'field_name': 'AssistantSid', 'style': 'simple', 'explode': False }})
    task_sid: str = dataclasses.field(metadata={'path_param': { 'field_name': 'TaskSid', 'style': 'simple', 'explode': False }})
    

@dataclasses.dataclass
class CreateSampleCreateSampleRequest:
    language: str = dataclasses.field(metadata={'form': { 'field_name': 'Language' }})
    tagged_text: str = dataclasses.field(metadata={'form': { 'field_name': 'TaggedText' }})
    source_channel: Optional[str] = dataclasses.field(default=None, metadata={'form': { 'field_name': 'SourceChannel' }})
    

@dataclasses.dataclass
class CreateSampleSecurity:
    account_sid_auth_token: shared_security.SchemeAccountSidAuthToken = dataclasses.field(metadata={'security': { 'scheme': True, 'type': 'http', 'sub_type': 'basic' }})
    

@dataclasses.dataclass
class CreateSampleRequest:
    path_params: CreateSamplePathParams = dataclasses.field()
    security: CreateSampleSecurity = dataclasses.field()
    request: Optional[CreateSampleCreateSampleRequest] = dataclasses.field(default=None, metadata={'request': { 'media_type': 'application/x-www-form-urlencoded' }})
    server_url: Optional[str] = dataclasses.field(default=None)
    

@dataclasses.dataclass
class CreateSampleResponse:
    content_type: str = dataclasses.field()
    status_code: int = dataclasses.field()
    autopilot_v1_assistant_task_sample: Optional[shared_autopilot_v1_assistant_task_sample.AutopilotV1AssistantTaskSample] = dataclasses.field(default=None)
    
