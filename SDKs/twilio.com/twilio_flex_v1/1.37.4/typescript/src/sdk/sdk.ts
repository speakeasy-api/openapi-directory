...
export class SDK {
  ...
  **/
  createAssessments(
  ...
  ): Promise<operations.CreateAssessmentsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.CreateAssessmentsRequest(req);
    }
    
    let baseURL: string = operations.CreateAssessmentsServerList[0];
    if (req.serverURL) {
      baseURL = req.serverURL;
    }
    const url: string = baseURL.replace(/\/$/, "") + "/v1/Accounts/Assessments";
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.CreateAssessmentsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 201:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.flexV1Assessments = httpRes?.data;
            }
            break;
        }
