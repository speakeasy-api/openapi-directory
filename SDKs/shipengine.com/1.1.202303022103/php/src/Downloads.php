...
class Downloads 
{
    ...
    ): \OpenAPI\OpenAPI\Models\Operations\DownloadFileResponse
    {
        ...
        $url = Utils\Utils::generateUrl($baseUrl, '/v1/downloads/{dir}/{subdir}/{filename}', \OpenAPI\OpenAPI\Models\Operations\DownloadFileRequest::class, $request);
        
        $options = ['http_errors' => false];
        $options = array_merge_recursive($options, Utils\Utils::getQueryParams(\OpenAPI\OpenAPI\Models\Operations\DownloadFileRequest::class, $request, null));
        ...
        if ($httpResponse->getStatusCode() === 200) {
            if (Utils\Utils::matchContentType($contentType, 'application/pdf')) {
                $response->downloadFilePdfResponseBody = $httpResponse->getBody()->getContents();
            }
            if (Utils\Utils::matchContentType($contentType, 'application/zpl')) {
                $response->downloadFileZplResponseBody = $httpResponse->getBody()->getContents();
            }
            if (Utils\Utils::matchContentType($contentType, 'image/png')) {
                $response->downloadFilePngResponseBody = $httpResponse->getBody()->getContents();
            }
        }
