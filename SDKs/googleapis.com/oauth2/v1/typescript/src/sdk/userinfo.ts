...
export class Userinfo {
  ...
  **/
  oauth2UserinfoGet(
    ...
    const url: string = baseURL.replace(/\/$/, "") + "/oauth2/v1/userinfo";
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.Oauth2UserinfoGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  oauth2UserinfoV2MeGet(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.Oauth2UserinfoV2MeGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.userinfo = httpRes?.data;
