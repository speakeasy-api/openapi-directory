...
export class Tags {
  ...
  tagsList(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.TagsListResponse = new operations.TagsListResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.tagsList200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.TagsList200ApplicationJSON
            );
          }
          break;
      }
  ...
  tagsRead(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.TagsReadResponse = new operations.TagsReadResponse({
        statusCode: httpRes.status,
        contentType: contentType,
        rawResponse: httpRes,
      });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.tagSingle = utils.objectToClass(
              httpRes?.data,
              shared.TagSingle
            );
          }
