import dataclasses
from typing import Any,Optional
from enum import Enum

class DfsSlatesByWeekFormatEnum(str, Enum):
    XML = "XML"
    JSON = "JSON"


@dataclasses.dataclass
class DfsSlatesByWeekPathParams:
    format: DfsSlatesByWeekFormatEnum = dataclasses.field(metadata={'path_param': { 'field_name': 'format', 'style': 'simple', 'explode': False }})
    season: str = dataclasses.field(metadata={'path_param': { 'field_name': 'season', 'style': 'simple', 'explode': False }})
    week: str = dataclasses.field(metadata={'path_param': { 'field_name': 'week', 'style': 'simple', 'explode': False }})
    

@dataclasses.dataclass
class DfsSlatesByWeekRequest:
    path_params: DfsSlatesByWeekPathParams = dataclasses.field()
    

@dataclasses.dataclass
class DfsSlatesByWeekResponse:
    content_type: str = dataclasses.field()
    status_code: int = dataclasses.field()
    dfs_slates: Optional[list[Any]] = dataclasses.field(default=None)
    
