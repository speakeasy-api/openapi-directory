<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;


class EditVideoRequestBodyPrivacy
{
    /**
     * Whether a user can add the video to an album, channel, or group.
     * 
     * @var ?bool $add
     */
	#[\JMS\Serializer\Annotation\SerializedName('add')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $add = null;
    
    /**
     * The privacy level required to comment on the video.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\EditVideoRequestBodyPrivacyCommentsEnum $comments
     */
	#[\JMS\Serializer\Annotation\SerializedName('comments')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Operations\EditVideoRequestBodyPrivacyCommentsEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?EditVideoRequestBodyPrivacyCommentsEnum $comments = null;
    
    /**
     * Whether a user can download the video. Not available to users with a Basic membership.
     * 
     * @var ?bool $download
     */
	#[\JMS\Serializer\Annotation\SerializedName('download')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $download = null;
    
    /**
     * The video's new embed settings. The `whitelist` value enables you to define all valid embed domains. See our [documentation](https://developer.vimeo.com/api/endpoints/videos#/{video_id}/privacy/domains) for details on adding and removing domains.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\EditVideoRequestBodyPrivacyEmbedEnum $embed
     */
	#[\JMS\Serializer\Annotation\SerializedName('embed')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Operations\EditVideoRequestBodyPrivacyEmbedEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?EditVideoRequestBodyPrivacyEmbedEnum $embed = null;
    
    /**
     * The video's new privacy setting. When privacy is `users`, `application/json` is the only valid content type. Basic users can't set privacy to `unlisted`.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\EditVideoRequestBodyPrivacyViewEnum $view
     */
	#[\JMS\Serializer\Annotation\SerializedName('view')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Operations\EditVideoRequestBodyPrivacyViewEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?EditVideoRequestBodyPrivacyViewEnum $view = null;
    
	public function __construct()
	{
		$this->add = null;
		$this->comments = null;
		$this->download = null;
		$this->embed = null;
		$this->view = null;
	}
}
