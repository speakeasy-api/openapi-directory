/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.shared;

import com.fasterxml.jackson.annotation.JsonProperty;

public class Blacklist {
    /**
     * True if the host is currently black-listed
     */
    @JsonProperty("is-listed")
    public Boolean isListed;

    public Blacklist withIsListed(Boolean isListed) {
        this.isListed = isListed;
        return this;
    }
    
    /**
     * The hostname of the DNSBL
     */
    @JsonProperty("list-host")
    public String listHost;

    public Blacklist withListHost(String listHost) {
        this.listHost = listHost;
        return this;
    }
    
    /**
     * The name of the DNSBL
     */
    @JsonProperty("list-name")
    public String listName;

    public Blacklist withListName(String listName) {
        this.listName = listName;
        return this;
    }
    
    /**
     * The list rating [1-3] with 1 being the best rating and 3 the lowest rating
     */
    @JsonProperty("list-rating")
    public Integer listRating;

    public Blacklist withListRating(Integer listRating) {
        this.listRating = listRating;
        return this;
    }
    
    /**
     * The DNSBL server response time in milliseconds
     */
    @JsonProperty("response-time")
    public Integer responseTime;

    public Blacklist withResponseTime(Integer responseTime) {
        this.responseTime = responseTime;
        return this;
    }
    
    /**
     * The specific return code for this listing (only set if listed)
     */
    @JsonProperty("return-code")
    public String returnCode;

    public Blacklist withReturnCode(String returnCode) {
        this.returnCode = returnCode;
        return this;
    }
    
    /**
     * The TXT record returned for this listing (only set if listed)
     */
    @JsonProperty("txt-record")
    public String txtRecord;

    public Blacklist withTxtRecord(String txtRecord) {
        this.txtRecord = txtRecord;
        return this;
    }
    
    public Blacklist(@JsonProperty("is-listed") Boolean isListed, @JsonProperty("list-host") String listHost, @JsonProperty("list-name") String listName, @JsonProperty("list-rating") Integer listRating, @JsonProperty("response-time") Integer responseTime, @JsonProperty("return-code") String returnCode, @JsonProperty("txt-record") String txtRecord) {
        this.isListed = isListed;
        this.listHost = listHost;
        this.listName = listName;
        this.listRating = listRating;
        this.responseTime = responseTime;
        this.returnCode = returnCode;
        this.txtRecord = txtRecord;
  }
}
