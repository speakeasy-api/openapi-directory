// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"net/http"
	"openapi/pkg/models/shared"
)

type FindKeywordLeaseConfigsSecurity struct {
	BasicAuth shared.SchemeBasicAuth `security:"scheme,type=http,subtype=basic"`
}

type FindKeywordLeaseConfigsQueryParams struct {
	// Limit fields received in response. E.g. fields: id, name or fields items (id, name), see more at [partial response](https://developers.callfire.com/docs.html#partial-response) page.
	Fields *string `queryParam:"style=form,explode=true,name=fields"`
	// Filter by part of Keyword name or Label name of Keyword
	Filter *string `queryParam:"style=form,explode=true,name=filter"`
	// An exact label name to search by
	LabelName *string `queryParam:"style=form,explode=true,name=labelName"`
	// To set the maximum number of records to return in a paged list response. The default is 100
	Limit *int `queryParam:"style=form,explode=true,name=limit"`
	// Offset to the start of a given page. The default is 0. Check [pagination](https://developers.callfire.com/docs.html#pagination) page for more information about pagination in CallFire API.
	Offset *int `queryParam:"style=form,explode=true,name=offset"`
}

type FindKeywordLeaseConfigsRequest struct {
	QueryParams FindKeywordLeaseConfigsQueryParams
	Security    FindKeywordLeaseConfigsSecurity
}

type FindKeywordLeaseConfigsResponse struct {
	ContentType string
	// Bad request
	ErrorResponse *shared.ErrorResponse
	// successful operation
	Page        *shared.Page
	StatusCode  int
	RawResponse *http.Response
}
