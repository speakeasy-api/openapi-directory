"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import inline_response_400 as shared_inline_response_400
from ..shared import inline_response_401 as shared_inline_response_401
from ..shared import inline_response_403 as shared_inline_response_403
from ..shared import inline_response_404 as shared_inline_response_404
from ..shared import pagedpayeeresponsev4 as shared_pagedpayeeresponsev4
from typing import Optional


@dataclasses.dataclass
class ListPayeesV4Request:
    
    payor_id: str = dataclasses.field(metadata={'query_param': { 'field_name': 'payorId', 'style': 'form', 'explode': True }})
    r"""The account owner Payor ID"""  
    disabled: Optional[bool] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'disabled', 'style': 'form', 'explode': True }})
    r"""Payee disabled"""  
    display_name: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'displayName', 'style': 'form', 'explode': True }})
    r"""The display name of the payees."""  
    email: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'email', 'style': 'form', 'explode': True }})
    r"""Email address"""  
    ofac_status: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'ofacStatus', 'style': 'form', 'explode': True }})
    r"""The ofacStatus of the payees."""  
    onboarded_status: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'onboardedStatus', 'style': 'form', 'explode': True }})
    r"""The onboarded status of the payees."""  
    page: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'page', 'style': 'form', 'explode': True }})
    r"""Page number. Default is 1."""  
    page_size: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'pageSize', 'style': 'form', 'explode': True }})
    r"""Page size. Default is 25. Max allowable is 100."""  
    payee_country: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'payeeCountry', 'style': 'form', 'explode': True }})
    r"""The country of the payee - 2 letter ISO 3166-1 country code (upper case)"""  
    payee_type: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'payeeType', 'style': 'form', 'explode': True }})
    r"""The onboarded status of the payees."""  
    remote_id: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'remoteId', 'style': 'form', 'explode': True }})
    r"""The remote id of the payees."""  
    sort: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'sort', 'style': 'form', 'explode': True }})
    r"""List of sort fields (e.g. ?sort=onboardedStatus:asc,name:asc) Default is name:asc 'name' is treated as company name for companies - last name + ',' + firstName for individuals The supported sort fields are - payeeId, displayName, payoutStatus, onboardedStatus."""  
    watchlist_status: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'watchlistStatus', 'style': 'form', 'explode': True }})
    r"""The watchlistStatus of the payees."""  
    

@dataclasses.dataclass
class ListPayeesV4Response:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    inline_response_400: Optional[shared_inline_response_400.InlineResponse400] = dataclasses.field(default=None)
    r"""Invalid request. See Error message payload for details of failure"""  
    inline_response_401: Optional[shared_inline_response_401.InlineResponse401] = dataclasses.field(default=None)
    r"""Invalid access token. May be expired or invalid"""  
    inline_response_403: Optional[shared_inline_response_403.InlineResponse403] = dataclasses.field(default=None)
    r"""The authentication does not have permissions to access the resource
    This usually occurs when there is a valid authentication instance (client or user) but they do not have the required permissions
    """  
    inline_response_404: Optional[shared_inline_response_404.InlineResponse404] = dataclasses.field(default=None)
    r"""The resource was not found or is no longer available"""  
    paged_payee_response_v4: Optional[shared_pagedpayeeresponsev4.PagedPayeeResponseV4] = dataclasses.field(default=None)
    r"""Details of Payee"""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    