...
export class Events {
  ...
  storyIdEventsGet(
    ...
    const url: string = utils.generateURL(baseURL, "/{id}/events", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.StoryIdEventsGetResponse =
        new operations.StoryIdEventsGetResponse({
  ...
  storyIdEventsPost(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.StoryIdEventsPostResponse =
        new operations.StoryIdEventsPostResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.storyIdEventsPost200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data
            );
          }
          break;
