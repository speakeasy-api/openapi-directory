...
export class Reports {
  ...
  analyticsreportingReportsBatchGet(
    ...
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.AnalyticsreportingReportsBatchGetSecurity(
        security
      );
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.AnalyticsreportingReportsBatchGetResponse =
        new operations.AnalyticsreportingReportsBatchGetResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getReportsResponse = utils.objectToClass(
              httpRes?.data,
              shared.GetReportsResponse
            );
          }
          break;
      }
