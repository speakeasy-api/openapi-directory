...
export class ProvisioningJobs {
  ...
  getProvisioningjobsJobId(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetProvisioningjobsJobIdResponse =
        new operations.GetProvisioningjobsJobIdResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.provisioningJobInfo = utils.objectToClass(
              httpRes?.data,
              shared.ProvisioningJobInfo
            );
          }
          break;
        case httpRes?.status == 201:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.provisioningJobCompletion = utils.objectToClass(
              httpRes?.data,
              shared.ProvisioningJobCompletion
            );
          }
          break;
      }
