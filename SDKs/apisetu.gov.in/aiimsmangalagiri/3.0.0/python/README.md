...
```bash
pip install git+https://github.com/speakeasy-api/openapi-directory.git#subdirectory=SDKs/apisetu.gov.in/aiimsmangalagiri/3.0.0/python
```
...
req = operations.LabrpRequestBody(
    certificate_parameters=operations.LabrpRequestBodyCertificateParameters(
        ...
        gender="M",
        mobile_no_rq="DigiLocker will only fetch the latest report if multiple laboratory reports are found.",
    ),
...
res = s.ap_is.labrp(req, operations.LabrpSecurity(
