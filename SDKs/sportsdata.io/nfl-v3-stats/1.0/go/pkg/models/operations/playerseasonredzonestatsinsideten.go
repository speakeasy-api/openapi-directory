// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"encoding/json"
	"fmt"
	"net/http"
	"openapi/pkg/models/shared"
)

// PlayerSeasonRedZoneStatsInsideTenFormatEnum - Desired response format. Valid entries are <code>XML</code> or <code>JSON</code>.
type PlayerSeasonRedZoneStatsInsideTenFormatEnum string

const (
	PlayerSeasonRedZoneStatsInsideTenFormatEnumXML  PlayerSeasonRedZoneStatsInsideTenFormatEnum = "XML"
	PlayerSeasonRedZoneStatsInsideTenFormatEnumJSON PlayerSeasonRedZoneStatsInsideTenFormatEnum = "JSON"
)

func (e PlayerSeasonRedZoneStatsInsideTenFormatEnum) ToPointer() *PlayerSeasonRedZoneStatsInsideTenFormatEnum {
	return &e
}

func (e *PlayerSeasonRedZoneStatsInsideTenFormatEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "XML":
		fallthrough
	case "JSON":
		*e = PlayerSeasonRedZoneStatsInsideTenFormatEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for PlayerSeasonRedZoneStatsInsideTenFormatEnum: %v", v)
	}
}

type PlayerSeasonRedZoneStatsInsideTenRequest struct {
	// Desired response format. Valid entries are <code>XML</code> or <code>JSON</code>.
	Format PlayerSeasonRedZoneStatsInsideTenFormatEnum `pathParam:"style=simple,explode=false,name=format"`
	// Year of the season and the season type. If no season type is provided, then the default is regular season.
	//           <br>Examples: <code>2015REG</code>, <code>2015PRE</code>, <code>2015POST</code>.
	//
	Season string `pathParam:"style=simple,explode=false,name=season"`
}

type PlayerSeasonRedZoneStatsInsideTenResponse struct {
	ContentType          string
	PlayerSeasonRedZones []shared.PlayerSeasonRedZone
	StatusCode           int
	RawResponse          *http.Response
}
