...
export class PodcasterAPI {
  ...
  deletePodcastById(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.DeletePodcastByIdResponse =
        new operations.DeletePodcastByIdResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.deletePodcastResponse = utils.objectToClass(
              httpRes?.data,
              shared.DeletePodcastResponse
            );
          }
          break;
  ...
  submitPodcast(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.SubmitPodcastResponse =
        new operations.SubmitPodcastResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
          headers: utils.getHeadersFromResponse(httpRes.headers),
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.submitPodcastResponse = utils.objectToClass(
              httpRes?.data,
              shared.SubmitPodcastResponse
            );
          }
          break;
