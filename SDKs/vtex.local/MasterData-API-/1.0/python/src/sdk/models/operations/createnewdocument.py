"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import createnewdocument as shared_createnewdocument
from typing import Any, Optional


@dataclasses.dataclass
class CreatenewdocumentRequest:
    
    accept: str = dataclasses.field(metadata={'header': { 'field_name': 'Accept', 'style': 'simple', 'explode': False }})
    r"""HTTP Client Negotiation _Accept_ Header. Indicates the types of responses the client can understand"""  
    acronym: str = dataclasses.field(metadata={'path_param': { 'field_name': 'acronym', 'style': 'simple', 'explode': False }})
    r"""Identifies the kind of data"""  
    request_body: dict[str, Any] = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})  
    

@dataclasses.dataclass
class CreatenewdocumentResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    createnewdocument: Optional[shared_createnewdocument.Createnewdocument] = dataclasses.field(default=None)  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    