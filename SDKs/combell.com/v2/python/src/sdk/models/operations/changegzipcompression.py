"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import gzipconfig as shared_gzipconfig
from typing import Optional


@dataclasses.dataclass
class ChangeGzipCompressionRequest:
    
    domain_name_query_parameter: str = dataclasses.field(metadata={'query_param': { 'field_name': 'domain_name', 'style': 'form', 'explode': True }})
    r"""Linux hosting domain name"""  
    domain_name_path_parameter: str = dataclasses.field(metadata={'path_param': { 'field_name': 'domainName', 'style': 'simple', 'explode': False }})
    r"""Automatically added"""  
    gzip_config: Optional[shared_gzipconfig.GzipConfig] = dataclasses.field(default=None, metadata={'request': { 'media_type': 'application/json' }})
    r"""Whether GZIP compression is enabled or not."""  
    

@dataclasses.dataclass
class ChangeGzipCompressionResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    