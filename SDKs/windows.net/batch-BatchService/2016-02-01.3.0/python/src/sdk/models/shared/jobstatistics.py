...
class JobStatistics:
    ...
    r"""The total kernel mode CPU time (summed across all cores and all compute nodes) consumed by all tasks in the job."""  
    ...
    r"""The total user mode CPU time (summed across all cores and all compute nodes) consumed by all tasks in the job."""  
    wait_time: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('waitTime') }})
    r"""The total wait time of all tasks in the job. The wait time for a task is defined as the elapsed time between the creation of the task and the start of task execution. (If the task is retried due to failures, the wait time is the time to the most recent task execution.)"""  
    wall_clock_time: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('wallClockTime') }})
    r"""The total wall clock time of all tasks in the job."""  
    write_io_gi_b: float = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('writeIOGiB') }})
    r"""The total gibibytes written to disk by all tasks in the job."""  
    write_i_ops: int = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('writeIOps') }})
    r"""The total number of disk write operations made by all tasks in the job."""  
