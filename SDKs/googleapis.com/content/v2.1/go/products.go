...
func (s *products) ContentProductsUpdate(ctx context.Context, request operations.ContentProductsUpdateRequest, security operations.ContentProductsUpdateSecurity) (*operations.ContentProductsUpdateResponse, error) {
	baseURL := s.serverURL
	url, err := utils.GenerateURL(ctx, baseURL, "/{merchantId}/products/{productId}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
