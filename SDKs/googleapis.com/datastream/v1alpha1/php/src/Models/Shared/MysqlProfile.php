<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * MysqlProfile - MySQL database profile.
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class MysqlProfile
{
    /**
     * Required. Hostname for the MySQL connection.
     * 
     * @var ?string $hostname
     */
	#[\JMS\Serializer\Annotation\SerializedName('hostname')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $hostname = null;
    
    /**
     * Required. Input only. Password for the MySQL connection.
     * 
     * @var ?string $password
     */
	#[\JMS\Serializer\Annotation\SerializedName('password')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $password = null;
    
    /**
     * Port for the MySQL connection, default value is 3306.
     * 
     * @var ?int $port
     */
	#[\JMS\Serializer\Annotation\SerializedName('port')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $port = null;
    
    /**
     * MySQL SSL configuration information.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\MysqlSslConfig $sslConfig
     */
	#[\JMS\Serializer\Annotation\SerializedName('sslConfig')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\MysqlSslConfig')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?MysqlSslConfig $sslConfig = null;
    
    /**
     * Required. Username for the MySQL connection.
     * 
     * @var ?string $username
     */
	#[\JMS\Serializer\Annotation\SerializedName('username')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $username = null;
    
	public function __construct()
	{
		$this->hostname = null;
		$this->password = null;
		$this->port = null;
		$this->sslConfig = null;
		$this->username = null;
	}
}
