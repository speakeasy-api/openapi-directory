...
export class AppPkgm {
  ...
  **/
  subscriptionsPost(
    req: operations.SubscriptionsPostRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.SubscriptionsPostResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.SubscriptionsPostRequest(req);
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.SubscriptionsPostResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 201:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.appPkgSubscriptionInfo = httpRes?.data;
