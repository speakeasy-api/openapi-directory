...
```bash
pip install git+https://github.com/speakeasy-api/openapi-directory.git#subdirectory=SDKs/amazonaws.com/groundstation/2019-05-23/python
```
...
* `create_ephemeris` - Creates an Ephemeris with the specified <code>EphemerisData</code>.
...
* `delete_ephemeris` - Deletes an ephemeris
* `delete_mission_profile` - Deletes a mission profile.
* `describe_contact` - Describes an existing contact.
* `describe_ephemeris` - Describes an existing ephemeris.
* `get_agent_configuration` - Gets the latest configuration information for a registered agent.
...
* `list_ephemerides` - List existing ephemerides.
...
* `register_agent` - Registers a new agent with AWS Groundstation.
...
* `update_agent_status` - Update the status of the agent.
...
* `update_ephemeris` - Updates an existing ephemeris
