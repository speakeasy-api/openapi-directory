/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import org.openapis.openapi.utils.SpeakeasyMetadata;

public class GetSpecTemplatePathParams {
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=spec_template_id")
    public String specTemplateId;
    public GetSpecTemplatePathParams withSpecTemplateId(String specTemplateId) {
        this.specTemplateId = specTemplateId;
        return this;
    }
    
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=workgroup_id")
    public String workgroupId;
    public GetSpecTemplatePathParams withWorkgroupId(String workgroupId) {
        this.workgroupId = workgroupId;
        return this;
    }
    
}
