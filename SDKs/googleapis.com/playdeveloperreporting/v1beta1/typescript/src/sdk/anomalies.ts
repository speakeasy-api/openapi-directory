...
export class Anomalies {
  ...
  **/
  playdeveloperreportingAnomaliesList(
    ...
    const url: string = utils.generateURL(baseURL, "/v1beta1/{parent}/anomalies", req.pathParams);
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.PlaydeveloperreportingAnomaliesListResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.googlePlayDeveloperReportingV1beta1ListAnomaliesResponse = httpRes?.data;
            }
            break;
        }
