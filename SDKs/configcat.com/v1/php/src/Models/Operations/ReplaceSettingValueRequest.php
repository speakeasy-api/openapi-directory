<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class ReplaceSettingValueRequest
{
	#[SpeakeasyMetadata('request:mediaType=application/json')]
    public \OpenAPI\OpenAPI\Models\Shared\UpdateSettingValueModel $updateSettingValueModel;
    
    /**
     * The identifier of the Environment.
     * 
     * @var string $environmentId
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=environmentId')]
    public string $environmentId;
    
    /**
     * The reason note for the Audit Log if the Product's "Config changes require a reason" preference is turned on.
     * 
     * @var ?string $reason
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=reason')]
    public ?string $reason = null;
    
    /**
     * The id of the Setting.
     * 
     * @var int $settingId
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=settingId')]
    public int $settingId;
    
	public function __construct()
	{
		$this->updateSettingValueModel = new \OpenAPI\OpenAPI\Models\Shared\UpdateSettingValueModel();
		$this->environmentId = "";
		$this->reason = null;
		$this->settingId = 0;
	}
}
