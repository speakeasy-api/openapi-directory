...
export class Projects {
  ...
  **/
  policyanalyzerProjectsLocationsActivityTypesActivitiesQuery(
    ...
    const url: string = utils.generateURL(baseURL, "/v1beta1/{parent}/activities:query", req.pathParams);
        ...
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.googleCloudPolicyanalyzerV1beta1QueryActivityResponse = httpRes?.data;
            }
            break;
        }
