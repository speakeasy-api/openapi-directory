...
export class Organizations {
  ...
  **/
  assuredworkloadsOrganizationsLocationsWorkloadsMutatePartnerPermissions(
    ...
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.AssuredworkloadsOrganizationsLocationsWorkloadsMutatePartnerPermissionsRequest(req);
    }
    
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/v1/{name}:mutatePartnerPermissions", req.pathParams);
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.AssuredworkloadsOrganizationsLocationsWorkloadsMutatePartnerPermissionsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
