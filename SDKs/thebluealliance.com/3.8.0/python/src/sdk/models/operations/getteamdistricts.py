import dataclasses
from typing import Optional
from ..shared import security as shared_security
from ..shared import district_list as shared_district_list


@dataclasses.dataclass
class GetTeamDistrictsPathParams:
    team_key: str = dataclasses.field(metadata={'path_param': { 'field_name': 'team_key', 'style': 'simple', 'explode': False }})
    

@dataclasses.dataclass
class GetTeamDistrictsHeaders:
    if_modified_since: Optional[str] = dataclasses.field(default=None, metadata={'header': { 'field_name': 'If-Modified-Since', 'style': 'simple', 'explode': False }})
    

@dataclasses.dataclass
class GetTeamDistrictsSecurity:
    api_key: shared_security.SchemeAPIKey = dataclasses.field(metadata={'security': { 'scheme': True, 'type': 'apiKey', 'sub_type': 'header' }})
    

@dataclasses.dataclass
class GetTeamDistrictsRequest:
    headers: GetTeamDistrictsHeaders = dataclasses.field()
    path_params: GetTeamDistrictsPathParams = dataclasses.field()
    security: GetTeamDistrictsSecurity = dataclasses.field()
    

@dataclasses.dataclass
class GetTeamDistrictsResponse:
    content_type: str = dataclasses.field()
    headers: dict[str, list[str]] = dataclasses.field()
    status_code: int = dataclasses.field()
    district_lists: Optional[list[shared_district_list.DistrictList]] = dataclasses.field(default=None)
    
