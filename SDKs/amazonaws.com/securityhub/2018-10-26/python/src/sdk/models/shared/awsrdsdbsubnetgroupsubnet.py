"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
from ..shared import awsrdsdbsubnetgroupsubnetavailabilityzone as shared_awsrdsdbsubnetgroupsubnetavailabilityzone
from dataclasses_json import Undefined, dataclass_json
from sdk import utils
from typing import Optional


@dataclass_json(undefined=Undefined.EXCLUDE)
@dataclasses.dataclass
class AwsRdsDbSubnetGroupSubnet:
    r"""Information about a subnet in a subnet group."""
    
    subnet_availability_zone: Optional[shared_awsrdsdbsubnetgroupsubnetavailabilityzone.AwsRdsDbSubnetGroupSubnetAvailabilityZone] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('SubnetAvailabilityZone'), 'exclude': lambda f: f is None }})  
    subnet_identifier: Optional[str] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('SubnetIdentifier'), 'exclude': lambda f: f is None }})  
    subnet_status: Optional[str] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('SubnetStatus'), 'exclude': lambda f: f is None }})  
    