<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * CloudSqlConnectionProfile - Specifies required connection parameters, and, optionally, the parameters required to create a Cloud SQL destination database instance.
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class CloudSqlConnectionProfile
{
    /**
     * Output only. The Cloud SQL database instance's additional (outgoing) public IP. Used when the Cloud SQL database availability type is REGIONAL (i.e. multiple zones / highly available).
     * 
     * @var ?string $additionalPublicIp
     */
	#[\JMS\Serializer\Annotation\SerializedName('additionalPublicIp')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $additionalPublicIp = null;
    
    /**
     * Output only. The Cloud SQL instance ID that this connection profile is associated with.
     * 
     * @var ?string $cloudSqlId
     */
	#[\JMS\Serializer\Annotation\SerializedName('cloudSqlId')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $cloudSqlId = null;
    
    /**
     * Output only. The Cloud SQL database instance's private IP.
     * 
     * @var ?string $privateIp
     */
	#[\JMS\Serializer\Annotation\SerializedName('privateIp')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $privateIp = null;
    
    /**
     * Output only. The Cloud SQL database instance's public IP.
     * 
     * @var ?string $publicIp
     */
	#[\JMS\Serializer\Annotation\SerializedName('publicIp')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $publicIp = null;
    
    /**
     * Settings for creating a Cloud SQL database instance.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\CloudSqlSettings $settings
     */
	#[\JMS\Serializer\Annotation\SerializedName('settings')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\CloudSqlSettings')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?CloudSqlSettings $settings = null;
    
	public function __construct()
	{
		$this->additionalPublicIp = null;
		$this->cloudSqlId = null;
		$this->privateIp = null;
		$this->publicIp = null;
		$this->settings = null;
	}
}
