/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.shared;

import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * ListManifestsResponseBody - A list manifests response body
 */
public class ListManifestsResponseBody {
    /**
     * Helpful links to other pages of results
     */
    @JsonProperty("links")
    public ListManifestsResponseBodyPaginationLink links;

    public ListManifestsResponseBody withLinks(ListManifestsResponseBodyPaginationLink links) {
        this.links = links;
        return this;
    }
    
    /**
     * The list of available manifests
     */
    @JsonProperty("manifests")
    public ListManifestsResponseBodyManifest[] manifests;

    public ListManifestsResponseBody withManifests(ListManifestsResponseBodyManifest[] manifests) {
        this.manifests = manifests;
        return this;
    }
    
    /**
     * Current page of the list manifests results
     */
    @JsonProperty("page")
    public Integer page;

    public ListManifestsResponseBody withPage(Integer page) {
        this.page = page;
        return this;
    }
    
    /**
     * Total number of pages for list manifests results
     */
    @JsonProperty("pages")
    public Integer pages;

    public ListManifestsResponseBody withPages(Integer pages) {
        this.pages = pages;
        return this;
    }
    
    /**
     * The total number of manifests returned
     */
    @JsonProperty("total")
    public Long total;

    public ListManifestsResponseBody withTotal(Long total) {
        this.total = total;
        return this;
    }
    
    public ListManifestsResponseBody(@JsonProperty("links") ListManifestsResponseBodyPaginationLink links, @JsonProperty("manifests") ListManifestsResponseBodyManifest[] manifests, @JsonProperty("page") Integer page, @JsonProperty("pages") Integer pages, @JsonProperty("total") Long total) {
        this.links = links;
        this.manifests = manifests;
        this.page = page;
        this.pages = pages;
        this.total = total;
  }
}
