...
class Operations:
    ...
    def post_billing_preview(self, request: operations.POSTBillingPreviewRequest) -> operations.POSTBillingPreviewResponse:
        ...
        **Note:** The Billing Preview feature is in **Limited Availability**. If you wish to have access to the feature, submit a request at [Zuora Global Support](http://support.zuora.com/). 
        ...
        req_content_type, data, form = utils.serialize_request_body(request, "post_billing_preview_param", 'json')
        ...
        res = operations.POSTBillingPreviewResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            res.headers = http_res.headers
            
            if utils.match_content_type(content_type, 'application/json; charset=utf-8'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.BillingPreviewResult])
    ...
    def post_transaction_invoice_payment(self, request: operations.POSTTransactionInvoicePaymentRequest) -> operations.POSTTransactionInvoicePaymentResponse:
        ...
        headers = utils.get_headers(request)
        req_content_type, data, form = utils.serialize_request_body(request, "post_invoice_collect_type", 'json')
        if req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        if data is None and form is None:
            raise Exception('request body is required')
        ...
        res = operations.POSTTransactionInvoicePaymentResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            res.headers = http_res.headers
            
            if utils.match_content_type(content_type, 'application/json; charset=utf-8'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.POSTInvoiceCollectResponseType])
