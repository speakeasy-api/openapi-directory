...
export class V3Tax {
  ...
  allTaxRates(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.AllTaxRatesResponse =
        new operations.AllTaxRatesResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.allTaxRates200ApplicationJSONObjects = [];
            const resFieldDepth: number = utils.getResFieldDepth(res);
            res.allTaxRates200ApplicationJSONObjects = utils.objectToClass(
              httpRes?.data,
              operations.AllTaxRates200ApplicationJSON,
              resFieldDepth
            );
          }
          break;
        ...
        case httpRes?.status == 500:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.allTaxRates500ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.AllTaxRates500ApplicationJSON
            );
          }
          break;
      }
