...
try {
    ...
    $request->aspectFilter = 'nulla';
    $request->categoryIds = 'corrupti';
    $request->fieldgroups = 'illum';
    $request->gtin = 'vel';
    $request->limit = 'error';
    $request->mpn = 'deserunt';
    ...
    $response = $sdk->productSummary->search($request, $requestSecurity);

    if ($response->productSearchResponse !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
