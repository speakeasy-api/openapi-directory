// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"net/http"
	"openapi/pkg/models/shared"
)

type TwoSendPaymentsPublicPathParams struct {
	TransactionID string `pathParam:"style=simple,explode=false,name=transactionId"`
}

type TwoSendPaymentsPublicQueryParams struct {
	OrderID string `queryParam:"style=form,explode=true,name=orderId"`
}

type TwoSendPaymentsPublicHeaders struct {
	// Media type(s) that is/are acceptable for the response. Default value for payment provider protocol is application/json
	Accept string `header:"style=simple,explode=false,name=Accept"`
	// The Media type of the body of the request.  Default value for payment provider protocol is application/json
	ContentType string `header:"style=simple,explode=false,name=Content-Type"`
	// The AppKey configured by the merchant (optional configuration)
	XPROVIDERAPIAppKey string `header:"style=simple,explode=false,name=X-PROVIDER-API-AppKey"`
	// The AppToken configured by the merchant (optional configuration)
	XPROVIDERAPIAppToken string `header:"style=simple,explode=false,name=X-PROVIDER-API-AppToken"`
}

type TwoSendPaymentsPublicRequest struct {
	PathParams  TwoSendPaymentsPublicPathParams
	QueryParams TwoSendPaymentsPublicQueryParams
	Headers     TwoSendPaymentsPublicHeaders
	Request     []shared.TwoSendPaymentsPublicRequest `request:"mediaType=application/json"`
}

type TwoSendPaymentsPublicResponse struct {
	ContentType string
	StatusCode  int
	RawResponse *http.Response
}
