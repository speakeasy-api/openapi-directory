...
export class Public {
   ...
   */
  getpublicdata(
    req: operations.GetpublicdataRequest,
    security: operations.GetpublicdataSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.GetpublicdataResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetpublicdataRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/getpublicdata";
    if (!(security instanceof utils.SpeakeasyBase)) {
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GetpublicdataResponse =
        new operations.GetpublicdataResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.naPublicDataResponse = utils.objectToClass(
              httpRes?.data,
              shared.NAPublicDataResponse
            );
          }
          break;
      }
