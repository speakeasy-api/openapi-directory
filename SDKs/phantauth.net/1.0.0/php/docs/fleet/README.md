...
use \OpenAPI\OpenAPI\Models\Operations\GetFleetFleetnameRequest;

$sdk = SDK::builder()
    ->build();

try {
    $request = new GetFleetFleetnameRequest();
    $request->fleetname = 'quo';

    $response = $sdk->fleet->getFleetFleetname($request);

    if ($response->getFleetFleetname200ApplicationJSONObject !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
