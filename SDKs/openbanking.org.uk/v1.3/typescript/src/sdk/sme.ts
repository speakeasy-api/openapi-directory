  ...
  **/
  getUnsecuredSmeLoans(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetUnsecuredSmeLoansResponse = {statusCode: httpRes.status, contentType: contentType, headers: utils.getHeadersFromResponse(httpRes.headers)};
        switch (true) {
  ...
  **/
  headUnsecuredSmeLoans(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.HeadUnsecuredSmeLoansResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          default:
            if (utils.matchContentType(contentType, `application/prs.openbanking.opendata.v1.3+json`)) {
