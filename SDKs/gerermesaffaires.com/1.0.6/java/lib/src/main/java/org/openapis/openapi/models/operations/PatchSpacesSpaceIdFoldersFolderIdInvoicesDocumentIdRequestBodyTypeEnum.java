/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import com.fasterxml.jackson.annotation.JsonValue;

public enum PatchSpacesSpaceIdFoldersFolderIdInvoicesDocumentIdRequestBodyTypeEnum {
    COMMERCIAL_INVOICE("commercial-invoice"),
    CREDIT_NOTE("credit-note"),
    AMENDING_INVOICE("amending-invoice"),
    SELF_BILLING("self-billing"),
    CREDIT_SELF_BILLING("credit-self-billing"),
    DOWN_PAYMENT_INVOICE("down-payment-invoice"),
    INFORMATIONS_INVOICE("informations-invoice");

    @JsonValue
    public final String value;

    private PatchSpacesSpaceIdFoldersFolderIdInvoicesDocumentIdRequestBodyTypeEnum(String value) {
        this.value = value;
    }
}
