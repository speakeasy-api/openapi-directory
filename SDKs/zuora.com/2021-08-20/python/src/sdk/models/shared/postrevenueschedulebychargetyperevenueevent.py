...
class POSTRevenueScheduleByChargeTypeRevenueEventEventTypeEnum(str, Enum):
...
class POSTRevenueScheduleByChargeTypeRevenueEvent:
    ...
    event_type: POSTRevenueScheduleByChargeTypeRevenueEventEventTypeEnum = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('eventType') }})
    r"""Label of the revenue event type. Revenue event type labels can be duplicated. You can configure your revenue event type labels by navigating to **Settings > Finance > Configure Revenue Event Types** in the Zuora UI.
    
    Note that `Credit Memo Posted` and `Debit Memo Posted` are only available if you enable the Invoice Settlement feature.
    """  
    event_type_system_id: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('eventTypeSystemId') }})
