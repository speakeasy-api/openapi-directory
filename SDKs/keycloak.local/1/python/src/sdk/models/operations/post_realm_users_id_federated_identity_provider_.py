"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import federatedidentityrepresentation as shared_federatedidentityrepresentation
from typing import Optional


@dataclasses.dataclass
class PostRealmUsersIDFederatedIdentityProviderRequest:
    
    federated_identity_representation: shared_federatedidentityrepresentation.FederatedIdentityRepresentation = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})  
    id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'id', 'style': 'simple', 'explode': False }})
    r"""User id"""  
    provider: str = dataclasses.field(metadata={'path_param': { 'field_name': 'provider', 'style': 'simple', 'explode': False }})
    r"""Social login provider id"""  
    realm: str = dataclasses.field(metadata={'path_param': { 'field_name': 'realm', 'style': 'simple', 'explode': False }})
    r"""realm name (not id!)"""  
    

@dataclasses.dataclass
class PostRealmUsersIDFederatedIdentityProviderResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    