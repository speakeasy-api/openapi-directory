/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import com.fasterxml.jackson.annotation.JsonProperty;
import org.openapis.openapi.utils.SpeakeasyMetadata;

public class SuggestKeywordsRequest {
    /**
     * The required data to retrieve suggested keywords.
     */
    @SpeakeasyMetadata("request:mediaType=application/json")
    public org.openapis.openapi.models.shared.TargetedKeywordRequest targetedKeywordRequest;

    public SuggestKeywordsRequest withTargetedKeywordRequest(org.openapis.openapi.models.shared.TargetedKeywordRequest targetedKeywordRequest) {
        this.targetedKeywordRequest = targetedKeywordRequest;
        return this;
    }
    
    /**
     * The ID of the ad group for which the keyword suggestions will be provided.
     */
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=ad_group_id")
    public String adGroupId;

    public SuggestKeywordsRequest withAdGroupId(String adGroupId) {
        this.adGroupId = adGroupId;
        return this;
    }
    
    /**
     * A unique eBay-assigned ID for an ad campaign that is generated when a campaign is created.&lt;br /&gt;&lt;br /&gt;&lt;span class="tablenote"&gt;&lt;b&gt;Note:&lt;/b&gt; You can retrieve the campaign IDs for a specified seller using the &lt;a href="/api-docs/sell/marketing/resources/campaign/methods/getCampaigns"&gt;getCampaigns&lt;/a&gt; method.&lt;/span&gt;
     */
    @SpeakeasyMetadata("pathParam:style=simple,explode=false,name=campaign_id")
    public String campaignId;

    public SuggestKeywordsRequest withCampaignId(String campaignId) {
        this.campaignId = campaignId;
        return this;
    }
    
    public SuggestKeywordsRequest(@JsonProperty("ad_group_id") String adGroupId, @JsonProperty("campaign_id") String campaignId) {
        this.adGroupId = adGroupId;
        this.campaignId = campaignId;
  }
}
