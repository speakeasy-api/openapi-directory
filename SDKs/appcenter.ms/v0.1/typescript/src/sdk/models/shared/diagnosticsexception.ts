/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import { Expose, Type } from "class-transformer";

/**
 * programming language of the frame
 */
export enum DiagnosticsExceptionFramesLanguageEnum {
  JavaScript = "JavaScript",
  CSharp = "CSharp",
  ObjectiveC = "Objective-C",
  ObjectiveCpp = "Objective-Cpp",
  Cpp = "Cpp",
  C = "C",
  Swift = "Swift",
  Java = "Java",
  Unknown = "Unknown",
}

/**
 * a single frame of a stack trace
 */
export class DiagnosticsExceptionFrames extends SpeakeasyBase {
  /**
   * address of the frame
   */
  @SpeakeasyMetadata()
  @Expose({ name: "address" })
  address?: string;

  /**
   * this line isn't from any framework
   */
  @SpeakeasyMetadata()
  @Expose({ name: "app_code" })
  appCode: boolean;

  /**
   * is a class method
   */
  @SpeakeasyMetadata()
  @Expose({ name: "class_method" })
  classMethod?: boolean;

  /**
   * name of the class
   */
  @SpeakeasyMetadata()
  @Expose({ name: "class_name" })
  className?: string;

  /**
   * Formatted frame string
   */
  @SpeakeasyMetadata()
  @Expose({ name: "code_formatted" })
  codeFormatted: string;

  /**
   * Raw frame string
   */
  @SpeakeasyMetadata()
  @Expose({ name: "code_raw" })
  codeRaw: string;

  /**
   * name of the file
   */
  @SpeakeasyMetadata()
  @Expose({ name: "file" })
  file?: string;

  /**
   * Name of the framework
   */
  @SpeakeasyMetadata()
  @Expose({ name: "framework_name" })
  frameworkName?: string;

  /**
   * programming language of the frame
   */
  @SpeakeasyMetadata()
  @Expose({ name: "language" })
  language?: DiagnosticsExceptionFramesLanguageEnum;

  /**
   * line number
   */
  @SpeakeasyMetadata()
  @Expose({ name: "line" })
  line?: number;

  /**
   * name of the method
   */
  @SpeakeasyMetadata()
  @Expose({ name: "method" })
  method?: string;

  /**
   * parameters of the frames method
   */
  @SpeakeasyMetadata()
  @Expose({ name: "method_params" })
  methodParams?: string;

  /**
   * frame should be shown always
   */
  @SpeakeasyMetadata()
  @Expose({ name: "relevant" })
  relevant?: boolean;
}

/**
 * SDK/Platform this thread is beeing generated from
 */
export enum DiagnosticsExceptionPlatformEnum {
  Ios = "ios",
  Android = "android",
  Xamarin = "xamarin",
  ReactNative = "react-native",
  Ndk = "ndk",
  Unity = "unity",
  Other = "other",
}

/**
 * a exception
 */
export class DiagnosticsException extends SpeakeasyBase {
  /**
   * frames of the excetpion
   */
  @SpeakeasyMetadata({ elemType: DiagnosticsExceptionFrames })
  @Expose({ name: "frames" })
  @Type(() => DiagnosticsExceptionFrames)
  frames: DiagnosticsExceptionFrames[];

  @SpeakeasyMetadata({ elemType: DiagnosticsException })
  @Expose({ name: "inner_exceptions" })
  @Type(() => DiagnosticsException)
  innerExceptions?: DiagnosticsException[];

  /**
   * SDK/Platform this thread is beeing generated from
   */
  @SpeakeasyMetadata()
  @Expose({ name: "platform" })
  platform?: DiagnosticsExceptionPlatformEnum;

  /**
   * Reason of the exception
   */
  @SpeakeasyMetadata()
  @Expose({ name: "reason" })
  reason?: string;

  /**
   * relevant exception (crashed)
   */
  @SpeakeasyMetadata()
  @Expose({ name: "relevant" })
  relevant?: boolean;

  /**
   * Type of the exception (NSSomethingException, NullPointerException)
   */
  @SpeakeasyMetadata()
  @Expose({ name: "type" })
  type?: string;
}
