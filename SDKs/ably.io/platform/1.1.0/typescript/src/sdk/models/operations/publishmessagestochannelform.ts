/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import * as shared from "../shared";
import { AxiosResponse } from "axios";
import { Expose } from "class-transformer";

export class PublishMessagesToChannelFormRequest extends SpeakeasyBase {
  @SpeakeasyMetadata({
    data: "request, media_type=application/x-www-form-urlencoded",
  })
  messageInput?: shared.MessageInput;

  /**
   * The version of the API you wish to use.
   */
  @SpeakeasyMetadata({
    data: "header, style=simple;explode=false;name=X-Ably-Version",
  })
  xAblyVersion?: string;

  /**
   * The [Channel's ID](https://www.ably.io/documentation/rest/channels).
   */
  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=channel_id",
  })
  channelId: string;

  /**
   * The response format you would like
   */
  @SpeakeasyMetadata({
    data: "queryParam, style=form;explode=true;name=format",
  })
  format?: shared.ResponseFormatEnum;
}

/**
 * OK
 */
export class PublishMessagesToChannelForm2XXApplicationXMsgpack extends SpeakeasyBase {
  @SpeakeasyMetadata()
  channel?: string;

  @SpeakeasyMetadata()
  messageId?: string;
}

/**
 * OK
 */
export class PublishMessagesToChannelForm2XXApplicationJSON extends SpeakeasyBase {
  @SpeakeasyMetadata()
  @Expose({ name: "channel" })
  channel?: string;

  @SpeakeasyMetadata()
  @Expose({ name: "messageId" })
  messageId?: string;
}

export class PublishMessagesToChannelFormResponse extends SpeakeasyBase {
  @SpeakeasyMetadata()
  body?: Uint8Array;

  @SpeakeasyMetadata()
  contentType: string;

  /**
   * Error
   */
  @SpeakeasyMetadata()
  error?: shared.ErrorT;

  @SpeakeasyMetadata()
  statusCode: number;

  @SpeakeasyMetadata()
  rawResponse?: AxiosResponse;

  /**
   * OK
   */
  @SpeakeasyMetadata()
  publishMessagesToChannelForm2XXApplicationJSONObject?: PublishMessagesToChannelForm2XXApplicationJSON;
}
