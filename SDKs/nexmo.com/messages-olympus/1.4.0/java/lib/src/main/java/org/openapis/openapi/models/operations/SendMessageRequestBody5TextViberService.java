/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;

public class SendMessageRequestBody5TextViberService {
    /**
     * Node for Viber action buttons.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("action")
    public SendMessageRequestBody5TextViberServiceAction action;

    public SendMessageRequestBody5TextViberService withAction(SendMessageRequestBody5TextViberServiceAction action) {
        this.action = action;
        return this;
    }
    
    /**
     * The use of different category tags enables the business to send messages for different use cases. For Viber Business Messages the first message sent from a business to a user must be personal, informative &amp; a targeted message - not promotional. By default Vonage sends the `transaction` category to Viber Business Messages.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("category")
    public SendMessageRequestBody5TextViberServiceCategoryEnum category;

    public SendMessageRequestBody5TextViberService withCategory(SendMessageRequestBody5TextViberServiceCategoryEnum category) {
        this.category = category;
        return this;
    }
    
    /**
     * Set the time-to-live of message to be delivered in seconds. i.e. if the message is not delivered in 600 seconds then delete the message.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("ttl")
    public Long ttl;

    public SendMessageRequestBody5TextViberService withTtl(Long ttl) {
        this.ttl = ttl;
        return this;
    }
    
    /**
     * Viber-specific type definition. To use "template", please contact your Vonage Account Manager to setup your templates. To find out more please visit the [product page](https://www.vonage.com/communications-apis/messages/)
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("type")
    public String type;

    public SendMessageRequestBody5TextViberService withType(String type) {
        this.type = type;
        return this;
    }
    
    public SendMessageRequestBody5TextViberService(){}
}
