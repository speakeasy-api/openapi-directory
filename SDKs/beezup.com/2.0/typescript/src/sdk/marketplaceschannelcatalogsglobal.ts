...
export class MarketplacesChannelCatalogsGlobal {
  ...
  getMarketplaceChannelCatalogs(
    ...
    const url: string =
      baseURL.replace(/\/$/, "") + "/v2/user/marketplaces/channelcatalogs/";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetMarketplaceChannelCatalogsResponse =
        new operations.GetMarketplaceChannelCatalogsResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.marketplaceChannelCatalogList = utils.objectToClass(
              httpRes?.data,
              shared.MarketplaceChannelCatalogList
            );
          }
          break;
