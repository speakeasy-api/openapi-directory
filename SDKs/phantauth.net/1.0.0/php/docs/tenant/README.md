...
use \OpenAPI\OpenAPI\Models\Operations\GetTenantTenantnameRequest;

$sdk = SDK::builder()
    ->build();

try {
    $request = new GetTenantTenantnameRequest();
    $request->tenantname = 'at';

    $response = $sdk->tenant->getTenantTenantname($request);

    if ($response->getTenantTenantname200ApplicationJSONObject !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
