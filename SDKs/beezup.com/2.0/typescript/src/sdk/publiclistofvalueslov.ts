...
export class PublicListOfValuesLOV {
  ...
  getPublicListOfValues(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetPublicListOfValuesResponse =
        new operations.GetPublicListOfValuesResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.publicListOfValuesResponse = utils.objectToClass(
              httpRes?.data,
              shared.PublicListOfValuesResponse
            );
          }
          break;
  ...
  getPublicLovIndex(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GetPublicLovIndexResponse =
        new operations.GetPublicLovIndexResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
          headers: utils.getHeadersFromResponse(httpRes.headers),
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.publicLovIndex = utils.objectToClass(
              httpRes?.data,
              shared.PublicLovIndex
            );
          }
          break;
        case httpRes?.status == 304:
