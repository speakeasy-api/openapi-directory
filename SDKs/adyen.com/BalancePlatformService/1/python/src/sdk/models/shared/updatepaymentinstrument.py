...
class UpdatePaymentInstrumentStatusEnum(str, Enum):
...
class UpdatePaymentInstrumentTypeEnum(str, Enum):
...
class UpdatePaymentInstrument:
    ...
    r"""The unique identifier of the payment instrument."""  
    ...
    type: UpdatePaymentInstrumentTypeEnum = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('type') }})
    r"""Type of payment instrument.
    ...
    status: Optional[UpdatePaymentInstrumentStatusEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('status'), 'exclude': lambda f: f is None }})
    r"""The status of the payment instrument. If a status is not specified when creating a payment instrument, it is set to **Active** by default. However, there can be exceptions for cards based on the `card.formFactor` and the `issuingCountryCode`. For example, when issuing physical cards in the US, the default status is **Requested**.
