...
export class Quality {
  ...
  qualityGetDailyDataQualityForSite(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.QualityGetDailyDataQualityForSiteResponse =
        new operations.QualityGetDailyDataQualityForSiteResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.dailyQualityResponse = utils.objectToClass(
              httpRes?.data,
              shared.DailyQualityResponse
            );
          }
          break;
  ...
  qualityGetOverallDataQualityForSites(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.QualityGetOverallDataQualityForSitesResponse =
        new operations.QualityGetOverallDataQualityForSitesResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.overallQualityResponse = utils.objectToClass(
              httpRes?.data,
              shared.OverallQualityResponse
            );
          }
          break;
        case [400, 500].includes(httpRes?.status):
