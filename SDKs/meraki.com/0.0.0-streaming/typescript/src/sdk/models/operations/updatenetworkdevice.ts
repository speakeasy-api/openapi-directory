/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import { AxiosResponse } from "axios";
import { Expose } from "class-transformer";

export class UpdateNetworkDeviceRequestBody extends SpeakeasyBase {
  /**
   * The address of a device
   */
  @SpeakeasyMetadata()
  @Expose({ name: "address" })
  address?: string;

  /**
   * The floor plan to associate to this device. null disassociates the device from the floorplan.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "floorPlanId" })
  floorPlanId?: string;

  /**
   * The latitude of a device
   */
  @SpeakeasyMetadata()
  @Expose({ name: "lat" })
  lat?: number;

  /**
   * The longitude of a device
   */
  @SpeakeasyMetadata()
  @Expose({ name: "lng" })
  lng?: number;

  /**
   * Whether or not to set the latitude and longitude of a device based on the new address. Only applies when lat and lng are not specified.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "moveMapMarker" })
  moveMapMarker?: boolean;

  /**
   * The name of a device
   */
  @SpeakeasyMetadata()
  @Expose({ name: "name" })
  name?: string;

  /**
   * The notes for the device. String. Limited to 255 characters.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "notes" })
  notes?: string;

  /**
   * The ID of a switch profile to bind to the device (for available switch profiles, see the 'Switch Profiles' endpoint). Use null to unbind the switch device from the current profile. For a device to be bindable to a switch profile, it must (1) be a switch, and (2) belong to a network that is bound to a configuration template.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "switchProfileId" })
  switchProfileId?: string;

  /**
   * The tags of a device
   */
  @SpeakeasyMetadata()
  @Expose({ name: "tags" })
  tags?: string;
}

export class UpdateNetworkDeviceRequest extends SpeakeasyBase {
  @SpeakeasyMetadata({ data: "request, media_type=application/json" })
  requestBody?: UpdateNetworkDeviceRequestBody;

  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=networkId",
  })
  networkId: string;

  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=serial",
  })
  serial: string;
}

export class UpdateNetworkDeviceResponse extends SpeakeasyBase {
  @SpeakeasyMetadata()
  contentType: string;

  @SpeakeasyMetadata()
  statusCode: number;

  @SpeakeasyMetadata()
  rawResponse?: AxiosResponse;

  /**
   * Successful operation
   */
  @SpeakeasyMetadata()
  updateNetworkDevice200ApplicationJSONObject?: Record<string, any>;
}
