...
class ListAuditFindingsRequestBody:
    ...
    r"""A filter to limit results to those found before the specified time. You must specify either the startTime and endTime or the taskId, but not both."""  
    list_suppressed_findings: Optional[bool] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('listSuppressedFindings'), 'exclude': lambda f: f is None }})
    r"""Boolean flag indicating whether only the suppressed findings or the unsuppressed findings should be listed. If this parameter isn't provided, the response will list both suppressed and unsuppressed findings."""  
    ...
    resource_identifier: Optional[ListAuditFindingsRequestBodyResourceIdentifier] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('resourceIdentifier'), 'exclude': lambda f: f is None }})
    r"""Information that identifies the noncompliant resource."""  
    ...
    r"""A filter to limit results to the audit with the specified ID. You must specify either the taskId or the startTime and endTime, but not both."""  
    

@dataclasses.dataclass
class ListAuditFindingsRequest:
    
    request_body: ListAuditFindingsRequestBody = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})  
