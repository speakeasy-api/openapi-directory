/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.shared;

import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * BareMetalAdminDrainingMachine - BareMetalAdminDrainingMachine represents the machines that are currently draining.
 */
public class BareMetalAdminDrainingMachine {
    /**
     * Draining machine IP address.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("nodeIp")
    public String nodeIp;

    public BareMetalAdminDrainingMachine withNodeIp(String nodeIp) {
        this.nodeIp = nodeIp;
        return this;
    }
    
    /**
     * The count of pods yet to drain.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("podCount")
    public Integer podCount;

    public BareMetalAdminDrainingMachine withPodCount(Integer podCount) {
        this.podCount = podCount;
        return this;
    }
    
    public BareMetalAdminDrainingMachine(){}
}
