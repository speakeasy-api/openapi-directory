...
export class NodeGroups {
  ...
  computeNodeGroupsPerformMaintenance(
    req: operations.ComputeNodeGroupsPerformMaintenanceRequest,
    security: operations.ComputeNodeGroupsPerformMaintenanceSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.ComputeNodeGroupsPerformMaintenanceResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
    ...
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.ComputeNodeGroupsPerformMaintenanceSecurity(
        security
      );
    }
    const client: AxiosInstance = utils.createSecurityClient(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.ComputeNodeGroupsPerformMaintenanceResponse =
        new operations.ComputeNodeGroupsPerformMaintenanceResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
