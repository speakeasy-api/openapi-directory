"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import authenticatorconfigrepresentation as shared_authenticatorconfigrepresentation
from typing import Optional


@dataclasses.dataclass
class GetRealmAuthenticationConfigIDRequest:
    
    id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'id', 'style': 'simple', 'explode': False }})
    r"""Configuration id"""  
    realm: str = dataclasses.field(metadata={'path_param': { 'field_name': 'realm', 'style': 'simple', 'explode': False }})
    r"""realm name (not id!)"""  
    

@dataclasses.dataclass
class GetRealmAuthenticationConfigIDResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    authenticator_config_representation: Optional[shared_authenticatorconfigrepresentation.AuthenticatorConfigRepresentation] = dataclasses.field(default=None)
    r"""success"""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    