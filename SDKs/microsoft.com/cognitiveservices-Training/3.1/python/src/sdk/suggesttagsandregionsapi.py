...
class SuggestTagsAndRegionsAPI:
    ...
    def suggest_tags_and_regions(self, request: operations.SuggestTagsAndRegionsRequest) -> operations.SuggestTagsAndRegionsResponse:
        ...
        url = utils.generate_url(base_url, "/projects/{projectId}/tagsandregions/suggestions", request.path_params)
        ...
        res = operations.SuggestTagsAndRegionsResponse(status_code=r.status_code, content_type=content_type)
        
        if r.status_code == 200:
            if utils.match_content_type(content_type, "application/json"):
                out = utils.unmarshal_json(r.text, Optional[list[shared.SuggestedTagAndRegion]])
