"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
from dataclasses_json import Undefined, dataclass_json
from enum import Enum
from sdk import utils
from typing import Optional

class TagTypeEnum(str, Enum):
    r"""Gets or sets the type of the tag."""
    REGULAR = 'Regular'
    NEGATIVE = 'Negative'


@dataclass_json(undefined=Undefined.EXCLUDE)
@dataclasses.dataclass
class TagInput:
    r"""Represents a Tag."""
    
    description: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('description') }, 'form': { 'field_name': 'description' }})
    r"""Gets or sets the description of the tag."""  
    name: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('name') }, 'form': { 'field_name': 'name' }})
    r"""Gets or sets the name of the tag."""  
    type: TagTypeEnum = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('type') }, 'form': { 'field_name': 'type' }})
    r"""Gets or sets the type of the tag."""  
    

@dataclass_json(undefined=Undefined.EXCLUDE)
@dataclasses.dataclass
class Tag:
    r"""Represents a Tag."""
    
    description: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('description') }})
    r"""Gets or sets the description of the tag."""  
    name: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('name') }})
    r"""Gets or sets the name of the tag."""  
    type: TagTypeEnum = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('type') }})
    r"""Gets or sets the type of the tag."""  
    id: Optional[str] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('id'), 'exclude': lambda f: f is None }})
    r"""Gets the Tag ID."""  
    image_count: Optional[int] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('imageCount'), 'exclude': lambda f: f is None }})
    r"""Gets the number of images with this tag."""  
    