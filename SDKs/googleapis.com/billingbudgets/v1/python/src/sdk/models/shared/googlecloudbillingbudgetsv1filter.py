...
class GoogleCloudBillingBudgetsV1FilterCalendarPeriodEnum(str, Enum):
    r"""Optional. Specifies to track usage for recurring calendar period. For example, assume that CalendarPeriod.QUARTER is set. The budget tracks usage from April 1 to June 30, when the current calendar month is April, May, June. After that, it tracks usage from July 1 to September 30 when the current calendar month is July, August, September, so on."""
...
class GoogleCloudBillingBudgetsV1Filter:
    ...
    calendar_period: Optional[GoogleCloudBillingBudgetsV1FilterCalendarPeriodEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('calendarPeriod'), 'exclude': lambda f: f is None }})
    r"""Optional. Specifies to track usage for recurring calendar period. For example, assume that CalendarPeriod.QUARTER is set. The budget tracks usage from April 1 to June 30, when the current calendar month is April, May, June. After that, it tracks usage from July 1 to September 30 when the current calendar month is July, August, September, so on."""  
    ...
    credit_types_treatment: Optional[GoogleCloudBillingBudgetsV1FilterCreditTypesTreatmentEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('creditTypesTreatment'), 'exclude': lambda f: f is None }})
    r"""Optional. If not set, default behavior is `INCLUDE_ALL_CREDITS`."""  
    custom_period: Optional[shared_googlecloudbillingbudgetsv1customperiod.GoogleCloudBillingBudgetsV1CustomPeriod] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('customPeriod'), 'exclude': lambda f: f is None }})
    ...
    r"""Optional. A set of projects of the form `projects/{project}`, specifying that usage from only this set of projects should be included in the budget. If omitted, the report includes all usage for the billing account, regardless of which project the usage occurred on."""  
