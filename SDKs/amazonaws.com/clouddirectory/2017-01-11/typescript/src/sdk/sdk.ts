...
export class SDK {
  ...
  **/
  listManagedSchemaArns(
    req: operations.ListManagedSchemaArnsRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.ListManagedSchemaArnsResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.ListManagedSchemaArnsRequest(req);
    }
    
    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/amazonclouddirectory/2017-01-11/schema/managed";
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ListManagedSchemaArnsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.listManagedSchemaArnsResponse = httpRes?.data;
            }
            break;
          case httpRes?.status == 480:
            if (utils.matchContentType(contentType, `application/json`)) {
