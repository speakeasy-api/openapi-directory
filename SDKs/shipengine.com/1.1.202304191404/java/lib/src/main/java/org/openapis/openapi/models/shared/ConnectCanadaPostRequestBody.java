/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.shared;

import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * ConnectCanadaPostRequestBody - A Canada Post account information request body
 */
public class ConnectCanadaPostRequestBody {
    /**
     * Canada Post Account Number
     */
    @JsonProperty("account_number")
    public String accountNumber;

    public ConnectCanadaPostRequestBody withAccountNumber(String accountNumber) {
        this.accountNumber = accountNumber;
        return this;
    }
    
    /**
     * Canada Post Account API Key
     */
    @JsonProperty("api_key")
    public String apiKey;

    public ConnectCanadaPostRequestBody withApiKey(String apiKey) {
        this.apiKey = apiKey;
        return this;
    }
    
    /**
     * Canada Post Account API Secret
     */
    @JsonProperty("api_secret")
    public String apiSecret;

    public ConnectCanadaPostRequestBody withApiSecret(String apiSecret) {
        this.apiSecret = apiSecret;
        return this;
    }
    
    /**
     * Canada Post Account Contract ID
     */
    @JsonProperty("contract_id")
    public String contractId;

    public ConnectCanadaPostRequestBody withContractId(String contractId) {
        this.contractId = contractId;
        return this;
    }
    
    /**
     * Nickname
     */
    @JsonProperty("nickname")
    public String nickname;

    public ConnectCanadaPostRequestBody withNickname(String nickname) {
        this.nickname = nickname;
        return this;
    }
    
    public ConnectCanadaPostRequestBody(@JsonProperty("account_number") String accountNumber, @JsonProperty("api_key") String apiKey, @JsonProperty("api_secret") String apiSecret, @JsonProperty("contract_id") String contractId, @JsonProperty("nickname") String nickname) {
        this.accountNumber = accountNumber;
        this.apiKey = apiKey;
        this.apiSecret = apiSecret;
        this.contractId = contractId;
        this.nickname = nickname;
  }
}
