...
export class SDK {
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      switch (true) {
        ...
        case httpRes?.status == 480:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.searchException = utils.objectToClass(
              httpRes?.data,
              shared.SearchException
            );
          }
          break;
      }
  ...
  suggest(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.SuggestResponse = new operations.SuggestResponse({
        statusCode: httpRes.status,
        contentType: contentType,
        rawResponse: httpRes,
      });
  ...
  uploadDocuments(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.UploadDocumentsResponse =
        new operations.UploadDocumentsResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.uploadDocumentsResponse = utils.objectToClass(
              httpRes?.data,
              shared.UploadDocumentsResponse
            );
          }
          break;
        case httpRes?.status == 480:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.documentServiceException = utils.objectToClass(
              httpRes?.data,
              shared.DocumentServiceException
            );
          }
          break;
      }
