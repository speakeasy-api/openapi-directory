...
export class Organization {
  ...
  getOrganizationData(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetOrganizationDataResponse =
        new operations.GetOrganizationDataResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.organizationData = utils.objectToClass(
              httpRes?.data,
              shared.OrganizationData
            );
          }
          break;
