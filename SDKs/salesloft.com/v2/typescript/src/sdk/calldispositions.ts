...
export class CallDispositions {
  ...
  getV2CallDispositionsJson(
    req: operations.GetV2CallDispositionsJsonRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.GetV2CallDispositionsJsonResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetV2CallDispositionsJsonRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string =
      baseURL.replace(/\/$/, "") + "/v2/call_dispositions.json";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetV2CallDispositionsJsonResponse =
        new operations.GetV2CallDispositionsJsonResponse({
