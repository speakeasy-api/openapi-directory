/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import * as shared from "../shared";
import { AxiosResponse } from "axios";

export class GetSpacesSpaceIdFoldersIdPayrollsRequest extends SpeakeasyBase {
  /**
   * begin date of the payrolls
   */
  @SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=Begin" })
  begin?: string;

  /**
   * range date of the documents
   */
  @SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=Date" })
  date?: string;

  /**
   * end date of the payrolls
   */
  @SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=End" })
  end?: string;

  /**
   * index range of the results
   */
  @SpeakeasyMetadata({ data: "queryParam, style=form;explode=true;name=Range" })
  range?: string;

  /**
   * Id of the folder social
   */
  @SpeakeasyMetadata({ data: "pathParam, style=simple;explode=false;name=id" })
  id: string;

  /**
   * Id of the space
   */
  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=spaceId",
  })
  spaceId: string;
}

export class GetSpacesSpaceIdFoldersIdPayrollsResponse extends SpeakeasyBase {
  @SpeakeasyMetadata()
  contentType: string;

  @SpeakeasyMetadata()
  statusCode: number;

  @SpeakeasyMetadata()
  rawResponse?: AxiosResponse;

  /**
   * A list of payrolls of the folder social
   */
  @SpeakeasyMetadata({ elemType: shared.Payroll })
  getSpacesSpaceIdFoldersIdPayrolls200ApplicationJSONAllOfs?: shared.Payroll[];
}
