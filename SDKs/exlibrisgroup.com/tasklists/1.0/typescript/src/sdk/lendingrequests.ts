...
export class LendingRequests {
  ...
  getAlmawsV1TaskListsRsLendingRequests(
    ...
    const url: string =
      baseURL.replace(/\/$/, "") + "/almaws/v1/task-lists/rs/lending-requests";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetAlmawsV1TaskListsRsLendingRequestsResponse =
        new operations.GetAlmawsV1TaskListsRsLendingRequestsResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getAlmawsV1TaskListsRsLendingRequests200ApplicationJSONObject =
              utils.objectToClass(
                httpRes?.data,
                operations.GetAlmawsV1TaskListsRsLendingRequests200ApplicationJSON
              );
          }
  ...
  postAlmawsV1TaskListsRsLendingRequests(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.PostAlmawsV1TaskListsRsLendingRequestsResponse =
        new operations.PostAlmawsV1TaskListsRsLendingRequestsResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
          headers: utils.getHeadersFromResponse(httpRes.headers),
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.onealmaws1v11taskLists1rs1lendingRequestsGetResponses200ContentApplication1jsonSchemaPropertiesUserResourceSharingRequestItems =
              utils.objectToClass(
                httpRes?.data,
                shared.Onealmaws1v11taskLists1rs1lendingRequestsGetResponses200ContentApplication1jsonSchemaPropertiesUserResourceSharingRequestItems
              );
          }
          if (utils.matchContentType(contentType, `application/xml`)) {
            const resBody: string = JSON.stringify(httpRes?.data, null, 0);
