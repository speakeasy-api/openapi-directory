"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import notfound as shared_notfound
from ..shared import variant as shared_variant
from ..shared import variantedit as shared_variantedit
from typing import Optional


@dataclasses.dataclass
class PutProductsIDVariantsVariantIDJSONRequest:
    
    authtoken: str = dataclasses.field(metadata={'query_param': { 'field_name': 'authtoken', 'style': 'form', 'explode': True }})
    r"""API OAuth token."""  
    id: int = dataclasses.field(metadata={'path_param': { 'field_name': 'id', 'style': 'simple', 'explode': False }})
    r"""Id of the Product"""  
    login: str = dataclasses.field(metadata={'query_param': { 'field_name': 'login', 'style': 'form', 'explode': True }})
    r"""API OAuth login."""  
    variant_id: int = dataclasses.field(metadata={'path_param': { 'field_name': 'variant_id', 'style': 'simple', 'explode': False }})
    r"""Id of the Product Variant"""  
    variant_edit: shared_variantedit.VariantEdit = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})
    r"""Product Variant parameters to change"""  
    

@dataclasses.dataclass
class PutProductsIDVariantsVariantIDJSONResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    not_found: Optional[shared_notfound.NotFound] = dataclasses.field(default=None)
    r"""Product Not Found."""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    variant: Optional[shared_variant.Variant] = dataclasses.field(default=None)
    r"""OK"""  
    