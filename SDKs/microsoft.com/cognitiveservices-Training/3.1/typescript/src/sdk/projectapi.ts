...
export class ProjectApi {
  ...
  **/
  publishIteration(
    req: operations.PublishIterationRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.PublishIterationResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.PublishIterationRequest(req);
    }
    
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/projects/{projectId}/iterations/{iterationId}/publish", req.pathParams);
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PublishIterationResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.publishIteration200ApplicationJSONBoolean = httpRes?.data;
            }
            if (utils.matchContentType(contentType, `application/xml`)) {
                const resBody: string = JSON.stringify(httpRes?.data, null, 0);
                let out: Uint8Array = new Uint8Array(resBody.length);
  ...
  **/
  unpublishIteration(
    req: operations.UnpublishIterationRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.UnpublishIterationResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.UnpublishIterationResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 204:
            break;
          default:
