"use strict";
/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OperatorConfiguration = void 0;
var utils_1 = require("../../../internal/utils");
var operatordbt_1 = require("./operatordbt");
var operatornormalization_1 = require("./operatornormalization");
var operatortypeenum_1 = require("./operatortypeenum");
var operatorwebhook_1 = require("./operatorwebhook");
var class_transformer_1 = require("class-transformer");
var OperatorConfiguration = /** @class */ (function (_super) {
    __extends(OperatorConfiguration, _super);
    function OperatorConfiguration() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "dbt" }),
        (0, class_transformer_1.Type)(function () { return operatordbt_1.OperatorDbt; }),
        __metadata("design:type", operatordbt_1.OperatorDbt)
    ], OperatorConfiguration.prototype, "dbt", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "normalization" }),
        (0, class_transformer_1.Type)(function () { return operatornormalization_1.OperatorNormalization; }),
        __metadata("design:type", operatornormalization_1.OperatorNormalization)
    ], OperatorConfiguration.prototype, "normalization", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "operatorType" }),
        __metadata("design:type", String)
    ], OperatorConfiguration.prototype, "operatorType", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "webhook" }),
        (0, class_transformer_1.Type)(function () { return operatorwebhook_1.OperatorWebhook; }),
        __metadata("design:type", operatorwebhook_1.OperatorWebhook)
    ], OperatorConfiguration.prototype, "webhook", void 0);
    return OperatorConfiguration;
}(utils_1.SpeakeasyBase));
exports.OperatorConfiguration = OperatorConfiguration;
