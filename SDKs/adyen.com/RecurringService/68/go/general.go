...
func (s *general) PostDisablePermit(ctx context.Context, request shared.DisablePermitRequest, security operations.PostDisablePermitSecurity) (*operations.PostDisablePermitResponse, error) {
	baseURL := s.serverURL
	url := strings.TrimSuffix(baseURL, "/") + "/disablePermit"
	...
	res := &operations.PostDisablePermitResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: contentType,
		RawResponse: httpRes,
	}
	...
	case httpRes.StatusCode == 200:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.DisablePermitResult
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}
