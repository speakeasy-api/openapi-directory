<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;


class GetItemsBatchInternalTrasnferBatchItemsBatchItem
{
    /**
     * The amount of funds to send. In cent or pence
     * 
     * @var ?int $amount
     */
	#[\JMS\Serializer\Annotation\SerializedName('amount')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $amount = null;
    
    /**
     * The amount of the transfer after fees and taxes. in pence or cent.
     * 
     * @var ?int $amountAfterCharges
     */
	#[\JMS\Serializer\Annotation\SerializedName('amountAfterCharges')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $amountAfterCharges = null;
    
    /**
     * A UUID for this item.
     * 
     * @var ?string $batchItemUuid
     */
	#[\JMS\Serializer\Annotation\SerializedName('batchItemUuid')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $batchItemUuid = null;
    
    /**
     * The datestamp the batch was created - ISO format - e.g. 2018-04-04T00:53:21.910Z
     * 
     * @var ?\DateTime $dateCreated
     */
	#[\JMS\Serializer\Annotation\SerializedName('dateCreated')]
    #[\JMS\Serializer\Annotation\Type("DateTime<'Y-m-d\TH:i:s.up'>")]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?\DateTime $dateCreated = null;
    
    /**
     * The fee charged by fire.com for the payment. In pence or cent.
     * 
     * @var ?int $feeAmount
     */
	#[\JMS\Serializer\Annotation\SerializedName('feeAmount')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $feeAmount = null;
    
    /**
     * The Fire account ID of the source account.
     * 
     * @var ?int $icanFrom
     */
	#[\JMS\Serializer\Annotation\SerializedName('icanFrom')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $icanFrom = null;
    
    /**
     * The Fire account ID for the fire.com account the funds are sent to.
     * 
     * @var ?int $icanTo
     */
	#[\JMS\Serializer\Annotation\SerializedName('icanTo')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $icanTo = null;
    
    /**
     * The datestamp of the last action on this batch - ISO format - e.g. 2018-04-04T10:48:53.540Z
     * 
     * @var ?\DateTime $lastUpdated
     */
	#[\JMS\Serializer\Annotation\SerializedName('lastUpdated')]
    #[\JMS\Serializer\Annotation\Type("DateTime<'Y-m-d\TH:i:s.up'>")]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?\DateTime $lastUpdated = null;
    
    /**
     * The reference on the transaction.
     * 
     * @var ?string $ref
     */
	#[\JMS\Serializer\Annotation\SerializedName('ref')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $ref = null;
    
    /**
     * The ID of the resulting payment in your account. Can be used to retrieve the transaction using the https://api.fire.com/business/v1/accounts/{accountId}/transactions/{refId} endpoint.
     * 
     * @var ?int $refId
     */
	#[\JMS\Serializer\Annotation\SerializedName('refId')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $refId = null;
    
    /**
     * The outcome of the attempted transaction.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\GetItemsBatchInternalTrasnferBatchItemsBatchItemResult $result
     */
	#[\JMS\Serializer\Annotation\SerializedName('result')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Operations\GetItemsBatchInternalTrasnferBatchItemsBatchItemResult')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?GetItemsBatchInternalTrasnferBatchItemsBatchItemResult $result = null;
    
    /**
     * status of the batch if internal trasnfer
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\GetItemsBatchInternalTrasnferBatchItemsBatchItemStatusEnum $status
     */
	#[\JMS\Serializer\Annotation\SerializedName('status')]
    #[\JMS\Serializer\Annotation\Type('enum<OpenAPI\OpenAPI\Models\Operations\GetItemsBatchInternalTrasnferBatchItemsBatchItemStatusEnum>')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?GetItemsBatchInternalTrasnferBatchItemsBatchItemStatusEnum $status = null;
    
    /**
     * Any taxes/duty collected by fire.com for this payments (e.g. stamp duty etc). In pence or cent.
     * 
     * @var ?int $taxAmount
     */
	#[\JMS\Serializer\Annotation\SerializedName('taxAmount')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $taxAmount = null;
    
	public function __construct()
	{
		$this->amount = null;
		$this->amountAfterCharges = null;
		$this->batchItemUuid = null;
		$this->dateCreated = null;
		$this->feeAmount = null;
		$this->icanFrom = null;
		$this->icanTo = null;
		$this->lastUpdated = null;
		$this->ref = null;
		$this->refId = null;
		$this->result = null;
		$this->status = null;
		$this->taxAmount = null;
	}
}
