...
class DestinyMilestonesDestinyMilestoneChallengeActivity:
    ...
    boolean_activity_options: Optional[dict[str, bool]] = dataclasses.field(default=None)
    r"""The set of activity options for this activity, keyed by an identifier that's unique for this activity (not guaranteed to be unique between or across all activities, though should be unique for every *variant* of a given *conceptual* activity: for instance, the original D2 Raid has many variant DestinyActivityDefinitions. While other activities could potentially have the same option hashes, for any given D2 base Raid variant the hash will be unique).
    ...
    loadout_requirement_index: Optional[int] = dataclasses.field(default=None)
    r"""If returned, this is the index into the DestinyActivityDefinition's \\"loadouts\\" property, indicating the currently active loadout requirements."""  
    modifier_hashes: Optional[list[int]] = dataclasses.field(default=None)
    r"""If the activity has modifiers, this will be the list of modifiers that all variants have in common. Perform lookups against DestinyActivityModifierDefinition which defines the modifier being applied to get at the modifier data.
