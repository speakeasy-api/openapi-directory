/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import { AxiosResponse } from "axios";
import { Expose, Type } from "class-transformer";

export class UpdateNetworkStaticRouteRequestBodyReservedIpRanges extends SpeakeasyBase {
  /**
   * A text comment for the reserved range
   */
  @SpeakeasyMetadata()
  @Expose({ name: "comment" })
  comment: string;

  /**
   * The last IP in the reserved range
   */
  @SpeakeasyMetadata()
  @Expose({ name: "end" })
  end: string;

  /**
   * The first IP in the reserved range
   */
  @SpeakeasyMetadata()
  @Expose({ name: "start" })
  start: string;
}

export class UpdateNetworkStaticRouteRequestBody extends SpeakeasyBase {
  /**
   * The enabled state of the static route
   */
  @SpeakeasyMetadata()
  @Expose({ name: "enabled" })
  enabled?: boolean;

  /**
   * The DHCP fixed IP assignments on the static route. This should be an object that contains mappings from MAC addresses to objects that themselves each contain "ip" and "name" string fields. See the sample request/response for more details.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "fixedIpAssignments" })
  fixedIpAssignments?: Record<string, any>;

  /**
   * The gateway IP (next hop) of the static route
   */
  @SpeakeasyMetadata()
  @Expose({ name: "gatewayIp" })
  gatewayIp?: string;

  /**
   * The name of the static route
   */
  @SpeakeasyMetadata()
  @Expose({ name: "name" })
  name?: string;

  /**
   * The DHCP reserved IP ranges on the static route
   */
  @SpeakeasyMetadata({
    elemType: UpdateNetworkStaticRouteRequestBodyReservedIpRanges,
  })
  @Expose({ name: "reservedIpRanges" })
  @Type(() => UpdateNetworkStaticRouteRequestBodyReservedIpRanges)
  reservedIpRanges?: UpdateNetworkStaticRouteRequestBodyReservedIpRanges[];

  /**
   * The subnet of the static route
   */
  @SpeakeasyMetadata()
  @Expose({ name: "subnet" })
  subnet?: string;
}

export class UpdateNetworkStaticRouteRequest extends SpeakeasyBase {
  @SpeakeasyMetadata({ data: "request, media_type=application/json" })
  requestBody?: UpdateNetworkStaticRouteRequestBody;

  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=networkId",
  })
  networkId: string;

  @SpeakeasyMetadata({
    data: "pathParam, style=simple;explode=false;name=staticRouteId",
  })
  staticRouteId: string;
}

export class UpdateNetworkStaticRouteResponse extends SpeakeasyBase {
  @SpeakeasyMetadata()
  contentType: string;

  @SpeakeasyMetadata()
  statusCode: number;

  @SpeakeasyMetadata()
  rawResponse?: AxiosResponse;

  /**
   * Successful operation
   */
  @SpeakeasyMetadata()
  updateNetworkStaticRoute200ApplicationJSONObject?: Record<string, any>;
}
