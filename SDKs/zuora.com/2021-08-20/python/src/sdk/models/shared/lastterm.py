...
class LastTerm:
    ...
    term_type: LastTermTermTypeEnum = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('termType') }})  
    ...
    r"""Specify only when the termType is 'TERMED'."""  
    period_type: Optional[LastTermPeriodTypeEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('periodType'), 'exclude': lambda f: f is None }})
    r"""Specify only when the termType is 'TERMED'."""  
    ...
    r"""The start date of the current term. You can change the term start date of a renewed subscription through a T&Cs order action. However, when changing it to an earlier date, this date must not be earlier than the term start date of the current term before this T&Cs."""  
