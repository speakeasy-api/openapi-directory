...
export class Webhooks {
  ...
  **/
  webhooksUnsubscribe(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/accounts/{accountId}/webhooks/{url}", req.pathParams);
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.WebhooksUnsubscribeResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
