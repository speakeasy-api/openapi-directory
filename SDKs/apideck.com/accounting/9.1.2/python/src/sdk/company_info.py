...
class CompanyInfo:
    ...
    def company_info_one(self, request: operations.CompanyInfoOneRequest, security: operations.CompanyInfoOneSecurity) -> operations.CompanyInfoOneResponse:
        r"""Get company info
        Get company info
        """
        base_url = self._server_url
        
        url = base_url.removesuffix('/') + '/accounting/company-info'
        
        headers = utils.get_headers(request)
        query_params = utils.get_query_params(operations.CompanyInfoOneRequest, request)
        ...
        res = operations.CompanyInfoOneResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.GetCompanyInfoResponse])
                res.get_company_info_response = out
