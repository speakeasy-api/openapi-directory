...
export class SecretScanning {
  ...
  secretScanningListAlertsForEnterprise(
    req: operations.SecretScanningListAlertsForEnterpriseRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.SecretScanningListAlertsForEnterpriseResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.SecretScanningListAlertsForEnterpriseResponse =
        new operations.SecretScanningListAlertsForEnterpriseResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        ...
        case httpRes?.status == 503:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.secretScanningListAlertsForEnterprise503ApplicationJSONObject =
              utils.objectToClass(
                httpRes?.data,
                operations.SecretScanningListAlertsForEnterprise503ApplicationJSON
              );
          }
          break;
      }
