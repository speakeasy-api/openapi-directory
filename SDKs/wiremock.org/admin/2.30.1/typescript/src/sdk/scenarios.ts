  ...
  **/
  getAdminScenarios(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetAdminScenariosResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  postAdminScenariosReset(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PostAdminScenariosResetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
