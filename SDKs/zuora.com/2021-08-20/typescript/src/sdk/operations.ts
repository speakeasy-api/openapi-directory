...
export class Operations {
  ...
  postBillingPreview(
    ...
    const url: string =
      baseURL.replace(/\/$/, "") + "/v1/operations/billing-preview";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.POSTBillingPreviewResponse =
        new operations.POSTBillingPreviewResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (
            ...
            res.billingPreviewResult = utils.objectToClass(
              httpRes?.data,
              shared.BillingPreviewResult
            );
          }
          break;
      }
  ...
  postTransactionInvoicePayment(
    ...
    const baseURL: string = this._serverURL;
    const url: string =
      baseURL.replace(/\/$/, "") + "/v1/operations/invoice-collect";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.POSTTransactionInvoicePaymentResponse =
        new operations.POSTTransactionInvoicePaymentResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
          headers: utils.getHeadersFromResponse(httpRes.headers),
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (
            ...
            res.postInvoiceCollectResponseType = utils.objectToClass(
              httpRes?.data,
              shared.POSTInvoiceCollectResponseType
            );
          }
          break;
      }
