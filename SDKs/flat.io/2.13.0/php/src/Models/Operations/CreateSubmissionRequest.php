<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class CreateSubmissionRequest
{
	#[SpeakeasyMetadata('request:mediaType=application/json')]
    public \OpenAPI\OpenAPI\Models\Shared\AssignmentSubmissionUpdate $assignmentSubmissionUpdate;
    
    /**
     * Unique identifier of the assignment
     * 
     * @var string $assignment
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=assignment')]
    public string $assignment;
    
    /**
     * Unique identifier of the class
     * 
     * @var string $class
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=class')]
    public string $class;
    
	public function __construct()
	{
		$this->assignmentSubmissionUpdate = new \OpenAPI\OpenAPI\Models\Shared\AssignmentSubmissionUpdate();
		$this->assignment = "";
		$this->class = "";
	}
}
