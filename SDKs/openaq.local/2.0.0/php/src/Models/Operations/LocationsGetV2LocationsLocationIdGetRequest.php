<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class LocationsGetV2LocationsLocationIdGetRequest
{
    /**
     * 
     *         Limit results by a certain city or cities.
     *         (ex. ?city=Chicago or ?city=Chicago&city=Boston)
     *         
     * 
     * @var ?array<string> $city
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=city')]
    public ?array $city = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=coordinates')]
    public ?string $coordinates = null;
    
    /**
     * 
     *         Limit results by a certain country using two letter country code.
     *         (ex. ?country=US or ?country=US&country=MX)
     *         
     * 
     * @var ?array<string> $country
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=country')]
    public ?array $country = null;
    
    /**
     * 
     *         Limit results by a certain country using two letter country code.
     *         (ex. /US)
     *         
     * 
     * @var ?string $countryId
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=country_id')]
    public ?string $countryId = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=dumpRaw')]
    public ?bool $dumpRaw = null;
    
    /**
     * Source entity type.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\EntityTypesEnum $entity
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=entity')]
    public ?\OpenAPI\OpenAPI\Models\Shared\EntityTypesEnum $entity = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=has_geo')]
    public ?bool $hasGeo = null;
    
    /**
     * Data is the product of a previous analysis/aggregation and not raw measurements
     * 
     * @var ?bool $isAnalysis
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=isAnalysis')]
    public ?bool $isAnalysis = null;
    
    /**
     * Location is mobile
     * 
     * @var ?bool $isMobile
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=isMobile')]
    public ?bool $isMobile = null;
    
    /**
     * Change the number of results returned.
     * 
     * @var ?int $limit
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=limit')]
    public ?int $limit = null;
    
    /**
     * $location
     * 
     * @var ?array<mixed> $location
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=location')]
    public ?array $location = null;
    
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=location_id')]
    public int $locationId;
    
    /**
     * Manufacturer of Sensor
     * 
     * @var ?array<string> $manufacturerName
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=manufacturerName')]
    public ?array $manufacturerName = null;
    
    /**
     * Model Name of Sensor
     * 
     * @var ?array<string> $modelName
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=modelName')]
    public ?array $modelName = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=offset')]
    public ?int $offset = null;
    
    /**
     * Order by a field
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\LocationsGetV2LocationsLocationIdGetOrderByLocationsOrderEnum $orderBy
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=order_by')]
    public ?LocationsGetV2LocationsLocationIdGetOrderByLocationsOrderEnum $orderBy = null;
    
    /**
     * Paginate through results.
     * 
     * @var ?int $page
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=page')]
    public ?int $page = null;
    
    /**
     * $parameter
     * 
     * @var ?array<mixed> $parameter
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=parameter')]
    public ?array $parameter = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=parameter_id')]
    public ?int $parameterId = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=radius')]
    public ?int $radius = null;
    
    /**
     * Type of Sensor
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\SensorTypesEnum $sensorType
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=sensorType')]
    public ?\OpenAPI\OpenAPI\Models\Shared\SensorTypesEnum $sensorType = null;
    
    /**
     * Sort Direction
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\LocationsGetV2LocationsLocationIdGetSortSortEnum $sort
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=sort')]
    public ?LocationsGetV2LocationsLocationIdGetSortSortEnum $sort = null;
    
    /**
     * Name of the data source
     * 
     * @var ?array<string> $sourceName
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=sourceName')]
    public ?array $sourceName = null;
    
    /**
     * $unit
     * 
     * @var ?array<string> $unit
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=unit')]
    public ?array $unit = null;
    
	public function __construct()
	{
		$this->city = null;
		$this->coordinates = null;
		$this->country = null;
		$this->countryId = null;
		$this->dumpRaw = null;
		$this->entity = null;
		$this->hasGeo = null;
		$this->isAnalysis = null;
		$this->isMobile = null;
		$this->limit = null;
		$this->location = null;
		$this->locationId = 0;
		$this->manufacturerName = null;
		$this->modelName = null;
		$this->offset = null;
		$this->orderBy = null;
		$this->page = null;
		$this->parameter = null;
		$this->parameterId = null;
		$this->radius = null;
		$this->sensorType = null;
		$this->sort = null;
		$this->sourceName = null;
		$this->unit = null;
	}
}
