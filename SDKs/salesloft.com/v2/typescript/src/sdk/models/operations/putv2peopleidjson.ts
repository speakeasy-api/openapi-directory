/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import { AxiosResponse } from "axios";

export class PutV2PeopleIdJsonRequestBody extends SpeakeasyBase {
  /**
   * ID of the Account to link this person to
   */
  @SpeakeasyMetadata({ data: "form, name=account_id" })
  accountId?: number;

  /**
   * City
   */
  @SpeakeasyMetadata({ data: "form, name=city" })
  city?: string;

  /**
   * Specific methods of communication to prevent for this person. This will prevent individual execution of these communication types as well as automatically skip cadence steps of this communication type for this person in SalesLoft. Values currently accepted: call, email, message
   */
  @SpeakeasyMetadata({ data: "form, name=contact_restrictions" })
  contactRestrictions?: string[];

  /**
   * Country
   */
  @SpeakeasyMetadata({ data: "form, name=country" })
  country?: string;

  /**
   * Requires Salesforce.
   *
   * @remarks
   *
   * ID of the person in your external CRM. You must provide a crm_id_type if this is included.
   *
   * Validations will be applied to the crm_id depending on the crm_id_type. A "salesforce" ID must be exactly 18 characters. A "salesforce" ID must be either a Lead (00Q) or Contact (003) object. The type will be validated using the 18 character ID.
   *
   * This field can only be used if your application or API key has the "person:set_crm_id" scope.
   *
   *
   */
  @SpeakeasyMetadata({ data: "form, name=crm_id" })
  crmId?: string;

  /**
   * The CRM that the provided crm_id is for. Must be one of: salesforce
   */
  @SpeakeasyMetadata({ data: "form, name=crm_id_type" })
  crmIdType?: string;

  /**
   * Custom fields are defined by the user's team. Only fields with values are presented in the API.
   */
  @SpeakeasyMetadata({ data: "form, name=custom_fields;json=true" })
  customFields?: Record<string, any>;

  /**
   * Whether or not this person has opted out of all communication. Setting this value to true prevents this person from being called, emailed, or added to a cadence in SalesLoft. If this person is currently in a cadence, they will be removed.
   */
  @SpeakeasyMetadata({ data: "form, name=do_not_contact" })
  doNotContact?: boolean;

  /**
   * Email address
   */
  @SpeakeasyMetadata({ data: "form, name=email_address" })
  emailAddress?: string;

  /**
   * First name
   */
  @SpeakeasyMetadata({ data: "form, name=first_name" })
  firstName?: string;

  /**
   * Home phone without formatting
   */
  @SpeakeasyMetadata({ data: "form, name=home_phone" })
  homePhone?: string;

  /**
   * ID of the Import this person is a part of. A person can be part of multiple imports, but this ID will always be the most recent Import
   */
  @SpeakeasyMetadata({ data: "form, name=import_id" })
  importId?: number;

  /**
   * The Job Seniority of a Person, must be one of director, executive, individual_contributor, manager, vice_president, unknown
   */
  @SpeakeasyMetadata({ data: "form, name=job_seniority" })
  jobSeniority?: string;

  /**
   * Last name
   */
  @SpeakeasyMetadata({ data: "form, name=last_name" })
  lastName?: string;

  /**
   * Linkedin URL
   */
  @SpeakeasyMetadata({ data: "form, name=linkedin_url" })
  linkedinUrl?: string;

  /**
   * Time locale of the person
   */
  @SpeakeasyMetadata({ data: "form, name=locale" })
  locale?: string;

  /**
   * Mobile phone without formatting
   */
  @SpeakeasyMetadata({ data: "form, name=mobile_phone" })
  mobilePhone?: string;

  /**
   * ID of the User that owns this person
   */
  @SpeakeasyMetadata({ data: "form, name=owner_id" })
  ownerId?: number;

  /**
   * Company industry. This property is specific to this person, unrelated to the company object. Updating the company object associated with this person is recommended
   */
  @SpeakeasyMetadata({ data: "form, name=person_company_industry" })
  personCompanyIndustry?: string;

  /**
   * Company name. This property is specific to this person, unrelated to the company object. Updating the company object associated with this person is recommended
   */
  @SpeakeasyMetadata({ data: "form, name=person_company_name" })
  personCompanyName?: string;

  /**
   * Company website. This property is specific to this person, unrelated to the company object. Updating the company object associated with this person is recommended
   */
  @SpeakeasyMetadata({ data: "form, name=person_company_website" })
  personCompanyWebsite?: string;

  /**
   * ID of the PersonStage of this person
   */
  @SpeakeasyMetadata({ data: "form, name=person_stage_id" })
  personStageId?: number;

  /**
   * Personal email address
   */
  @SpeakeasyMetadata({ data: "form, name=personal_email_address" })
  personalEmailAddress?: string;

  /**
   * The website of this person
   */
  @SpeakeasyMetadata({ data: "form, name=personal_website" })
  personalWebsite?: string;

  /**
   * Phone without formatting
   */
  @SpeakeasyMetadata({ data: "form, name=phone" })
  phone?: string;

  /**
   * Phone extension without formatting
   */
  @SpeakeasyMetadata({ data: "form, name=phone_extension" })
  phoneExtension?: string;

  /**
   * Alternate email address
   */
  @SpeakeasyMetadata({ data: "form, name=secondary_email_address" })
  secondaryEmailAddress?: string;

  /**
   * State
   */
  @SpeakeasyMetadata({ data: "form, name=state" })
  state?: string;

  /**
   * All tags applied to this person
   */
  @SpeakeasyMetadata({ data: "form, name=tags" })
  tags?: string[];

  /**
   * Job title
   */
  @SpeakeasyMetadata({ data: "form, name=title" })
  title?: string;

  /**
   * The twitter handle of this person
   */
  @SpeakeasyMetadata({ data: "form, name=twitter_handle" })
  twitterHandle?: string;

  /**
   * Work location - city
   */
  @SpeakeasyMetadata({ data: "form, name=work_city" })
  workCity?: string;

  /**
   * Work location - country
   */
  @SpeakeasyMetadata({ data: "form, name=work_country" })
  workCountry?: string;

  /**
   * Work location - state
   */
  @SpeakeasyMetadata({ data: "form, name=work_state" })
  workState?: string;
}

export class PutV2PeopleIdJsonRequest extends SpeakeasyBase {
  @SpeakeasyMetadata({
    data: "request, media_type=application/x-www-form-urlencoded",
  })
  requestBody?: PutV2PeopleIdJsonRequestBody;

  /**
   * Person id
   */
  @SpeakeasyMetadata({ data: "pathParam, style=simple;explode=false;name=id" })
  id: string;
}

export class PutV2PeopleIdJsonResponse extends SpeakeasyBase {
  @SpeakeasyMetadata()
  body?: Uint8Array;

  @SpeakeasyMetadata()
  contentType: string;

  @SpeakeasyMetadata()
  statusCode: number;

  @SpeakeasyMetadata()
  rawResponse?: AxiosResponse;
}
