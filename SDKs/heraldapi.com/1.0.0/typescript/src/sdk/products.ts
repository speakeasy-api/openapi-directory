      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getProducts200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.GetProducts200ApplicationJSON
            );
          }
          break;
        case httpRes?.status == 400:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getProducts400ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.GetProducts400ApplicationJSON
            );
          }
          break;
      }
