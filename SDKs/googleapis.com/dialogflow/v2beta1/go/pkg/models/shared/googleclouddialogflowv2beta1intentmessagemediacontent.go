// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
)

// GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum - Optional. What type of media is the content (ie "audio").
type GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum string

const (
	GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnumResponseMediaTypeUnspecified GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum = "RESPONSE_MEDIA_TYPE_UNSPECIFIED"
	GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnumAudio                        GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum = "AUDIO"
)

func (e GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum) ToPointer() *GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum {
	return &e
}

func (e *GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "RESPONSE_MEDIA_TYPE_UNSPECIFIED":
		fallthrough
	case "AUDIO":
		*e = GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum: %v", v)
	}
}

// GoogleCloudDialogflowV2beta1IntentMessageMediaContent - The media content card for Actions on Google.
type GoogleCloudDialogflowV2beta1IntentMessageMediaContent struct {
	// Required. List of media objects.
	MediaObjects []GoogleCloudDialogflowV2beta1IntentMessageMediaContentResponseMediaObject `json:"mediaObjects,omitempty"`
	// Optional. What type of media is the content (ie "audio").
	MediaType *GoogleCloudDialogflowV2beta1IntentMessageMediaContentMediaTypeEnum `json:"mediaType,omitempty"`
}
