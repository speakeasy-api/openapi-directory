...
func (s *V1) StsOauthtoken(ctx context.Context, request operations.StsOauthtokenRequest) (*operations.StsOauthtokenResponse, error) {
	baseURL := s._serverURL
	...
	res := &operations.StsOauthtokenResponse{
		StatusCode:  int64(httpRes.StatusCode),
		ContentType: contentType,
	}
	switch {
	case httpRes.StatusCode == 200:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.GoogleIdentityStsV1ExchangeOauthTokenResponse
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}
