  ...
  **/
  mybusinessGoogleLocationsReport(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.MybusinessGoogleLocationsReportResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  mybusinessGoogleLocationsSearch(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.MybusinessGoogleLocationsSearchResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
