...
```bash
pip install git+https://github.com/speakeasy-api/openapi-directory.git#subdirectory=SDKs/googleapis.com/retail/v2alpha/python
```
...
* `retail_projects_locations_catalogs_merchant_center_account_links_create_merchant_center_account_link` - Creates a MerchantCenterAccountLink. MerchantCenterAccountLink cannot be set to a different oneof field, if so an INVALID_ARGUMENT is returned.
* `retail_projects_locations_catalogs_merchant_center_account_links_list` - Lists all MerchantCenterAccountLinks under the specified parent Catalog.
