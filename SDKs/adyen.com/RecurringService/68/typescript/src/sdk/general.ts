...
export class General {
   ...
   */
  postDisablePermit(
    req: shared.DisablePermitRequest,
    security: operations.PostDisablePermitSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.PostDisablePermitResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new shared.DisablePermitRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = baseURL.replace(/\/$/, "") + "/disablePermit";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.PostDisablePermitResponse =
        new operations.PostDisablePermitResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.disablePermitResult = utils.objectToClass(
              httpRes?.data,
              shared.DisablePermitResult
            );
          }
          break;
        case [400, 401, 403, 422, 500].includes(httpRes?.status):
