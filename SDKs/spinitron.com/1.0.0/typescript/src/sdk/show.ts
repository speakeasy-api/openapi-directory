...
export class Show {
  ...
  getShows(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetShowsResponse = new operations.GetShowsResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getShows200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.GetShows200ApplicationJSON
            );
          }
  ...
  getShowsId(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/shows/{id}", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GetShowsIdResponse =
        new operations.GetShowsIdResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.show = utils.objectToClass(httpRes?.data, shared.Show);
          }
          if (utils.matchContentType(contentType, `application/xml`)) {
            const resBody: string = JSON.stringify(httpRes?.data, null, 0);
            const out: Uint8Array = new Uint8Array(resBody.length);
