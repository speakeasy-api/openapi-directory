<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class UpdateSubmissionCommentRequest
{
	#[SpeakeasyMetadata('request:mediaType=application/json')]
    public \OpenAPI\OpenAPI\Models\Shared\AssignmentSubmissionCommentCreation $assignmentSubmissionCommentCreation;
    
    /**
     * Unique identifier of the assignment
     * 
     * @var string $assignment
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=assignment')]
    public string $assignment;
    
    /**
     * Unique identifier of the class
     * 
     * @var string $class
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=class')]
    public string $class;
    
    /**
     * Unique identifier of the comment
     * 
     * @var string $comment
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=comment')]
    public string $comment;
    
    /**
     * Unique identifier of the submission
     * 
     * @var string $submission
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=submission')]
    public string $submission;
    
	public function __construct()
	{
		$this->assignmentSubmissionCommentCreation = new \OpenAPI\OpenAPI\Models\Shared\AssignmentSubmissionCommentCreation();
		$this->assignment = "";
		$this->class = "";
		$this->comment = "";
		$this->submission = "";
	}
}
