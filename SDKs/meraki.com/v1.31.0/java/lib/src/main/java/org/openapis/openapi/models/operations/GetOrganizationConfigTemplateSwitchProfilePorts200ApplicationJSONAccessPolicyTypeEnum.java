/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import com.fasterxml.jackson.annotation.JsonValue;

/**
 * GetOrganizationConfigTemplateSwitchProfilePorts200ApplicationJSONAccessPolicyTypeEnum - The type of the access policy of the switch profile port. Only applicable to access ports. Can be one of 'Open', 'Custom access policy', 'MAC allow list' or 'Sticky MAC allow list'.
 */
public enum GetOrganizationConfigTemplateSwitchProfilePorts200ApplicationJSONAccessPolicyTypeEnum {
    CUSTOM_ACCESS_POLICY("Custom access policy"),
    MAC_ALLOW_LIST("MAC allow list"),
    OPEN("Open"),
    STICKY_MAC_ALLOW_LIST("Sticky MAC allow list");

    @JsonValue
    public final String value;

    private GetOrganizationConfigTemplateSwitchProfilePorts200ApplicationJSONAccessPolicyTypeEnum(String value) {
        this.value = value;
    }
}
