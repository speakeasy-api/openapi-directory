"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import federatedidentityrepresentation as shared_federatedidentityrepresentation
from typing import Optional


@dataclasses.dataclass
class GetRealmUsersIDFederatedIdentityRequest:
    
    id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'id', 'style': 'simple', 'explode': False }})
    r"""User id"""  
    realm: str = dataclasses.field(metadata={'path_param': { 'field_name': 'realm', 'style': 'simple', 'explode': False }})
    r"""realm name (not id!)"""  
    

@dataclasses.dataclass
class GetRealmUsersIDFederatedIdentityResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    federated_identity_representations: Optional[list[shared_federatedidentityrepresentation.FederatedIdentityRepresentation]] = dataclasses.field(default=None)
    r"""success"""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    