...
class SDK:
    ...
    def line_combinations_by_season(self, request: operations.LineCombinationsBySeasonRequest) -> operations.LineCombinationsBySeasonResponse:
        r"""Line Combinations by Season"""
        base_url = self._server_url
        
        url = utils.generate_url(operations.LineCombinationsBySeasonRequest, base_url, '/{format}/LinesBySeason/{season}', request)
        ...
        res = operations.LineCombinationsBySeasonResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[list[shared.TeamLine]])
                res.team_lines = out
