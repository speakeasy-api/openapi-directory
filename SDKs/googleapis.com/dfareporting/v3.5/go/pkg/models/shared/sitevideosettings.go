// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
)

// SiteVideoSettingsOrientationEnum - Orientation of a site template used for video. This will act as default for new placements created under this site.
type SiteVideoSettingsOrientationEnum string

const (
	SiteVideoSettingsOrientationEnumAny       SiteVideoSettingsOrientationEnum = "ANY"
	SiteVideoSettingsOrientationEnumLandscape SiteVideoSettingsOrientationEnum = "LANDSCAPE"
	SiteVideoSettingsOrientationEnumPortrait  SiteVideoSettingsOrientationEnum = "PORTRAIT"
)

func (e SiteVideoSettingsOrientationEnum) ToPointer() *SiteVideoSettingsOrientationEnum {
	return &e
}

func (e *SiteVideoSettingsOrientationEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "ANY":
		fallthrough
	case "LANDSCAPE":
		fallthrough
	case "PORTRAIT":
		*e = SiteVideoSettingsOrientationEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for SiteVideoSettingsOrientationEnum: %v", v)
	}
}

// SiteVideoSettings - Video Settings
type SiteVideoSettings struct {
	// Companion Settings
	CompanionSettings *SiteCompanionSetting `json:"companionSettings,omitempty"`
	// Identifies what kind of resource this is. Value: the fixed string "dfareporting#siteVideoSettings".
	Kind *string `json:"kind,omitempty"`
	// Whether OBA icons are enabled for this placement.
	ObaEnabled *bool `json:"obaEnabled,omitempty"`
	// Online Behavioral Advertiser icon.
	ObaSettings *ObaIcon `json:"obaSettings,omitempty"`
	// Orientation of a site template used for video. This will act as default for new placements created under this site.
	Orientation *SiteVideoSettingsOrientationEnum `json:"orientation,omitempty"`
	// Skippable Settings
	SkippableSettings *SiteSkippableSetting `json:"skippableSettings,omitempty"`
	// Transcode Settings
	TranscodeSettings *SiteTranscodeSetting `json:"transcodeSettings,omitempty"`
}
