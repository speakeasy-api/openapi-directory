import dataclasses
from typing import Optional
from ..shared import security as shared_security
from ..shared import event_simple as shared_event_simple


@dataclasses.dataclass
class GetEventSimplePathParams:
    event_key: str = dataclasses.field(metadata={'path_param': { 'field_name': 'event_key', 'style': 'simple', 'explode': False }})
    

@dataclasses.dataclass
class GetEventSimpleHeaders:
    if_modified_since: Optional[str] = dataclasses.field(default=None, metadata={'header': { 'field_name': 'If-Modified-Since', 'style': 'simple', 'explode': False }})
    

@dataclasses.dataclass
class GetEventSimpleSecurity:
    api_key: shared_security.SchemeAPIKey = dataclasses.field(metadata={'security': { 'scheme': True, 'type': 'apiKey', 'sub_type': 'header' }})
    

@dataclasses.dataclass
class GetEventSimpleRequest:
    headers: GetEventSimpleHeaders = dataclasses.field()
    path_params: GetEventSimplePathParams = dataclasses.field()
    security: GetEventSimpleSecurity = dataclasses.field()
    

@dataclasses.dataclass
class GetEventSimpleResponse:
    content_type: str = dataclasses.field()
    headers: dict[str, list[str]] = dataclasses.field()
    status_code: int = dataclasses.field()
    event_simple: Optional[shared_event_simple.EventSimple] = dataclasses.field(default=None)
    
