...
```bash
pip install git+https://github.com/speakeasy-api/openapi-directory.git#subdirectory=SDKs/amazonaws.com/keyspaces/2022-02-10/python
```
...
req = operations.CreateKeyspaceRequest(
    create_keyspace_request=shared.CreateKeyspaceRequest(
        keyspace_name="corrupti",
        tags=[
            shared.Tag(
                key="distinctio",
                value="quibusdam",
    ...
    x_amz_target="KeyspacesService.CreateKeyspace",
)
    
res = s.create_keyspace(req)

if res.create_keyspace_response is not None:
    # handle response
```
