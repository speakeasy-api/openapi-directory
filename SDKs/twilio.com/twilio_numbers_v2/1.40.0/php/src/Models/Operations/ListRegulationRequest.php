<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class ListRegulationRequest
{
    /**
     * The type of End User the regulation requires - can be `individual` or `business`.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\RegulationEnumEndUserTypeEnum $endUserType
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=EndUserType')]
    public ?\OpenAPI\OpenAPI\Models\Shared\RegulationEnumEndUserTypeEnum $endUserType = null;
    
    /**
     * The ISO country code of the phone number's country.
     * 
     * @var ?string $isoCountry
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=IsoCountry')]
    public ?string $isoCountry = null;
    
    /**
     * The type of phone number that the regulatory requiremnt is restricting.
     * 
     * @var ?string $numberType
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=NumberType')]
    public ?string $numberType = null;
    
    /**
     * The page index. This value is simply for client state.
     * 
     * @var ?int $page
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=Page')]
    public ?int $page = null;
    
    /**
     * How many resources to return in each list page. The default is 50, and the maximum is 1000.
     * 
     * @var ?int $pageSize
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=PageSize')]
    public ?int $pageSize = null;
    
    /**
     * The page token. This is provided by the API.
     * 
     * @var ?string $pageToken
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=PageToken')]
    public ?string $pageToken = null;
    
	public function __construct()
	{
		$this->endUserType = null;
		$this->isoCountry = null;
		$this->numberType = null;
		$this->page = null;
		$this->pageSize = null;
		$this->pageToken = null;
	}
}
