/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.shared;

import com.fasterxml.jackson.annotation.JsonProperty;

public class IPBlocklistResponse {
    /**
     * An array of strings indicating which blocklist categories this IP is listed on
     */
    @JsonProperty("blocklists")
    public String[] blocklists;

    public IPBlocklistResponse withBlocklists(String[] blocklists) {
        this.blocklists = blocklists;
        return this;
    }
    
    /**
     * The CIDR address for this listing (only set if the IP is listed)
     */
    @JsonProperty("cidr")
    public String cidr;

    public IPBlocklistResponse withCidr(String cidr) {
        this.cidr = cidr;
        return this;
    }
    
    /**
     * The IP address
     */
    @JsonProperty("ip")
    public String ip;

    public IPBlocklistResponse withIp(String ip) {
        this.ip = ip;
        return this;
    }
    
    /**
     * IP is hosting a malicious bot or is part of a botnet. This is a broad category which includes brute-force crackers
     */
    @JsonProperty("is-bot")
    public Boolean isBot;

    public IPBlocklistResponse withIsBot(Boolean isBot) {
        this.isBot = isBot;
        return this;
    }
    
    /**
     * IP has been flagged as a significant attack source by DShield (dshield.org)
     */
    @JsonProperty("is-dshield")
    public Boolean isDshield;

    public IPBlocklistResponse withIsDshield(Boolean isDshield) {
        this.isDshield = isDshield;
        return this;
    }
    
    /**
     * IP is hosting an exploit finding bot or is running exploit scanning software
     */
    @JsonProperty("is-exploit-bot")
    public Boolean isExploitBot;

    public IPBlocklistResponse withIsExploitBot(Boolean isExploitBot) {
        this.isExploitBot = isExploitBot;
        return this;
    }
    
    /**
     * IP is part of a hijacked netblock or a netblock controlled by a criminal organization
     */
    @JsonProperty("is-hijacked")
    public Boolean isHijacked;

    public IPBlocklistResponse withIsHijacked(Boolean isHijacked) {
        this.isHijacked = isHijacked;
        return this;
    }
    
    /**
     * Is this IP on a blocklist
     */
    @JsonProperty("is-listed")
    public Boolean isListed;

    public IPBlocklistResponse withIsListed(Boolean isListed) {
        this.isListed = isListed;
        return this;
    }
    
    /**
     * IP is involved in distributing or is running malware
     */
    @JsonProperty("is-malware")
    public Boolean isMalware;

    public IPBlocklistResponse withIsMalware(Boolean isMalware) {
        this.isMalware = isMalware;
        return this;
    }
    
    /**
     * IP has been detected as an anonymous web proxy or anonymous HTTP proxy
     */
    @JsonProperty("is-proxy")
    public Boolean isProxy;

    public IPBlocklistResponse withIsProxy(Boolean isProxy) {
        this.isProxy = isProxy;
        return this;
    }
    
    /**
     * IP address is hosting a spam bot, comment spamming or any other spamming type software
     */
    @JsonProperty("is-spam-bot")
    public Boolean isSpamBot;

    public IPBlocklistResponse withIsSpamBot(Boolean isSpamBot) {
        this.isSpamBot = isSpamBot;
        return this;
    }
    
    /**
     * IP is running a hostile web spider / web crawler
     */
    @JsonProperty("is-spider")
    public Boolean isSpider;

    public IPBlocklistResponse withIsSpider(Boolean isSpider) {
        this.isSpider = isSpider;
        return this;
    }
    
    /**
     * IP is involved in distributing or is running spyware
     */
    @JsonProperty("is-spyware")
    public Boolean isSpyware;

    public IPBlocklistResponse withIsSpyware(Boolean isSpyware) {
        this.isSpyware = isSpyware;
        return this;
    }
    
    /**
     * IP is a Tor node or running a Tor related service
     */
    @JsonProperty("is-tor")
    public Boolean isTor;

    public IPBlocklistResponse withIsTor(Boolean isTor) {
        this.isTor = isTor;
        return this;
    }
    
    /**
     * IP belongs to a public VPN provider (only set if the 'vpn-lookup' option is enabled)
     */
    @JsonProperty("is-vpn")
    public Boolean isVpn;

    public IPBlocklistResponse withIsVpn(Boolean isVpn) {
        this.isVpn = isVpn;
        return this;
    }
    
    /**
     * The unix time when this IP was last seen on any blocklist. IPs are automatically removed after 7 days therefor this value will never be older than 7 days
     */
    @JsonProperty("last-seen")
    public Integer lastSeen;

    public IPBlocklistResponse withLastSeen(Integer lastSeen) {
        this.lastSeen = lastSeen;
        return this;
    }
    
    /**
     * The number of blocklists the IP is listed on
     */
    @JsonProperty("list-count")
    public Integer listCount;

    public IPBlocklistResponse withListCount(Integer listCount) {
        this.listCount = listCount;
        return this;
    }
    
    /**
     * An array of objects containing details on which specific sensors detected the IP
     */
    @JsonProperty("sensors")
    public BlocklistSensor[] sensors;

    public IPBlocklistResponse withSensors(BlocklistSensor[] sensors) {
        this.sensors = sensors;
        return this;
    }
    
    public IPBlocklistResponse(@JsonProperty("blocklists") String[] blocklists, @JsonProperty("cidr") String cidr, @JsonProperty("ip") String ip, @JsonProperty("is-bot") Boolean isBot, @JsonProperty("is-dshield") Boolean isDshield, @JsonProperty("is-exploit-bot") Boolean isExploitBot, @JsonProperty("is-hijacked") Boolean isHijacked, @JsonProperty("is-listed") Boolean isListed, @JsonProperty("is-malware") Boolean isMalware, @JsonProperty("is-proxy") Boolean isProxy, @JsonProperty("is-spam-bot") Boolean isSpamBot, @JsonProperty("is-spider") Boolean isSpider, @JsonProperty("is-spyware") Boolean isSpyware, @JsonProperty("is-tor") Boolean isTor, @JsonProperty("is-vpn") Boolean isVpn, @JsonProperty("last-seen") Integer lastSeen, @JsonProperty("list-count") Integer listCount, @JsonProperty("sensors") BlocklistSensor[] sensors) {
        this.blocklists = blocklists;
        this.cidr = cidr;
        this.ip = ip;
        this.isBot = isBot;
        this.isDshield = isDshield;
        this.isExploitBot = isExploitBot;
        this.isHijacked = isHijacked;
        this.isListed = isListed;
        this.isMalware = isMalware;
        this.isProxy = isProxy;
        this.isSpamBot = isSpamBot;
        this.isSpider = isSpider;
        this.isSpyware = isSpyware;
        this.isTor = isTor;
        this.isVpn = isVpn;
        this.lastSeen = lastSeen;
        this.listCount = listCount;
        this.sensors = sensors;
  }
}
