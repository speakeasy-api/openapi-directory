...
```bash
pip install git+https://github.com/speakeasy-api/openapi-directory.git#subdirectory=SDKs/sportsdata.io/mlb-v3-stats/1.0/python
```
...
* `batter_vs_pitcher_stats` - Batter vs. Pitcher Stats
* `box_score` - Box Score
...
* `player_season_away_stats` - Player Season Away Stats
* `player_season_home_stats` - Player Season Home Stats
* `player_season_split_stats` - Player Season Split Stats
...
* `player_season_stats_split_by_team` - Player Season Stats Split By Team
...
* `team_hitting_vs_starting_pitcher` - Team Hitting vs. Starting Pitcher
