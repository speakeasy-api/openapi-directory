...
export class InventoryItems {
  ...
  dfareportingInventoryItemsGet(
    ...
    security: operations.DfareportingInventoryItemsGetSecurity,
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.DfareportingInventoryItemsGetResponse =
        new operations.DfareportingInventoryItemsGetResponse({
  ...
  dfareportingInventoryItemsList(
    ...
    security: operations.DfareportingInventoryItemsListSecurity,
    config?: AxiosRequestConfig
    ...
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.DfareportingInventoryItemsListSecurity(
        security
      );
    }
    const client: AxiosInstance = utils.createSecurityClient(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.DfareportingInventoryItemsListResponse =
        new operations.DfareportingInventoryItemsListResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.inventoryItemsListResponse = utils.objectToClass(
              httpRes?.data,
              shared.InventoryItemsListResponse
            );
          }
          break;
      }
