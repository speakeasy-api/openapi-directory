...
export class Areas {
  ...
  areasGet(
    ...
    const url: string = utils.generateURL(baseURL, "/v{version}/areas", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.AreasGetResponse = new operations.AreasGetResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.areaResponse = utils.objectToClass(
              httpRes?.data,
              shared.AreaResponse
            );
          }
  ...
  getVVersionAreasAreaIds(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GetVVersionAreasAreaIdsResponse =
        new operations.GetVVersionAreasAreaIdsResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
