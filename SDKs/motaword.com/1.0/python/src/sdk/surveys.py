...
class Surveys:
    ...
    def get_questions(self, request: operations.GetQuestionsRequest) -> operations.GetQuestionsResponse:
        r"""Get survey questions in given scope and type
        Get survey questions in given scope and type
        """
        base_url = self._server_url
        
        url = utils.generate_url(operations.GetQuestionsRequest, base_url, '/surveys/{scope}/{type}', request)
        ...
        if http_res.status_code == 200:
            if utils.match_content_type(content_type, 'application/json'):
                out = utils.unmarshal_json(http_res.text, Optional[list[shared.SurveyQuestion]])
                res.survey_questions = out
    ...
    def submit_answers(self, request: operations.SubmitAnswersRequest) -> operations.SubmitAnswersResponse:
        r"""Post survey answers for scope and type
        Post survey answers for scope and type
        """
        base_url = self._server_url
        
        url = utils.generate_url(operations.SubmitAnswersRequest, base_url, '/surveys/{scope}/{type}', request)
        
        headers = {}
        req_content_type, data, form = utils.serialize_request_body(request, "survey_answers", 'json')
