...
export class BundleDownloads {
  ...
  getBundleDownloads(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetBundleDownloadsResponse =
        new operations.GetBundleDownloadsResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.bundleDownloadEntities = [];
            const resFieldDepth: number = utils.getResFieldDepth(res);
            res.bundleDownloadEntities = utils.objectToClass(
              httpRes?.data,
              shared.BundleDownloadEntity,
              resFieldDepth
            );
          }
          break;
