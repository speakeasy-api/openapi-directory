<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class GetConnectionsIdConnectionLogsRequest
{
    /**
     * UUID of a connector
     * 
     * @var ?string $connectorUuid
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=connector_uuid')]
    public ?string $connectorUuid = null;
    
    /**
     * connections log error filter
     * 
     * @var ?string $error
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=error')]
    public ?string $error = null;
    
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=expand')]
    public ?string $expand = null;
    
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=id_connection')]
    public int $idConnectionPathParameter;
    
    /**
     * ID of a connection
     * 
     * @var ?int $idConnectionQueryParameter
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=id_connection')]
    public ?int $idConnectionQueryParameter = null;
    
    /**
     * ID of a connector
     * 
     * @var ?int $idConnector
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=id_connector')]
    public ?int $idConnector = null;
    
    /**
     * filter "id" of logs, maximum id to return
     * 
     * @var ?int $idMax
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=id_max')]
    public ?int $idMax = null;
    
    /**
     * ID of a source
     * 
     * @var ?int $idSource
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=id_source')]
    public ?int $idSource = null;
    
    /**
     * ID of a user
     * 
     * @var ?int $idUser
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=id_user')]
    public ?int $idUser = null;
    
    /**
     * limit number of results
     * 
     * @var ?int $limit
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=limit')]
    public ?int $limit = null;
    
    /**
     * maximum date
     * 
     * @var ?\DateTime $maxDate
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=max_date,dateTimeFormat=Y-m-d')]
    public ?\DateTime $maxDate = null;
    
    /**
     * minimal date
     * 
     * @var ?\DateTime $minDate
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=min_date,dateTimeFormat=Y-m-d')]
    public ?\DateTime $minDate = null;
    
    /**
     * offset of first result
     * 
     * @var ?int $offset
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=offset')]
    public ?int $offset = null;
    
    /**
     * period to group logs
     * 
     * @var ?string $period
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=period')]
    public ?string $period = null;
    
	public function __construct()
	{
		$this->connectorUuid = null;
		$this->error = null;
		$this->expand = null;
		$this->idConnectionPathParameter = 0;
		$this->idConnectionQueryParameter = null;
		$this->idConnector = null;
		$this->idMax = null;
		$this->idSource = null;
		$this->idUser = null;
		$this->limit = null;
		$this->maxDate = null;
		$this->minDate = null;
		$this->offset = null;
		$this->period = null;
	}
}
