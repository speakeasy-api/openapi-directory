<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * DeleteConsumerResponse - Consumer deleted
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class DeleteConsumerResponse
{
	#[\JMS\Serializer\Annotation\SerializedName('data')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\DeleteConsumerResponseData')]
    public DeleteConsumerResponseData $data;
    
    /**
     * HTTP Response Status
     * 
     * @var string $status
     */
	#[\JMS\Serializer\Annotation\SerializedName('status')]
    #[\JMS\Serializer\Annotation\Type('string')]
    public string $status;
    
    /**
     * HTTP Response Status Code
     * 
     * @var int $statusCode
     */
	#[\JMS\Serializer\Annotation\SerializedName('status_code')]
    #[\JMS\Serializer\Annotation\Type('int')]
    public int $statusCode;
    
	public function __construct()
	{
		$this->data = new \OpenAPI\OpenAPI\Models\Shared\DeleteConsumerResponseData();
		$this->status = "";
		$this->statusCode = 0;
	}
}
