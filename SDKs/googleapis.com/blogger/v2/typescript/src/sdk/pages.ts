  ...
  **/
  bloggerPagesGet(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.BloggerPagesGetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  bloggerPagesList(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.BloggerPagesListResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
