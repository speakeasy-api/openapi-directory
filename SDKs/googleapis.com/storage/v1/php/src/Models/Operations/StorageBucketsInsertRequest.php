<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class StorageBucketsInsertRequest
{
	#[SpeakeasyMetadata('request:mediaType=application/json')]
    public ?\OpenAPI\OpenAPI\Models\Shared\Bucket $bucket = null;
    
    /**
     * Data format for the response.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\AltEnum $alt
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=alt')]
    public ?\OpenAPI\OpenAPI\Models\Shared\AltEnum $alt = null;
    
    /**
     * Selector specifying which fields to include in a partial response.
     * 
     * @var ?string $fields
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=fields')]
    public ?string $fields = null;
    
    /**
     * API key. Your API key identifies your project and provides you with API access, quota, and reports. Required unless you provide an OAuth 2.0 token.
     * 
     * @var ?string $key
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=key')]
    public ?string $key = null;
    
    /**
     * OAuth 2.0 token for the current user.
     * 
     * @var ?string $oauthToken
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=oauth_token')]
    public ?string $oauthToken = null;
    
    /**
     * Apply a predefined set of access controls to this bucket.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\StorageBucketsInsertPredefinedACLEnum $predefinedAcl
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=predefinedAcl')]
    public ?StorageBucketsInsertPredefinedACLEnum $predefinedAcl = null;
    
    /**
     * Apply a predefined set of default object access controls to this bucket.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\StorageBucketsInsertPredefinedDefaultObjectACLEnum $predefinedDefaultObjectAcl
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=predefinedDefaultObjectAcl')]
    public ?StorageBucketsInsertPredefinedDefaultObjectACLEnum $predefinedDefaultObjectAcl = null;
    
    /**
     * Returns response with indentations and line breaks.
     * 
     * @var ?bool $prettyPrint
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=prettyPrint')]
    public ?bool $prettyPrint = null;
    
    /**
     * A valid API project identifier.
     * 
     * @var string $project
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=project')]
    public string $project;
    
    /**
     * Set of properties to return. Defaults to noAcl, unless the bucket resource specifies acl or defaultObjectAcl properties, when it defaults to full.
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\StorageBucketsInsertProjectionEnum $projection
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=projection')]
    public ?StorageBucketsInsertProjectionEnum $projection = null;
    
    /**
     * An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
     * 
     * @var ?string $quotaUser
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=quotaUser')]
    public ?string $quotaUser = null;
    
    /**
     * Upload protocol for media (e.g. "media", "multipart", "resumable").
     * 
     * @var ?string $uploadType
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=uploadType')]
    public ?string $uploadType = null;
    
    /**
     * Deprecated. Please use quotaUser instead.
     * 
     * @var ?string $userIp
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=userIp')]
    public ?string $userIp = null;
    
    /**
     * The project to be billed for this request.
     * 
     * @var ?string $userProject
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=userProject')]
    public ?string $userProject = null;
    
	public function __construct()
	{
		$this->bucket = null;
		$this->alt = null;
		$this->fields = null;
		$this->key = null;
		$this->oauthToken = null;
		$this->predefinedAcl = null;
		$this->predefinedDefaultObjectAcl = null;
		$this->prettyPrint = null;
		$this->project = "";
		$this->projection = null;
		$this->quotaUser = null;
		$this->uploadType = null;
		$this->userIp = null;
		$this->userProject = null;
	}
}
