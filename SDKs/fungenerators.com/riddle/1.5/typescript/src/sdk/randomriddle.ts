  ...
  **/
  getRiddleRandom(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetRiddleRandomResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  getRiddleSearch(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GetRiddleSearchResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
          case httpRes?.status == 401:
