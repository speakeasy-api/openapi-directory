import dataclasses
from typing import Optional
from dataclasses_json import dataclass_json
from sdk import utils
from ..shared import attachment as shared_attachment
from ..shared import perobjectstatus as shared_perobjectstatus


@dataclass_json
@dataclasses.dataclass
class SyncState:
    attachment: Optional[shared_attachment.Attachment] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.field_name('Attachment') }})
    config: Optional[dict[str, shared_perobjectstatus.PerObjectStatus]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.field_name('Config') }})
    
