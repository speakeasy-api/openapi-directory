...
export class Customers {
  ...
  **/
  chromemanagementCustomersTelemetryEventsList(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/v1/{parent}/telemetry/events", req.pathParams);
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ChromemanagementCustomersTelemetryEventsListResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.googleChromeManagementV1ListTelemetryEventsResponse = httpRes?.data;
            }
            break;
        }
