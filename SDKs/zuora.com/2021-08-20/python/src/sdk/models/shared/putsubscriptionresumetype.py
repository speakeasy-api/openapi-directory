...
class PUTSubscriptionResumeTypeRunBillingEnum(str, Enum):
...
class PUTSubscriptionResumeType:
    ...
    resume_policy: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('resumePolicy') }})
    r"""Resume methods. Specify a way to resume a subscription.
    ...
    **Note**: This field is only available if you set the `zuora-version` request header to `196.0` or later.
    ...
    r"""The date when the customer notifies you that they want to resume their subscription."""  
    ...
    r"""Whether to extend the subscription term by the length of time the suspension is in effect.
    ...
    **Note:** This field is only applicable only when the `resumePolicy` field is set to `SpecificDate`.
    ...
    """  
    run_billing: Optional[PUTSubscriptionResumeTypeRunBillingEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('runBilling'), 'exclude': lambda f: f is None }})
    r"""Creates an invoice for a subscription. If you have the Invoice Settlement feature enabled, a credit memo might also be created based on the [invoice and credit memo generation rule](https://knowledgecenter.zuora.com/CB_Billing/Invoice_Settlement/Credit_and_Debit_Memos/Rules_for_Generating_Invoices_and_Credit_Memos).
