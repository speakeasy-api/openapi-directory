<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * OBWritePaymentDetailsResponse1 - Payment Details Read
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class OBWritePaymentDetailsResponse1
{
	#[\JMS\Serializer\Annotation\SerializedName('Data')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\OBWritePaymentDetailsResponse1Data')]
    public OBWritePaymentDetailsResponse1Data $data;
    
    /**
     * Links relevant to the payload
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\Links $links
     */
	#[\JMS\Serializer\Annotation\SerializedName('Links')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\Links')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?Links $links = null;
    
    /**
     * Meta Data relevant to the payload
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\Meta $meta
     */
	#[\JMS\Serializer\Annotation\SerializedName('Meta')]
    #[\JMS\Serializer\Annotation\Type('OpenAPI\OpenAPI\Models\Shared\Meta')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?Meta $meta = null;
    
	public function __construct()
	{
		$this->data = new \OpenAPI\OpenAPI\Models\Shared\OBWritePaymentDetailsResponse1Data();
		$this->links = null;
		$this->meta = null;
	}
}
