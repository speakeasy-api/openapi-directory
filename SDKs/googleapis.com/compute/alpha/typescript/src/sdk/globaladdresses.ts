...
export class GlobalAddresses {
  ...
  computeGlobalAddressesGetOwnerInstance(
    req: operations.ComputeGlobalAddressesGetOwnerInstanceRequest,
    security: operations.ComputeGlobalAddressesGetOwnerInstanceSecurity,
    config?: AxiosRequestConfig
  ): Promise<operations.ComputeGlobalAddressesGetOwnerInstanceResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.ComputeGlobalAddressesGetOwnerInstanceRequest(req);
    }

    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(
      baseURL,
      "/projects/{project}/global/addresses/getOwnerInstance",
      req
    );
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.ComputeGlobalAddressesGetOwnerInstanceSecurity(
        security
      );
    }
    const client: AxiosInstance = utils.createSecurityClient(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.ComputeGlobalAddressesGetOwnerInstanceResponse =
        new operations.ComputeGlobalAddressesGetOwnerInstanceResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getOwnerInstanceResponse = utils.objectToClass(
              httpRes?.data,
              shared.GetOwnerInstanceResponse
            );
          }
          break;
      }
