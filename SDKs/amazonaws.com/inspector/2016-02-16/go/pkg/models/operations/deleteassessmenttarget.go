// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"encoding/json"
	"fmt"
	"net/http"
	"openapi/pkg/models/shared"
)

// DeleteAssessmentTargetXAmzTargetEnum
type DeleteAssessmentTargetXAmzTargetEnum string

const (
	DeleteAssessmentTargetXAmzTargetEnumInspectorServiceDeleteAssessmentTarget DeleteAssessmentTargetXAmzTargetEnum = "InspectorService.DeleteAssessmentTarget"
)

func (e DeleteAssessmentTargetXAmzTargetEnum) ToPointer() *DeleteAssessmentTargetXAmzTargetEnum {
	return &e
}

func (e *DeleteAssessmentTargetXAmzTargetEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "InspectorService.DeleteAssessmentTarget":
		*e = DeleteAssessmentTargetXAmzTargetEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for DeleteAssessmentTargetXAmzTargetEnum: %v", v)
	}
}

type DeleteAssessmentTargetRequest struct {
	DeleteAssessmentTargetRequest shared.DeleteAssessmentTargetRequest `request:"mediaType=application/json"`
	XAmzAlgorithm                 *string                              `header:"style=simple,explode=false,name=X-Amz-Algorithm"`
	XAmzContentSha256             *string                              `header:"style=simple,explode=false,name=X-Amz-Content-Sha256"`
	XAmzCredential                *string                              `header:"style=simple,explode=false,name=X-Amz-Credential"`
	XAmzDate                      *string                              `header:"style=simple,explode=false,name=X-Amz-Date"`
	XAmzSecurityToken             *string                              `header:"style=simple,explode=false,name=X-Amz-Security-Token"`
	XAmzSignature                 *string                              `header:"style=simple,explode=false,name=X-Amz-Signature"`
	XAmzSignedHeaders             *string                              `header:"style=simple,explode=false,name=X-Amz-SignedHeaders"`
	XAmzTarget                    DeleteAssessmentTargetXAmzTargetEnum `header:"style=simple,explode=false,name=X-Amz-Target"`
}

type DeleteAssessmentTargetResponse struct {
	// AccessDeniedException
	AccessDeniedException interface{}
	// AssessmentRunInProgressException
	AssessmentRunInProgressException interface{}
	ContentType                      string
	// InternalException
	InternalException interface{}
	// InvalidInputException
	InvalidInputException interface{}
	// NoSuchEntityException
	NoSuchEntityException interface{}
	// ServiceTemporarilyUnavailableException
	ServiceTemporarilyUnavailableException interface{}
	StatusCode                             int
	RawResponse                            *http.Response
}
