...
export class CodeScanning {
  ...
  **/
  codeScanningListAlertsInstances(
    req: operations.CodeScanningListAlertsInstancesRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.CodeScanningListAlertsInstancesResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.CodeScanningListAlertsInstancesRequest(req);
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.CodeScanningListAlertsInstancesResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
          ...
          case httpRes?.status == 503:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.codeScanningListAlertsInstances503ApplicationJSONObject = httpRes?.data;
            }
            break;
        }
