"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
import requests as requests_http
from ..shared import bulkimagesearchresults as shared_bulkimagesearchresults
from ..shared import language_enum as shared_language_enum
from ..shared import searchimage as shared_searchimage
from ..shared import security as shared_security
from datetime import date
from enum import Enum
from typing import Any, Optional


@dataclasses.dataclass
class BulkSearchImagesSecurity:
    
    basic: Optional[shared_security.SchemeBasic] = dataclasses.field(default=None, metadata={'security': { 'scheme': True, 'type': 'http', 'sub_type': 'basic' }})  
    customer_access_code: Optional[str] = dataclasses.field(default=None, metadata={'security': { 'scheme': True, 'type': 'oauth2', 'field_name': 'Authorization' }})  
    
class BulkSearchImagesImageTypeEnum(str, Enum):
    PHOTO = 'photo'
    ILLUSTRATION = 'illustration'
    VECTOR = 'vector'

class BulkSearchImagesLicenseEnum(str, Enum):
    COMMERCIAL = 'commercial'
    EDITORIAL = 'editorial'
    ENHANCED = 'enhanced'

class BulkSearchImagesOrientationEnum(str, Enum):
    r"""Show image results with horizontal or vertical orientation"""
    HORIZONTAL = 'horizontal'
    VERTICAL = 'vertical'

class BulkSearchImagesPeopleAgeEnum(str, Enum):
    r"""Show images that feature people of the specified age category"""
    INFANTS = 'infants'
    CHILDREN = 'children'
    TEENAGERS = 'teenagers'
    TWENTYS = '20s'
    THIRTYS = '30s'
    FORTYS = '40s'
    FIFTYS = '50s'
    SIXTYS = '60s'
    OLDER = 'older'

class BulkSearchImagesPeopleEthnicityEnum(str, Enum):
    AFRICAN = 'african'
    AFRICAN_AMERICAN = 'african_american'
    BLACK = 'black'
    BRAZILIAN = 'brazilian'
    CHINESE = 'chinese'
    CAUCASIAN = 'caucasian'
    EAST_ASIAN = 'east_asian'
    HISPANIC = 'hispanic'
    JAPANESE = 'japanese'
    MIDDLE_EASTERN = 'middle_eastern'
    NATIVE_AMERICAN = 'native_american'
    PACIFIC_ISLANDER = 'pacific_islander'
    SOUTH_ASIAN = 'south_asian'
    SOUTHEAST_ASIAN = 'southeast_asian'
    OTHER = 'other'
    NOT_AFRICAN = 'NOT african'
    NOT_AFRICAN_AMERICAN = 'NOT african_american'
    NOT_BLACK = 'NOT black'
    NOT_BRAZILIAN = 'NOT brazilian'
    NOT_CHINESE = 'NOT chinese'
    NOT_CAUCASIAN = 'NOT caucasian'
    NOT_EAST_ASIAN = 'NOT east_asian'
    NOT_HISPANIC = 'NOT hispanic'
    NOT_JAPANESE = 'NOT japanese'
    NOT_MIDDLE_EASTERN = 'NOT middle_eastern'
    NOT_NATIVE_AMERICAN = 'NOT native_american'
    NOT_PACIFIC_ISLANDER = 'NOT pacific_islander'
    NOT_SOUTH_ASIAN = 'NOT south_asian'
    NOT_SOUTHEAST_ASIAN = 'NOT southeast_asian'
    NOT_OTHER = 'NOT other'

class BulkSearchImagesPeopleGenderEnum(str, Enum):
    r"""Show images with people of the specified gender"""
    MALE = 'male'
    FEMALE = 'female'
    BOTH = 'both'

class BulkSearchImagesSortEnum(str, Enum):
    r"""Sort by"""
    NEWEST = 'newest'
    POPULAR = 'popular'
    RELEVANCE = 'relevance'
    RANDOM = 'random'

class BulkSearchImagesViewEnum(str, Enum):
    r"""Amount of detail to render in the response"""
    MINIMAL = 'minimal'
    FULL = 'full'


@dataclasses.dataclass
class BulkSearchImagesRequest:
    
    request_body: list[shared_searchimage.SearchImage] = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})
    r"""List of queries to request results for and filters to apply per query; these values override the defaults in the query parameters"""  
    added_date: Optional[date] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'added_date', 'style': 'form', 'explode': True }})
    r"""Show images added on the specified date"""  
    added_date_end: Optional[date] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'added_date_end', 'style': 'form', 'explode': True }})
    r"""Show images added before the specified date"""  
    added_date_start: Optional[date] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'added_date_start', 'style': 'form', 'explode': True }})
    r"""Show images added on or after the specified date"""  
    aspect_ratio: Optional[float] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'aspect_ratio', 'style': 'form', 'explode': True }})
    r"""Show images with the specified aspect ratio, using a positive decimal of the width divided by the height, such as 1.7778 for a 16:9 image"""  
    aspect_ratio_max: Optional[float] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'aspect_ratio_max', 'style': 'form', 'explode': True }})
    r"""Show images with the specified aspect ratio or lower, using a positive decimal of the width divided by the height, such as 1.7778 for a 16:9 image"""  
    aspect_ratio_min: Optional[float] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'aspect_ratio_min', 'style': 'form', 'explode': True }})
    r"""Show images with the specified aspect ratio or higher, using a positive decimal of the width divided by the height, such as 1.7778 for a 16:9 image"""  
    category: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'category', 'style': 'form', 'explode': True }})
    r"""Show images with the specified Shutterstock-defined category; specify a category name or ID"""  
    color: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'color', 'style': 'form', 'explode': True }})
    r"""Specify either a hexadecimal color in the format '4F21EA' or 'grayscale'; the API returns images that use similar colors"""  
    contributor: Optional[list[str]] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'contributor', 'style': 'form', 'explode': True }})
    r"""Show images with the specified contributor names or IDs, allows multiple"""  
    contributor_country: Optional[Any] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'contributor_country', 'style': 'form', 'explode': True }})
    r"""Show images from contributors in one or more specified countries, or start with NOT to exclude a country from the search"""  
    fields_: Optional[str] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'fields', 'style': 'form', 'explode': True }})
    r"""Fields to display in the response; see the documentation for the fields parameter in the overview section"""  
    height: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'height', 'style': 'form', 'explode': True }})
    r"""(Deprecated; use height_from and height_to instead) Show images with the specified height"""  
    height_from: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'height_from', 'style': 'form', 'explode': True }})
    r"""Show images with the specified height or larger, in pixels"""  
    height_to: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'height_to', 'style': 'form', 'explode': True }})
    r"""Show images with the specified height or smaller, in pixels"""  
    image_type: Optional[list[BulkSearchImagesImageTypeEnum]] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'image_type', 'style': 'form', 'explode': True }})
    r"""Show images of the specified type"""  
    keyword_safe_search: Optional[bool] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'keyword_safe_search', 'style': 'form', 'explode': True }})
    r"""Hide results with potentially unsafe keywords"""  
    language: Optional[shared_language_enum.LanguageEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'language', 'style': 'form', 'explode': True }})
    r"""Set query and result language (uses Accept-Language header if not set)"""  
    license: Optional[list[BulkSearchImagesLicenseEnum]] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'license', 'style': 'form', 'explode': True }})
    r"""Show only images with the specified license"""  
    model: Optional[list[str]] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'model', 'style': 'form', 'explode': True }})
    r"""Show image results with the specified model IDs"""  
    orientation: Optional[BulkSearchImagesOrientationEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'orientation', 'style': 'form', 'explode': True }})
    r"""Show image results with horizontal or vertical orientation"""  
    page: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'page', 'style': 'form', 'explode': True }})
    r"""Page number"""  
    people_age: Optional[BulkSearchImagesPeopleAgeEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'people_age', 'style': 'form', 'explode': True }})
    r"""Show images that feature people of the specified age category"""  
    people_ethnicity: Optional[list[BulkSearchImagesPeopleEthnicityEnum]] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'people_ethnicity', 'style': 'form', 'explode': True }})
    r"""Show images with people of the specified ethnicities, or start with NOT to show images without those ethnicities"""  
    people_gender: Optional[BulkSearchImagesPeopleGenderEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'people_gender', 'style': 'form', 'explode': True }})
    r"""Show images with people of the specified gender"""  
    people_model_released: Optional[bool] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'people_model_released', 'style': 'form', 'explode': True }})
    r"""Show images of people with a signed model release"""  
    people_number: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'people_number', 'style': 'form', 'explode': True }})
    r"""Show images with the specified number of people"""  
    per_page: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'per_page', 'style': 'form', 'explode': True }})
    r"""Number of results per page"""  
    region: Optional[Any] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'region', 'style': 'form', 'explode': True }})
    r"""Raise or lower search result rankings based on the result's relevance to a specified region; you can provide a country code or an IP address from which the API infers a country"""  
    safe: Optional[bool] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'safe', 'style': 'form', 'explode': True }})
    r"""Enable or disable safe search"""  
    sort: Optional[BulkSearchImagesSortEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'sort', 'style': 'form', 'explode': True }})
    r"""Sort by"""  
    spellcheck_query: Optional[bool] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'spellcheck_query', 'style': 'form', 'explode': True }})
    r"""Spellcheck the search query and return results on suggested spellings"""  
    view: Optional[BulkSearchImagesViewEnum] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'view', 'style': 'form', 'explode': True }})
    r"""Amount of detail to render in the response"""  
    width: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'width', 'style': 'form', 'explode': True }})
    r"""(Deprecated; use width_from and width_to instead) Show images with the specified width"""  
    width_from: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'width_from', 'style': 'form', 'explode': True }})
    r"""Show images with the specified width or larger, in pixels"""  
    width_to: Optional[int] = dataclasses.field(default=None, metadata={'query_param': { 'field_name': 'width_to', 'style': 'form', 'explode': True }})
    r"""Show images with the specified width or smaller, in pixels"""  
    

@dataclasses.dataclass
class BulkSearchImagesResponse:
    
    content_type: str = dataclasses.field()  
    status_code: int = dataclasses.field()  
    bulk_image_search_results: Optional[shared_bulkimagesearchresults.BulkImageSearchResults] = dataclasses.field(default=None)
    r"""OK"""  
    raw_response: Optional[requests_http.Response] = dataclasses.field(default=None)  
    