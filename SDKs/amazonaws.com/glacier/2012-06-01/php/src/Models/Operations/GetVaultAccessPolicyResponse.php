<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;


class GetVaultAccessPolicyResponse
{
	
    public string $contentType;
    
    /**
     * Success
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Shared\GetVaultAccessPolicyOutput $getVaultAccessPolicyOutput
     */
	
    public ?\OpenAPI\OpenAPI\Models\Shared\GetVaultAccessPolicyOutput $getVaultAccessPolicyOutput = null;
    
    /**
     * InvalidParameterValueException
     * 
     * @var mixed $invalidParameterValueException
     */
	
    public mixed $invalidParameterValueException = null;
    
    /**
     * MissingParameterValueException
     * 
     * @var mixed $missingParameterValueException
     */
	
    public mixed $missingParameterValueException = null;
    
    /**
     * ResourceNotFoundException
     * 
     * @var mixed $resourceNotFoundException
     */
	
    public mixed $resourceNotFoundException = null;
    
    /**
     * ServiceUnavailableException
     * 
     * @var mixed $serviceUnavailableException
     */
	
    public mixed $serviceUnavailableException = null;
    
	
    public int $statusCode;
    
	
    public ?\Psr\Http\Message\ResponseInterface $rawResponse = null;
    
	public function __construct()
	{
		$this->contentType = "";
		$this->getVaultAccessPolicyOutput = null;
		$this->invalidParameterValueException = null;
		$this->missingParameterValueException = null;
		$this->resourceNotFoundException = null;
		$this->serviceUnavailableException = null;
		$this->statusCode = 0;
		$this->rawResponse = null;
	}
}
