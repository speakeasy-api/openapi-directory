...
export class Previews {
  ...
  previewsRead(
    ...
    const url: string = utils.generateURL(baseURL, "/previews/{file_id}/", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.PreviewsReadResponse =
        new operations.PreviewsReadResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.previewResponse = utils.objectToClass(
              httpRes?.data,
              shared.PreviewResponse
            );
          }
          break;
