...
class BatchCreateTableRowsRequestBody:
    ...
    rows_to_create: list[shared_createrowdata.CreateRowData] = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('rowsToCreate') }})
    r"""<p> The list of rows to create at the end of the table. Each item in this list needs to have a batch item id to uniquely identify the element in the request and the cells to create for that row. You need to specify at least one item in this list. </p> <p> Note that if one of the column ids in any of the rows in the request does not exist in the table, then the request fails and no updates are made to the table. </p>"""  
...
class BatchCreateTableRowsRequest:
    ...
    request_body: BatchCreateTableRowsRequestBody = dataclasses.field(metadata={'request': { 'media_type': 'application/json' }})  
    table_id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'tableId', 'style': 'simple', 'explode': False }})
    r"""<p>The ID of the table where the new rows are being added.</p> <p> If a table with the specified ID could not be found, this API throws ResourceNotFoundException. </p>"""  
    workbook_id: str = dataclasses.field(metadata={'path_param': { 'field_name': 'workbookId', 'style': 'simple', 'explode': False }})
    r"""<p>The ID of the workbook where the new rows are being added.</p> <p> If a workbook with the specified ID could not be found, this API throws ResourceNotFoundException. </p>"""  
