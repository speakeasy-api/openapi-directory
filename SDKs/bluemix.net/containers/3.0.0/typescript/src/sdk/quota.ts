...
export class Quota {
  ...
  getContainersQuota(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetContainersQuotaResponse =
        new operations.GetContainersQuotaResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.containersQuotaInfo = utils.objectToClass(
              httpRes?.data,
              shared.ContainersQuotaInfo
            );
          }
          break;
  ...
  getContainersUsage(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.GetContainersUsageResponse =
        new operations.GetContainersUsageResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.containersUsageInfo = utils.objectToClass(
              httpRes?.data,
              shared.ContainersUsageInfo
            );
          }
          break;
        case [401, 500].includes(httpRes?.status):
  ...
  putContainersQuota(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.PutContainersQuotaResponse =
        new operations.PutContainersQuotaResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case [204, 400, 401, 500].includes(httpRes?.status):
          break;
      }
