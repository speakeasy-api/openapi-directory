...
export class CaseClassifications {
  ...
  cloudsupportCaseClassificationsSearch(
    ...
    const url: string =
      baseURL.replace(/\/$/, "") + "/v2beta/caseClassifications:search";
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.CloudsupportCaseClassificationsSearchSecurity(
        security
      );
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.CloudsupportCaseClassificationsSearchResponse =
        new operations.CloudsupportCaseClassificationsSearchResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.searchCaseClassificationsResponse = utils.objectToClass(
              httpRes?.data,
              shared.SearchCaseClassificationsResponse
            );
          }
          break;
      }
