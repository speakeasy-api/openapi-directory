...
export class AccountActiveAdSummaries {
  ...
  dfareportingAccountActiveAdSummariesGet(
    ...
    if (!(security instanceof utils.SpeakeasyBase)) {
      security = new operations.DfareportingAccountActiveAdSummariesGetSecurity(
        security
      );
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.DfareportingAccountActiveAdSummariesGetResponse =
        new operations.DfareportingAccountActiveAdSummariesGetResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.accountActiveAdSummary = utils.objectToClass(
              httpRes?.data,
              shared.AccountActiveAdSummary
            );
          }
          break;
      }
