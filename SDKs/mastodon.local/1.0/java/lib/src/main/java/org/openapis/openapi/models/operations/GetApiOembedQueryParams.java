/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;

import org.openapis.openapi.utils.SpeakeasyMetadata;

public class GetApiOembedQueryParams {
    /**
     * height of the iframe. Defaults to null
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=maxheight")
    public Long maxheight;
    public GetApiOembedQueryParams withMaxheight(Long maxheight) {
        this.maxheight = maxheight;
        return this;
    }
    
    /**
     * width of the iframe. Defaults to 400
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=maxwidth")
    public Long maxwidth;
    public GetApiOembedQueryParams withMaxwidth(Long maxwidth) {
        this.maxwidth = maxwidth;
        return this;
    }
    
    /**
     * URL of a status
     */
    @SpeakeasyMetadata("queryParam:style=form,explode=true,name=url")
    public String url;
    public GetApiOembedQueryParams withUrl(String url) {
        this.url = url;
        return this;
    }
    
}
