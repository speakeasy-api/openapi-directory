...
class VideosVersions:
    ...
    def create_video_version(self, request: operations.CreateVideoVersionRequest) -> operations.CreateVideoVersionResponse:
        ...
        url = utils.generate_url(operations.CreateVideoVersionRequest, base_url, '/videos/{video_id}/versions', request)
        ...
        res = operations.CreateVideoVersionResponse(status_code=http_res.status_code, content_type=content_type, raw_response=http_res)
        
        if http_res.status_code == 201:
            if utils.match_content_type(content_type, 'application/vnd.vimeo.video.version+json'):
                out = utils.unmarshal_json(http_res.text, Optional[shared.VideoVersions])
