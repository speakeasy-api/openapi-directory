<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * StatusCheckPolicy - Status Check Policy
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class StatusCheckPolicy
{
    /**
     * $checks
     * 
     * @var array<\OpenAPI\OpenAPI\Models\Shared\StatusCheckPolicyChecks> $checks
     */
	#[\JMS\Serializer\Annotation\SerializedName('checks')]
    #[\JMS\Serializer\Annotation\Type('array<OpenAPI\OpenAPI\Models\Shared\StatusCheckPolicyChecks>')]
    public array $checks;
    
    /**
     * $contexts
     * 
     * @var array<string> $contexts
     */
	#[\JMS\Serializer\Annotation\SerializedName('contexts')]
    #[\JMS\Serializer\Annotation\Type('array<string>')]
    public array $contexts;
    
	#[\JMS\Serializer\Annotation\SerializedName('contexts_url')]
    #[\JMS\Serializer\Annotation\Type('string')]
    public string $contextsUrl;
    
	#[\JMS\Serializer\Annotation\SerializedName('strict')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    public bool $strict;
    
	#[\JMS\Serializer\Annotation\SerializedName('url')]
    #[\JMS\Serializer\Annotation\Type('string')]
    public string $url;
    
	public function __construct()
	{
		$this->checks = [];
		$this->contexts = [];
		$this->contextsUrl = "";
		$this->strict = false;
		$this->url = "";
	}
}
