/* 
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

package org.openapis.openapi.models.operations;



public class GetGroupLockSettings200ApplicationXML1EmailNotification {
    /**
     * Notify the alternative host who is set or removed.
     */
    
    public Boolean alternativeHostReminder;
    public GetGroupLockSettings200ApplicationXML1EmailNotification withAlternativeHostReminder(Boolean alternativeHostReminder) {
        this.alternativeHostReminder = alternativeHostReminder;
        return this;
    }
    
    /**
     * Notify host and participants when the meeting is cancelled.
     */
    
    public Boolean cancelMeetingReminder;
    public GetGroupLockSettings200ApplicationXML1EmailNotification withCancelMeetingReminder(Boolean cancelMeetingReminder) {
        this.cancelMeetingReminder = cancelMeetingReminder;
        return this;
    }
    
    /**
     * Notify host when cloud recording is available.
     */
    
    public Boolean cloudRecordingAvailableReminder;
    public GetGroupLockSettings200ApplicationXML1EmailNotification withCloudRecordingAvailableReminder(Boolean cloudRecordingAvailableReminder) {
        this.cloudRecordingAvailableReminder = cloudRecordingAvailableReminder;
        return this;
    }
    
    /**
     * Notify host when participants join the meeting before them.
     */
    
    public Boolean jbhReminder;
    public GetGroupLockSettings200ApplicationXML1EmailNotification withJbhReminder(Boolean jbhReminder) {
        this.jbhReminder = jbhReminder;
        return this;
    }
    
    /**
     * Notify the host there is a meeting is scheduled, rescheduled, or cancelled.
     */
    
    public Boolean scheduleForHostReminder;
    public GetGroupLockSettings200ApplicationXML1EmailNotification withScheduleForHostReminder(Boolean scheduleForHostReminder) {
        this.scheduleForHostReminder = scheduleForHostReminder;
        return this;
    }
    
}
