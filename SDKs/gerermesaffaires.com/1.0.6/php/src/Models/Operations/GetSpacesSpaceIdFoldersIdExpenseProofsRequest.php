<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class GetSpacesSpaceIdFoldersIdExpenseProofsRequest
{
    /**
     * date range of the documents
     * 
     * @var ?string $date
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=Date')]
    public ?string $date = null;
    
    /**
     * date range of attachment
     * 
     * @var ?string $folderDate
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=FolderDate')]
    public ?string $folderDate = null;
    
    /**
     * To return expense proofs not attached to an expense report
     * 
     * @var ?bool $noExpenseReport
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=NoExpenseReport')]
    public ?bool $noExpenseReport = null;
    
    /**
     * index range of the results
     * 
     * @var ?string $range
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=Range')]
    public ?string $range = null;
    
    /**
     * Status of the expense proof
     * 
     * @var ?\OpenAPI\OpenAPI\Models\Operations\GetSpacesSpaceIdFoldersIdExpenseProofsStatusEnum $status
     */
	#[SpeakeasyMetadata('queryParam:style=form,explode=true,name=Status')]
    public ?GetSpacesSpaceIdFoldersIdExpenseProofsStatusEnum $status = null;
    
    /**
     * Id of the folder
     * 
     * @var string $id
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=id')]
    public string $id;
    
    /**
     * Id of the space
     * 
     * @var string $spaceId
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=spaceId')]
    public string $spaceId;
    
	public function __construct()
	{
		$this->date = null;
		$this->folderDate = null;
		$this->noExpenseReport = null;
		$this->range = null;
		$this->status = null;
		$this->id = "";
		$this->spaceId = "";
	}
}
