<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class SendDownloadShareLinkViaEmailRequest
{
	#[SpeakeasyMetadata('request:mediaType=application/json')]
    public \OpenAPI\OpenAPI\Models\Shared\DownloadShareLinkEmail $downloadShareLinkEmail;
    
    /**
     * Authentication token
     * 
     * @var ?string $xSdsAuthToken
     */
	#[SpeakeasyMetadata('header:style=simple,explode=false,name=X-Sds-Auth-Token')]
    public ?string $xSdsAuthToken = null;
    
    /**
     * Share ID
     * 
     * @var int $shareId
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=share_id')]
    public int $shareId;
    
	public function __construct()
	{
		$this->downloadShareLinkEmail = new \OpenAPI\OpenAPI\Models\Shared\DownloadShareLinkEmail();
		$this->xSdsAuthToken = null;
		$this->shareId = 0;
	}
}
