// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
	"openapi/pkg/types"
	"time"
)

// OracleDbDetailSLAAssignmentEnum - The SLA assignment type. Direct SLA assignment means that a SLA Domain was configured directly on the Rubrik object by the user. Derived SLA assignment means that the Rubrik object inherits an SLA Domain from its parent Rubrik object.
type OracleDbDetailSLAAssignmentEnum string

const (
	OracleDbDetailSLAAssignmentEnumDerived    OracleDbDetailSLAAssignmentEnum = "Derived"
	OracleDbDetailSLAAssignmentEnumDirect     OracleDbDetailSLAAssignmentEnum = "Direct"
	OracleDbDetailSLAAssignmentEnumUnassigned OracleDbDetailSLAAssignmentEnum = "Unassigned"
)

func (e OracleDbDetailSLAAssignmentEnum) ToPointer() *OracleDbDetailSLAAssignmentEnum {
	return &e
}

func (e *OracleDbDetailSLAAssignmentEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "Derived":
		fallthrough
	case "Direct":
		fallthrough
	case "Unassigned":
		*e = OracleDbDetailSLAAssignmentEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for OracleDbDetailSLAAssignmentEnum: %v", v)
	}
}

// OracleDbDetail - Successfully updated the Oracle Data Guard group object.
type OracleDbDetail struct {
	// An array that contains the archive log destinations for the specified Oracle database.
	ArchiveLogDestinations []string `json:"archiveLogDestinations,omitempty"`
	// The ID of the SLA Domain configured directly on the Rubrik object.
	ConfiguredSLADomainID string `json:"configuredSlaDomainId"`
	// The name of the SLA Domain configured directly on the Rubrik object.
	ConfiguredSLADomainName string `json:"configuredSlaDomainName"`
	// Specifies whether the SLA Domain is used for protection or retention.
	ConfiguredSLADomainType *ConfiguredSLATypeEnum    `json:"configuredSlaDomainType,omitempty"`
	CurrentBackupTaskInfo   *BackupTaskDiagnosticInfo `json:"currentBackupTaskInfo,omitempty"`
	// Rubrik ID of the Data Guard group to which this database belongs.
	DataGuardGroupID *string `json:"dataGuardGroupId,omitempty"`
	// List of Data Guard group members.
	DataGuardGroupMembers []DataGuardGroupMember `json:"dataGuardGroupMembers,omitempty"`
	// Name of the Data Guard group to which this database belongs.
	DataGuardGroupName *string `json:"dataGuardGroupName,omitempty"`
	// Specifies whether this database is a Data Guard member, Data Guard group, or a non-Data Guard database.
	DataGuardType *DataGuardTypeEnum `json:"dataGuardType,omitempty"`
	// Current role of the database.
	DatabaseRole *string `json:"databaseRole,omitempty"`
	// Unique name for the Oracle database (DB_UNIQUE_NAME).
	DbUniqueName *string `json:"dbUniqueName,omitempty"`
	// The ID of the SLA Domain that controls the protection of the Rubrik object.
	EffectiveSLADomainID string `json:"effectiveSlaDomainId"`
	// The name of the SLA Domain that controls the protection of the Rubrik object.
	EffectiveSLADomainName string `json:"effectiveSlaDomainName"`
	// Optional. This field contains the managed ID of of the Polaris-managed effective SLA Domain.
	EffectiveSLADomainPolarisManagedID *string `json:"effectiveSlaDomainPolarisManagedId,omitempty"`
	// The ID of the parent of the Rubrik object from which the SLA Domain that controls the protection of Rubrik object is inherited.
	EffectiveSLASourceObjectID *string `json:"effectiveSlaSourceObjectId,omitempty"`
	// The name of the parent of the Rubrik object from which the SLA Domain that controls the protection of Rubrik object is inherited.
	EffectiveSLASourceObjectName *string `json:"effectiveSlaSourceObjectName,omitempty"`
	// Specifies an interval in hours. The next log snapshot job deletes archived Oracle redo log files whose 'nextTime' field specifies a time more than the specified number of hours ago. To immediately delete archived redo log files regardless of age, specify an interval of -1. To preserve all archived redo log files, specify an interval of -2.
	HostLogRetentionHours int `json:"hostLogRetentionHours"`
	// Path where the NFS share is mounted on the host.
	HostMount string `json:"hostMount"`
	// An array that contains the host info for each instance.
	HostsInfo []HostInfo `json:"hostsInfo,omitempty"`
	// The ID of the Rubrik object.
	ID string `json:"id"`
	// True/false value indicating if backup task information is included in the response.
	IncludeBackupTaskInfo *bool `json:"includeBackupTaskInfo,omitempty"`
	// An array that contains information about the objects in the infrastructure path of a specified Oracle database.
	InfraPath []ManagedHierarchyObjectAncestor `json:"infraPath"`
	// Details of the instances of the Oracle database.
	Instances []OracleInstanceProperties `json:"instances,omitempty"`
	// Boolean value that indicates whether the ARCHIVELOG mode is enabled on the Oracle database or not.
	IsArchiveLogModeEnabled *bool `json:"isArchiveLogModeEnabled,omitempty"`
	// Indicates whether the configured SLA Domain is Retention Locked. When this value is 'true', the configured SLA Domain is a Retention Lock SLA Domain.
	IsConfiguredSLADomainRetentionLocked *bool `json:"isConfiguredSlaDomainRetentionLocked,omitempty"`
	// A Boolean value that specifies whether the Oracle database is local to the cluster. When this value is 'true', the Oracle database is local to the cluster.
	IsDbLocalToTheCluster bool `json:"isDbLocalToTheCluster"`
	// Indicates whether the effective SLA Domain is Retention Locked. When this value is 'true', the effective SLA domain is a Retention Lock SLA Domain.
	IsEffectiveSLADomainRetentionLocked *bool `json:"isEffectiveSlaDomainRetentionLocked,omitempty"`
	// Value that indicates whether an Oracle database object is a Live Mount or not. A true value indicates that the object is a Live Mount.
	IsLiveMount *bool `json:"isLiveMount,omitempty"`
	// Indicates whether the current DATABASE_ROLE is PRIMARY which specifies the database is accepting read/write transactions as the primary database in a Data Guard configuration.
	IsPrimary *bool `json:"isPrimary,omitempty"`
	// Boolean value that indicates whether a Oracle database object is in an archived state and has retained snapshots. Value is true when the object is archived with retained snapshots.
	IsRelic bool `json:"isRelic"`
	// The timestamp of the previous snapshot.
	LastSnapshotTime     *time.Time                  `json:"lastSnapshotTime,omitempty"`
	LastValidationResult *OracleLastValidationResult `json:"lastValidationResult,omitempty"`
	// The time stamp of the most recent recovery point for this database.
	LatestRecoveryPoint *types.Date `json:"latestRecoveryPoint,omitempty"`
	// Specifies an interval in minutes. This interval is the period between successive log backups.
	LogBackupFrequencyInMinutes int `json:"logBackupFrequencyInMinutes"`
	// Specifies an interval in hours. Log backups are retained for the duration of the interval.
	LogRetentionHours int `json:"logRetentionHours"`
	// The name of the Rubrik object.
	Name string `json:"name"`
	// Number of channels used to backup the Oracle database.
	NumChannels int `json:"numChannels"`
	// Count of the number of instances of the Oracle database.
	NumInstances *int `json:"numInstances,omitempty"`
	// An integer that specifies the number of missed snapshots.
	NumMissedSnapshot int `json:"numMissedSnapshot"`
	// Count of the number of table spaces in Oracle database.
	NumTablespaces int `json:"numTablespaces"`
	// The time stamp of the earliest recovery point for this database.
	OldestRecoveryPoint *types.Date `json:"oldestRecoveryPoint,omitempty"`
	// Oracle Home of the Oracle database.
	OracleHome       *string                      `json:"oracleHome,omitempty"`
	PendingSLADomain *ManagedObjectPendingSLAInfo `json:"pendingSlaDomain,omitempty"`
	// Ordered list of database unique names to use for backup in a Data Guard group.
	PreferredDGMemberUniqueNames []string `json:"preferredDGMemberUniqueNames,omitempty"`
	// The ID of the cluster that manages the Rubrik object.
	PrimaryClusterID string `json:"primaryClusterId"`
	// Rubrik ID of the RAC on which this database is hosted. This field will be empty if the database is not hosted on a RAC environment.
	RacID *string `json:"racId,omitempty"`
	// RAC name of the cluster database.
	RacName *string `json:"racName,omitempty"`
	// The ID of the SLA Domain whose retention policy is in use.
	RetentionSLADomainID *string `json:"retentionSlaDomainId,omitempty"`
	// Indicates whether to backup only from the PRIMARY Data Guard member or from any available member.
	ShouldBackupFromPrimaryDGGroupMemberOnly *bool `json:"shouldBackupFromPrimaryDGGroupMemberOnly,omitempty"`
	// System identifier (SID) of the Oracle database.
	Sid *string `json:"sid,omitempty"`
	// The SLA assignment type. Direct SLA assignment means that a SLA Domain was configured directly on the Rubrik object by the user. Derived SLA assignment means that the Rubrik object inherits an SLA Domain from its parent Rubrik object.
	SLAAssignment OracleDbDetailSLAAssignmentEnum `json:"slaAssignment"`
	// The UTC time when the SLA Domain was last updated.
	SLALastUpdateTime *time.Time `json:"slaLastUpdateTime,omitempty"`
	SnapshotCount     int        `json:"snapshotCount"`
	// Rubrik ID of the standalone Oracle host on which this database is hosted. This field will be empty if the database is not hosted on a standalone system.
	StandaloneHostID *string `json:"standaloneHostId,omitempty"`
	// Hostname of the standalone Oracle database host.
	StandaloneHostName *string `json:"standaloneHostName,omitempty"`
	// An array that contains tablespace names of the specified Oracle database.
	Tablespaces []string `json:"tablespaces"`
}
