...
export class Fleet {
  ...
  getFleetFleetname(
    ...
    const url: string = utils.generateURL(baseURL, "/fleet/{fleetname}", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.GetFleetFleetnameResponse =
        new operations.GetFleetFleetnameResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.getFleetFleetname200ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.GetFleetFleetname200ApplicationJSON
            );
          }
          break;
      }
