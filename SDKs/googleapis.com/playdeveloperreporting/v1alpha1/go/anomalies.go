...
func (s *Anomalies) PlaydeveloperreportingAnomaliesList(ctx context.Context, request operations.PlaydeveloperreportingAnomaliesListRequest) (*operations.PlaydeveloperreportingAnomaliesListResponse, error) {
	...
	url := utils.GenerateURL(ctx, baseURL, "/v1alpha1/{parent}/anomalies", request.PathParams)
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.GooglePlayDeveloperReportingV1alpha1ListAnomaliesResponse
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}

			res.GooglePlayDeveloperReportingV1alpha1ListAnomaliesResponse = out
		}
	}
