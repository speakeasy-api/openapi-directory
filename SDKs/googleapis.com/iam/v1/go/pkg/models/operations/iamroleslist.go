...
type IamRolesListSecurity struct {
	Oauth2  shared.SchemeOauth2  `security:"scheme,type=oauth2"`
	Oauth2c shared.SchemeOauth2c `security:"scheme,type=oauth2"`
}

// IamRolesListViewEnum - Optional view for the returned Role objects. When `FULL` is specified, the `includedPermissions` field is returned, which includes a list of all permissions in the role. The default value is `BASIC`, which does not return the `includedPermissions` field.
type IamRolesListViewEnum string

const (
	IamRolesListViewEnumBasic IamRolesListViewEnum = "BASIC"
	IamRolesListViewEnumFull  IamRolesListViewEnum = "FULL"
)

func (e *IamRolesListViewEnum) UnmarshalJSON(data []byte) error {
	var s string
	...
	case "BASIC":
		fallthrough
	case "FULL":
		*e = IamRolesListViewEnum(s)
		return nil
	default:
		return fmt.Errorf("invalid value for IamRolesListViewEnum: %s", s)
	}
}

type IamRolesListQueryParams struct {
	// V1 error format.
	...
	View *IamRolesListViewEnum `queryParam:"style=form,explode=true,name=view"`
}

type IamRolesListRequest struct {
