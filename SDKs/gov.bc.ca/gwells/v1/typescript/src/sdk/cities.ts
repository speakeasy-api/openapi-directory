...
export class Cities {
  ...
  citiesDrillersList(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.CitiesDrillersListResponse =
        new operations.CitiesDrillersListResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            ...
            res.cityLists = utils.objectToClass(
              httpRes?.data,
              shared.CityList,
              resFieldDepth
            );
  ...
  citiesInstallersList(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.CitiesInstallersListResponse =
        new operations.CitiesInstallersListResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
