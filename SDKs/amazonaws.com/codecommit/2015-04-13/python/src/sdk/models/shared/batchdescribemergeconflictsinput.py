"""Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT."""

from __future__ import annotations
import dataclasses
from ..shared import conflictdetailleveltypeenum_enum as shared_conflictdetailleveltypeenum_enum
from ..shared import conflictresolutionstrategytypeenum_enum as shared_conflictresolutionstrategytypeenum_enum
from ..shared import mergeoptiontypeenum_enum as shared_mergeoptiontypeenum_enum
from dataclasses_json import Undefined, dataclass_json
from sdk import utils
from typing import Optional


@dataclass_json(undefined=Undefined.EXCLUDE)
@dataclasses.dataclass
class BatchDescribeMergeConflictsInput:
    
    destination_commit_specifier: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('destinationCommitSpecifier') }})  
    merge_option: shared_mergeoptiontypeenum_enum.MergeOptionTypeEnumEnum = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('mergeOption') }})  
    repository_name: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('repositoryName') }})  
    source_commit_specifier: str = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('sourceCommitSpecifier') }})  
    conflict_detail_level: Optional[shared_conflictdetailleveltypeenum_enum.ConflictDetailLevelTypeEnumEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('conflictDetailLevel'), 'exclude': lambda f: f is None }})  
    conflict_resolution_strategy: Optional[shared_conflictresolutionstrategytypeenum_enum.ConflictResolutionStrategyTypeEnumEnum] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('conflictResolutionStrategy'), 'exclude': lambda f: f is None }})  
    file_paths: Optional[list[str]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('filePaths'), 'exclude': lambda f: f is None }})  
    max_conflict_files: Optional[int] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('maxConflictFiles'), 'exclude': lambda f: f is None }})  
    max_merge_hunks: Optional[int] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('maxMergeHunks'), 'exclude': lambda f: f is None }})  
    next_token: Optional[str] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('nextToken'), 'exclude': lambda f: f is None }})  
    