// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package shared

import (
	"encoding/json"
	"fmt"
)

type ControlResponseEnum string

const (
	ControlResponseEnumManual   ControlResponseEnum = "MANUAL"
	ControlResponseEnumAutomate ControlResponseEnum = "AUTOMATE"
	ControlResponseEnumDefer    ControlResponseEnum = "DEFER"
	ControlResponseEnumIgnore   ControlResponseEnum = "IGNORE"
)

func (e ControlResponseEnum) ToPointer() *ControlResponseEnum {
	return &e
}

func (e *ControlResponseEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "MANUAL":
		fallthrough
	case "AUTOMATE":
		fallthrough
	case "DEFER":
		fallthrough
	case "IGNORE":
		*e = ControlResponseEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for ControlResponseEnum: %v", v)
	}
}
