/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SpeakeasyBase, SpeakeasyMetadata } from "../../../internal/utils";
import { AdditionalSettings } from "./additionalsettings";
import { Expose, Type } from "class-transformer";

/**
 * Format or protocol for receiving webhooks. Possible values:
 *
 * @remarks
 * * **soap**
 * * **http**
 * * **json**
 */
export enum UpdateMerchantWebhookRequestCommunicationFormatEnum {
  Http = "http",
  Json = "json",
  Soap = "soap",
}

/**
 * Network type for Terminal API notification webhooks. Possible values:
 *
 * @remarks
 * * **public**
 * * **local**
 *
 * Default Value: **public**.
 */
export enum UpdateMerchantWebhookRequestNetworkTypeEnum {
  Local = "LOCAL",
  Public = "PUBLIC",
}

/**
 * SSL version to access the public webhook URL specified in the `url` field. Possible values:
 *
 * @remarks
 * * **TLSv1.3**
 * * **TLSv1.2**
 * * **HTTP** - Only allowed on Test environment.
 *
 * If not specified, the webhook will use `sslVersion`: **TLSv1.2**.
 */
export enum UpdateMerchantWebhookRequestSslVersionEnum {
  Http = "HTTP",
  Ssl = "SSL",
  SSLv3 = "SSLv3",
  Tls = "TLS",
  TLSv1 = "TLSv1",
  TLSv11 = "TLSv1.1",
  TLSv12 = "TLSv1.2",
  TLSv13 = "TLSv1.3",
}

export class UpdateMerchantWebhookRequest extends SpeakeasyBase {
  /**
   * Indicates if expired SSL certificates are accepted. Default value: **false**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "acceptsExpiredCertificate" })
  acceptsExpiredCertificate?: boolean;

  /**
   * Indicates if self-signed SSL certificates are accepted. Default value: **false**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "acceptsSelfSignedCertificate" })
  acceptsSelfSignedCertificate?: boolean;

  /**
   * Indicates if untrusted SSL certificates are accepted. Default value: **false**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "acceptsUntrustedRootCertificate" })
  acceptsUntrustedRootCertificate?: boolean;

  /**
   * Indicates if the webhook configuration is active. The field must be **true** for us to send webhooks about events related an account.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "active" })
  active?: boolean;

  @SpeakeasyMetadata()
  @Expose({ name: "additionalSettings" })
  @Type(() => AdditionalSettings)
  additionalSettings?: AdditionalSettings;

  /**
   * Format or protocol for receiving webhooks. Possible values:
   *
   * @remarks
   * * **soap**
   * * **http**
   * * **json**
   */
  @SpeakeasyMetadata()
  @Expose({ name: "communicationFormat" })
  communicationFormat?: UpdateMerchantWebhookRequestCommunicationFormatEnum;

  /**
   * Your description for this webhook configuration.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "description" })
  description?: string;

  /**
   * Network type for Terminal API notification webhooks. Possible values:
   *
   * @remarks
   * * **public**
   * * **local**
   *
   * Default Value: **public**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "networkType" })
  networkType?: UpdateMerchantWebhookRequestNetworkTypeEnum;

  /**
   * Password to access the webhook URL.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "password" })
  password?: string;

  /**
   * Indicates if the SOAP action header needs to be populated. Default value: **false**.
   *
   * @remarks
   *
   * Only applies if `communicationFormat`: **soap**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "populateSoapActionHeader" })
  populateSoapActionHeader?: boolean;

  /**
   * SSL version to access the public webhook URL specified in the `url` field. Possible values:
   *
   * @remarks
   * * **TLSv1.3**
   * * **TLSv1.2**
   * * **HTTP** - Only allowed on Test environment.
   *
   * If not specified, the webhook will use `sslVersion`: **TLSv1.2**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "sslVersion" })
  sslVersion?: UpdateMerchantWebhookRequestSslVersionEnum;

  /**
   * Public URL where webhooks will be sent, for example **https://www.domain.com/webhook-endpoint**.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "url" })
  url?: string;

  /**
   * Username to access the webhook URL.
   */
  @SpeakeasyMetadata()
  @Expose({ name: "username" })
  username?: string;
}
