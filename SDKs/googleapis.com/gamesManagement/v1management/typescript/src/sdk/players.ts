  ...
  **/
  gamesManagementPlayersHide(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GamesManagementPlayersHideResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  gamesManagementPlayersUnhide(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.GamesManagementPlayersUnhideResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            break;
        }
