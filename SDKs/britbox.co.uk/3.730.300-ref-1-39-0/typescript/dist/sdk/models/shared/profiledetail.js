"use strict";
/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProfileDetail = void 0;
var utils_1 = require("../../../internal/utils");
var classificationsummary_1 = require("./classificationsummary");
var watched_1 = require("./watched");
var class_transformer_1 = require("class-transformer");
/**
 * Details of the active profile.
 */
var ProfileDetail = /** @class */ (function (_super) {
    __extends(ProfileDetail, _super);
    function ProfileDetail() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "bookmarked" }),
        __metadata("design:type", Object)
    ], ProfileDetail.prototype, "bookmarked", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "color" }),
        __metadata("design:type", String)
    ], ProfileDetail.prototype, "color", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "heroAutoplay" }),
        __metadata("design:type", Boolean)
    ], ProfileDetail.prototype, "heroAutoplay", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "heroWithAudio" }),
        __metadata("design:type", Boolean)
    ], ProfileDetail.prototype, "heroWithAudio", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "id" }),
        __metadata("design:type", String)
    ], ProfileDetail.prototype, "id", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "isActive" }),
        __metadata("design:type", Boolean)
    ], ProfileDetail.prototype, "isActive", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "languageCode" }),
        __metadata("design:type", String)
    ], ProfileDetail.prototype, "languageCode", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "marketingEnabled" }),
        __metadata("design:type", Boolean)
    ], ProfileDetail.prototype, "marketingEnabled", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "maxRatingContentFilter" }),
        (0, class_transformer_1.Type)(function () { return classificationsummary_1.ClassificationSummary; }),
        __metadata("design:type", classificationsummary_1.ClassificationSummary)
    ], ProfileDetail.prototype, "maxRatingContentFilter", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "minRatingPlaybackGuard" }),
        (0, class_transformer_1.Type)(function () { return classificationsummary_1.ClassificationSummary; }),
        __metadata("design:type", classificationsummary_1.ClassificationSummary)
    ], ProfileDetail.prototype, "minRatingPlaybackGuard", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "name" }),
        __metadata("design:type", String)
    ], ProfileDetail.prototype, "name", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "pinEnabled" }),
        __metadata("design:type", Boolean)
    ], ProfileDetail.prototype, "pinEnabled", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "purchaseEnabled" }),
        __metadata("design:type", Boolean)
    ], ProfileDetail.prototype, "purchaseEnabled", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "rated" }),
        __metadata("design:type", Object)
    ], ProfileDetail.prototype, "rated", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)(),
        (0, class_transformer_1.Expose)({ name: "segments" }),
        __metadata("design:type", Array)
    ], ProfileDetail.prototype, "segments", void 0);
    __decorate([
        (0, utils_1.SpeakeasyMetadata)({ elemType: watched_1.Watched }),
        (0, class_transformer_1.Expose)({ name: "watched" }),
        (0, class_transformer_1.Transform)(function (_a) {
            var value = _a.value;
            var obj = {};
            for (var key in value) {
                obj[key] = (0, utils_1.objectToClass)(value[key], watched_1.Watched);
            }
            return obj;
        }, { toClassOnly: true }),
        __metadata("design:type", Object)
    ], ProfileDetail.prototype, "watched", void 0);
    return ProfileDetail;
}(utils_1.SpeakeasyBase));
exports.ProfileDetail = ProfileDetail;
