...
use \OpenAPI\OpenAPI\Models\Operations\AllTaxRatesRequest;

$sdk = SDK::builder()
    ->build();

try {
    $request = new AllTaxRatesRequest();
    $request->productCode = 'C012';
    $request->cursor = 'corrupti';
    $request->domain = '<string>';
    $request->filter = 'illum';

    $response = $sdk->v3Tax->allTaxRates($request);

    if ($response->allTaxRates200ApplicationJSONObjects !== null) {
        // handle response
    }
} catch (Exception $e) {
    // handle exception
