<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;


/**
 * LeadSourceDTO - origin of a customer (ie. recomendation from another customer)
 * 
 * @package OpenAPI\OpenAPI\Models\Shared
 * @access public
 */
class LeadSourceDTO
{
    /**
     * should this value be available on the XTRF selection lists
     * 
     * @var ?bool $active
     */
	#[\JMS\Serializer\Annotation\SerializedName('active')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $active = null;
    
    /**
     * should be available for customer
     * 
     * @var ?bool $availableForCustomer
     */
	#[\JMS\Serializer\Annotation\SerializedName('availableForCustomer')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $availableForCustomer = null;
    
    /**
     * should be available for provider
     * 
     * @var ?bool $availableForProvider
     */
	#[\JMS\Serializer\Annotation\SerializedName('availableForProvider')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $availableForProvider = null;
    
    /**
     * should this value be set by default in XTRF selection lists
     * 
     * @var ?bool $default
     */
	#[\JMS\Serializer\Annotation\SerializedName('default')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $default = null;
    
    /**
     * internal identifier
     * 
     * @var ?int $id
     */
	#[\JMS\Serializer\Annotation\SerializedName('id')]
    #[\JMS\Serializer\Annotation\Type('int')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?int $id = null;
    
    /**
     * localised name (formatted in the current user's locale)
     * 
     * @var ?string $name
     */
	#[\JMS\Serializer\Annotation\SerializedName('name')]
    #[\JMS\Serializer\Annotation\Type('string')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?string $name = null;
    
    /**
     * should this value be available on the top of XTRF selection lists, in the Preferred section
     * 
     * @var ?bool $preferred
     */
	#[\JMS\Serializer\Annotation\SerializedName('preferred')]
    #[\JMS\Serializer\Annotation\Type('bool')]
    #[\JMS\Serializer\Annotation\SkipWhenEmpty]
    public ?bool $preferred = null;
    
	public function __construct()
	{
		$this->active = null;
		$this->availableForCustomer = null;
		$this->availableForProvider = null;
		$this->default = null;
		$this->id = null;
		$this->name = null;
		$this->preferred = null;
	}
}
