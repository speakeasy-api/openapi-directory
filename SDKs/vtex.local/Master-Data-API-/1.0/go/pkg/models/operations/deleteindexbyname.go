// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"net/http"
)

type DeleteindexbynamePathParams struct {
	// Name of the data entity. Defined by the api. Examples of native data entities you can use are `CL` for client profiles and `AD` for client addresses.
	DataEntityName string `pathParam:"style=simple,explode=false,name=dataEntityName"`
	// Name of the index.
	IndexName string `pathParam:"style=simple,explode=false,name=index_name"`
}

type DeleteindexbynameHeaders struct {
	// Type of the content being sent.
	ContentType string `header:"style=simple,explode=false,name=Content-Type"`
}

type DeleteindexbynameRequest struct {
	PathParams DeleteindexbynamePathParams
	Headers    DeleteindexbynameHeaders
}

type DeleteindexbynameResponse struct {
	ContentType string
	StatusCode  int
	RawResponse *http.Response
}
