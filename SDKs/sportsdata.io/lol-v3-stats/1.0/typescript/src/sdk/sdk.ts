...
export class SDK {
  ...
  champions(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/{format}/Champions", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.ChampionsResponse =
        new operations.ChampionsResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.champions = [];
            const resFieldDepth: number = utils.getResFieldDepth(res);
            res.champions = utils.objectToClass(
              httpRes?.data,
              shared.Champion,
              resFieldDepth
            );
  ...
  items(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/{format}/Items", req);
  ...
  spells(
    ...
    const baseURL: string = this._serverURL;
    const url: string = utils.generateURL(baseURL, "/{format}/Spells", req);
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.SpellsResponse = new operations.SpellsResponse({
        statusCode: httpRes.status,
        contentType: contentType,
        rawResponse: httpRes,
      });
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.spells = [];
            const resFieldDepth: number = utils.getResFieldDepth(res);
            res.spells = utils.objectToClass(
              httpRes?.data,
              shared.Spell,
              resFieldDepth
            );
