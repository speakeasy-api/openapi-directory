// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"net/http"
)

type ResistanceAndSupportQueryParams struct {
	EndDate   *string `queryParam:"style=form,explode=true,name=endDate"`
	Limit     *string `queryParam:"style=form,explode=true,name=limit"`
	StartDate *string `queryParam:"style=form,explode=true,name=startDate"`
	Tokens    *string `queryParam:"style=form,explode=true,name=tokens"`
}

type ResistanceAndSupportRequest struct {
	QueryParams ResistanceAndSupportQueryParams
}

type ResistanceAndSupportResponse struct {
	ContentType string
	StatusCode  int
	RawResponse *http.Response
}
