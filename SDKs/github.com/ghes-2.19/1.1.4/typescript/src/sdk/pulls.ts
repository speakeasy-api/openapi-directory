...
export class Pulls {
  ...
  pullsCreateReviewCommentAlternative(
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.PullsCreateReviewCommentAlternativeResponse =
        new operations.PullsCreateReviewCommentAlternativeResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
          headers: utils.getHeadersFromResponse(httpRes.headers),
      ...
      switch (true) {
        case httpRes?.status == 201:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.legacyReviewComment = utils.objectToClass(
              httpRes?.data,
              shared.LegacyReviewComment
            );
          }
          break;
      }
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      switch (true) {
        ...
        case httpRes?.status == 415:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.pullsUpdateBranch415ApplicationJSONObject = utils.objectToClass(
              httpRes?.data,
              operations.PullsUpdateBranch415ApplicationJSON
            );
          }
          break;
        case httpRes?.status == 422:
