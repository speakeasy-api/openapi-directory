// Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.

package operations

import (
	"encoding/json"
	"fmt"
	"net/http"
	"openapi/pkg/models/shared"
)

// GetSimSearchMetricEnum - Metric for computing similarity
type GetSimSearchMetricEnum string

const (
	GetSimSearchMetricEnumPhenodigm       GetSimSearchMetricEnum = "phenodigm"
	GetSimSearchMetricEnumJaccard         GetSimSearchMetricEnum = "jaccard"
	GetSimSearchMetricEnumSimGic          GetSimSearchMetricEnum = "simGIC"
	GetSimSearchMetricEnumResnik          GetSimSearchMetricEnum = "resnik"
	GetSimSearchMetricEnumSymmetricResnik GetSimSearchMetricEnum = "symmetric_resnik"
)

func (e GetSimSearchMetricEnum) ToPointer() *GetSimSearchMetricEnum {
	return &e
}

func (e *GetSimSearchMetricEnum) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "phenodigm":
		fallthrough
	case "jaccard":
		fallthrough
	case "simGIC":
		fallthrough
	case "resnik":
		fallthrough
	case "symmetric_resnik":
		*e = GetSimSearchMetricEnum(v)
		return nil
	default:
		return fmt.Errorf("invalid value for GetSimSearchMetricEnum: %v", v)
	}
}

type GetSimSearchRequest struct {
	// A phenotype or identifier that is composed of phenotypes (eg disease, gene)
	ID []string `queryParam:"style=form,explode=true,name=id"`
	// set to true if *all* input ids are phenotypic features, else set to false
	IsFeatureSet *bool `queryParam:"style=form,explode=true,name=is_feature_set"`
	// number of rows, max 500
	Limit *int64 `queryParam:"style=form,explode=true,name=limit"`
	// Metric for computing similarity
	Metric *GetSimSearchMetricEnum `queryParam:"style=form,explode=true,name=metric"`
	// ncbi taxon id
	Taxon *string `queryParam:"style=form,explode=true,name=taxon"`
}

type GetSimSearchResponse struct {
	ContentType string
	// Success
	SimResult   *shared.SimResult
	StatusCode  int
	RawResponse *http.Response
}
