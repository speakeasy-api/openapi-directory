<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Operations;

use \OpenAPI\OpenAPI\Utils\SpeakeasyMetadata;
class GetNamespaceDataByTimespanRequest
{
    /**
     * Type of data to fetch
     * 
     * @var \OpenAPI\OpenAPI\Models\Shared\DataviewTypeEnum $dataview
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=dataview')]
    public \OpenAPI\OpenAPI\Models\Shared\DataviewTypeEnum $dataview;
    
    /**
     * Namespace to fetch data for
     * 
     * @var string $namespace
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=namespace')]
    public string $namespace;
    
    /**
     * Timespan to fetch data for
     * 
     * @var int $timespan
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=timespan')]
    public int $timespan;
    
    /**
     * Type of timespan to fetch data for
     * 
     * @var \OpenAPI\OpenAPI\Models\Shared\TimespanTypeEnum $timespantype
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=timespantype')]
    public \OpenAPI\OpenAPI\Models\Shared\TimespanTypeEnum $timespantype;
    
    /**
     * Year to fetch data for
     * 
     * @var int $year
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=year')]
    public int $year;
    
	public function __construct()
	{
		$this->dataview = \OpenAPI\OpenAPI\Models\Shared\DataviewTypeEnum::RAW;
		$this->namespace = "";
		$this->timespan = 0;
		$this->timespantype = \OpenAPI\OpenAPI\Models\Shared\TimespanTypeEnum::MONTHS;
		$this->year = 0;
	}
}
