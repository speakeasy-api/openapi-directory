...
export class Subscriptions {
  ...
  postV05SubscriptionRequestsHiuOnInit(
    ...
    const baseURL: string = this._serverURL;
    const url: string =
      baseURL.replace(/\/$/, "") + "/v0.5/subscription-requests/hiu/on-init";
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      if (httpRes?.status == null)
        throw new Error(`status code not found in response: ${httpRes}`);
      const res: operations.PostV05SubscriptionRequestsHiuOnInitResponse =
        new operations.PostV05SubscriptionRequestsHiuOnInitResponse({
          statusCode: httpRes.status,
          contentType: contentType,
          rawResponse: httpRes,
        });
